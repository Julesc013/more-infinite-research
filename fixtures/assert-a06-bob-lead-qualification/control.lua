...
local recipe_name = "bob-lead-plate"
...
local function runtime_effect(technology)
  ...
  for _, effect in ipairs(technology.prototype.effects or {}) do
    ...
    if effect.type == "change-recipe-productivity" and effect.recipe == "bob-lead-plate-2" then fail("excluded Lead alternative gained a runtime owner") end
  end
  if count ~= 1 or change ~= 0.02 then fail("runtime Lead productivity owner differs") end
  return change
end
...
local function prepare_technology(force, completed_level)
  ...
  if not technology then fail("Lead technology absent") end
  ...
  for level = 1, completed_level do
    if technology.level ~= level or not force.add_research(technology) or not force.current_research or force.current_research.name ~= technology_name then fail("could not offer exact Lead level " .. level) end
    force.research_queue = nil
    force.cancel_current_research()
    technology.researched = true
  end
  if technology.level ~= completed_level + 1 then fail("completed Lead level differs") end
  return technology
end

local function no_level_four()
  local force = game.create_force("mir-a06-bob-lead-cap")
  ...
  if not result then fail("effective Lead cap admitted level four") end
  return result
end

local function create_production_case(level)
  local force = game.create_force("mir-a06-bob-lead-production-" .. level)
  prepare_technology(force, level)
  local entity = game.surfaces[1].create_entity{name = "electric-furnace", position = {level * 5, 0}, force = force}
  if not entity or entity.type ~= "furnace" then fail("electric-furnace cannot execute Lead recipe at level " .. level) end
  ...
  if not input or not output then fail("electric-furnace crafting inventories absent") end
  if input.insert{name = "bob-lead-ore", count = input_ore} ~= input_ore then fail("exact Lead ore input could not be established") end
  local ok = pcall(function() entity.energy = 1000000000 end)
  if not ok or not entity.energy or entity.energy <= 0 then fail("electric-furnace electric buffer is unavailable") end
...
local function maintain_production(case)
  ...
  if not ok or not case.entity.energy or case.entity.energy <= 0 then fail("electric-furnace electric buffer disappeared") end
  ...
  if not input or not output then fail("electric-furnace crafting inventories disappeared") end
  local available = output.get_item_count(recipe_name)
  if available > 0 then
    local removed = output.remove{name = recipe_name, count = available}
    if removed ~= available then fail("Lead output drain differs") end
    case.drained = case.drained + removed
    case.drain_events = case.drain_events + 1
  end
  local exhausted = input.get_item_count("bob-lead-ore") == 0
  if exhausted and case.entity.crafting_progress == 0 and output.get_item_count(recipe_name) == 0 then
    if case.drained ~= expected_outputs[case.level] or case.drain_events == 0 then fail("exact Lead output differs at level " .. case.level .. ": " .. case.drained) end
    case.done = true
  end
end
...
local function log_state(stage, state)
  ...
  log("[mir-a06-bob-lead-qualification] STATE stage=" .. stage .. " generation=" .. state.generation .. " predecessor=" .. state.predecessor_generation .. " stable=" .. stable)
end

local function initialize_persisted_state()
  local force = game.forces.player
  local technology = prepare_technology(force, 2)
  if not force.add_research(technology) or not force.current_research or force.current_research.name ~= technology_name then fail("could not establish partially researched Lead level three") end
  force.research_queue = {technology}
  force.research_progress = 0.42
  storage.mir_a06_bob_lead = {generation = 0, predecessor_generation = -1, production = {caps = cap_state(), no_level_four = no_level_four(), cases = {create_production_case(0), create_production_case(1), create_production_case(2), create_production_case(3)}, passed = false}}
  log_state("initial", storage.mir_a06_bob_lead)
end
...
script.on_load(function()
  local state = storage.mir_a06_bob_lead
...
script.on_event(defines.events.on_tick, function(event)
  ...
  if not production.passed then
    ...
    if complete then
      ...
      log("[mir-a06-bob-lead-qualification] PRODUCTION raw=" .. production.caps.raw .. " imported=" .. production.caps.imported .. " effective=" .. production.caps.effective .. " no-level-4=" .. bool(production.no_level_four) .. " input-lead-ore=100 levels=" .. table.concat(levels, ",") .. " inputs-exhausted=true output-drained=true")
    elseif event.tick >= 30000 then
      fail("Lead production did not finish before bounded runtime deadline")
    end
  end
  ...
  if loaded_generation == 0 then
    ...
    game.server_save("mir-a06-bob-lead-cycle-1")
  elseif loaded_generation == 1 then
    state.generation, state.predecessor_generation = 2, 1
    log_state("cycle", state)
    game.server_save("mir-a06-bob-lead-cycle-2")
