...
local allowed_sources = { ["2.5.10"] = true, ["2.5.9"] = true, ["2.5.5"] = true }
...
local function fail(message)
  error("MIR terminal 2.5.x to 2.5.11 " .. archetype .. " upgrade validation failed: " .. message)
end
...
script.on_init(function()
  ...
  if not allowed_sources[source_version] then fail("source save did not use a governed MIR predecessor") end
...
script.on_configuration_changed(function()
  if script.active_mods["more-infinite-research"] ~= "2.5.11" then fail("upgraded save did not use MIR 2.5.11") end
  local state = storage.mir_upgrade_fixture
  if not state or not allowed_sources[state.source_version] then fail("fixture storage did not survive governed source upgrade") end
  ...
  log("[mir-fixture] " .. state.source_version .. " to 2.5.11 upgrade proof complete archetype=" .. archetype)
end)
...
script.on_event(defines.events.on_tick, function()
  ...
  if state and state.upgrade_complete and not state.server_save_requested then
    ...
    game.server_save("mir-2511-upgraded")
...
script.on_load(function()
  if storage.mir_upgrade_fixture and storage.mir_upgrade_fixture.upgrade_complete then
    log("[mir-fixture] 2.5.11 upgraded save reload proof complete archetype=" .. archetype)
  end
end)
