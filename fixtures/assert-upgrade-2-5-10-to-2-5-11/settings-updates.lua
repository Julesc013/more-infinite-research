...
if selector.default_value == "space-age-native-owner" then
  ...
  values["ips-max-level-research_low_density_structure"] = {type = "int-setting", value = 13}
...
for name, row in pairs(values) do
  ...
  if not setting then error("missing 2.5.x to 2.5.11 upgrade setting " .. name) end
