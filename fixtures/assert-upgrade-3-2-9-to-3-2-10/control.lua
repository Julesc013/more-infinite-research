...
local profiles = {
  ...
  ["space-age-native-owner"] = {
    technology = "low-density-structure-productivity", level = 4, requires_space_age = true,
    expected_absolute_cap = 5
  },
...
local function fail(message)
  error("MIR 3.2.9 to 3.2.10 " .. archetype .. " upgrade validation failed: " .. message)
end
...
script.on_init(function()
  if script.active_mods["more-infinite-research"] ~= "3.2.9" then fail("source save did not use MIR 3.2.9") end
  ...
  log("[mir-fixture] 3.2.9 upgrade source proof complete archetype=" .. archetype)
end)

script.on_configuration_changed(function()
  if script.active_mods["more-infinite-research"] ~= "3.2.10" then fail("upgraded save did not use MIR 3.2.10") end
  local state = storage.mir_upgrade_fixture
  if not state or state.source_version ~= "3.2.9" then fail("fixture storage did not survive upgrade") end
  ...
  if archetype == "space-age-native-owner" then
    if tech.prototype.max_level < 4294967295 then fail("native-owner prototype is not infinite for lossless capping") end
    if settings.startup["ips-max-level-research_low_density_structure"].value ~= profile.expected_absolute_cap then
      fail("native-owner absolute cap setting changed")
    end
  elseif archetype == "automatic-family-creation" and not has_recipe_effect(tech, profile.target_recipe) then
  ...
  log("[mir-fixture] 3.2.9 to 3.2.10 upgrade proof complete archetype=" .. archetype)
end)
...
script.on_load(function()
  if storage.mir_upgrade_fixture and storage.mir_upgrade_fixture.upgrade_complete then
    log("[mir-fixture] 3.2.10 upgraded save reload proof complete archetype=" .. archetype)
  end
end)
