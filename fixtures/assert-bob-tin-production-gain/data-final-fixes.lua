...
if mods["angelssmelting"] or mods["angelsrefining"] or mods["angelspetrochem"] then fail("Bob-only production-gain fixture refuses Angel modules") end
...
if #recipe.ingredients ~= 1 or entry_type(recipe.ingredients[1]) ~= "item" or entry_name(recipe.ingredients[1]) ~= "bob-tin-ore" or entry_amount(recipe.ingredients[1]) ~= 1 then fail("exact Bob Tin input differs") end
if #recipe.results ~= 1 or entry_type(recipe.results[1]) ~= "item" or entry_name(recipe.results[1]) ~= "bob-tin-plate" or entry_amount(recipe.results[1]) ~= 1 then fail("exact Bob Tin output differs") end
...
if not furnace or not contains(furnace.crafting_categories, "smelting") then fail("stone-furnace does not witness exact smelting capability") end

local technology = data.raw.technology[technology_name]
if not technology or technology.max_level ~= "infinite" then fail("finite runtime Tin technology prototype differs") end
local owner_count = 0
...
if not profile or type(profile.value) ~= "string" or string.sub(profile.value, 1, 8) ~= "MIRSET1:" or not cap or cap.value ~= raw_direct_cap then fail("distinguishable raw and MIRSET1 cap transport differs") end
local decoded, decode_error = profile_codec.decode(profile.value)
if not decoded or not decoded.settings or decoded.settings["ips-max-level-research_material_tin"] ~= imported_cap then fail("MIRSET1 cap transport does not decode to level three: " .. tostring(decode_error)) end
if startup_settings.raw("ips-max-level-research_material_tin") ~= raw_direct_cap or startup_settings.get("ips-max-level-research_material_tin") ~= imported_cap then fail("effective imported Tin cap does not override raw direct cap") end

log("[mir-bob-tin-production-gain] DATA PASS recipe=" .. recipe_name .. " machine=" .. furnace.name .. " owner=" .. owner_count .. " change=" .. owner_change .. " raw=" .. startup_settings.raw("ips-max-level-research_material_tin") .. " imported=" .. decoded.settings["ips-max-level-research_material_tin"] .. " effective=" .. startup_settings.get("ips-max-level-research_material_tin"))
