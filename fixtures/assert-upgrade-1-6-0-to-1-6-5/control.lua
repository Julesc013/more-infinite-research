local from_version = "1.6.0"
local to_version = "1.6.5"
...
local function fail(message)
  error("MIR 1.6.0 to 1.6.5 retention validation failed: " .. message)
end
...
script.on_init(function()
  ...
  if not force.current_research or force.current_research.name ~= technology_name then
    fail("could not select lab-productivity research")
  end
  ...
  global.mir.target_upgrade_fixture = {
    ...
    runtime_marker = "factorio-0.16-global-state"
  }
  game.server_save("mir-1.6.0-save")
  log("[mir-fixture] 1.6.0 upgrade source proof complete")
end)
...
script.on_configuration_changed(function()
  ...
  if not state or state.runtime_marker ~= "factorio-0.16-global-state" then
  ...
  log("[mir-fixture] 1.6.0 to 1.6.5 upgrade proof complete")
end)
