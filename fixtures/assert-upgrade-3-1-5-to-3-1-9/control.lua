...
local function fail(message)
  error("MIR 3.1.5 to 3.1.9 native-owner upgrade validation failed: " .. message)
end
...
script.on_init(function()
  if script.active_mods["more-infinite-research"] ~= "3.1.5" then
    fail("initial save did not use MIR 3.1.5")
  end
  assert_settings()
  ...
  log("[mir-fixture] 3.1.5 upgrade source proof complete")
end)

script.on_configuration_changed(function()
  if script.active_mods["more-infinite-research"] ~= "3.1.9" then
    fail("upgraded save did not use MIR 3.1.9")
  end
  assert_settings()
  ...
  log("[mir-fixture] 3.1.5 to 3.1.9 upgrade proof complete")
end)
