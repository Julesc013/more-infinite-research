...
local function fail(message)
  error("MIR 2.4.0 to 2.5.0 upgrade validation failed: " .. message)
end
...
script.on_init(function()
  if script.active_mods["more-infinite-research"] ~= "2.4.0" then
    fail("initial save did not use MIR 2.4.0")
  end
  ...
  assert_close("2.4.0 scripted runtime application", game.difficulty_settings.spoil_time_modifier, expected)
  storage.mir_upgrade_fixture = {
    baseline = baseline,
    expected = expected,
    technology_level = tech.level
  }
  log("[mir-fixture] 2.4.0 upgrade source proof complete")
end)
...
script.on_configuration_changed(function()
  ...
  assert_close("2.5.0 scripted runtime retention", game.difficulty_settings.spoil_time_modifier, state.expected)
  log("[mir-fixture] 2.4.0 to 2.5.0 upgrade proof complete")
end)
