...
local profiles = {
  ...
  ["affected-planet-discovery"] = {
    ...
    discovery_technology = "planet-discovery-vulcanus"
  }
}
...
local function fail(message)
  error("MIR 3.2.x to 3.2.2 " .. archetype .. " upgrade validation failed: " .. message)
end
...
script.on_init(function()
  ...
  if source_version ~= "3.2.0" and source_version ~= "3.2.1" then
    fail("initial save did not use MIR 3.2.0 or 3.2.1")
  end
  assert_settings()
  ...
  if profile.space_location then
    local discovery = force.technologies[profile.discovery_technology]
    if not discovery or not discovery.researched then
      fail("planet discovery technology was not researched before simulating the affected save")
    end
    local was_unlocked = force.is_space_location_unlocked(profile.space_location)
    if source_version == "3.2.1" and not was_unlocked then
      fail("3.2.1 source did not retain the researched planet discovery")
    end
    if was_unlocked then
      force.lock_space_location(profile.space_location)
    end
    if force.is_space_location_unlocked(profile.space_location) then
      fail("could not reproduce the affected locked-planet save state")
    end
  end
  ...
  storage.mir_upgrade_fixture = {
    ...
    affected_space_location = profile.space_location,
...
script.on_configuration_changed(function()
  if script.active_mods["more-infinite-research"] ~= "3.2.2" then
    fail("upgraded save did not use MIR 3.2.2")
  end
  assert_settings()
  ...
  if state.source_version ~= "3.2.0" and state.source_version ~= "3.2.1" then
    fail("fixture source-version authority did not survive upgrade")
  end
  ...
  if profile.space_location then
    local discovery = force.technologies[profile.discovery_technology]
    if not discovery or not discovery.researched then
      fail("planet discovery research state did not survive upgrade")
    end
    if not force.is_space_location_unlocked(profile.space_location) then
      fail("researched planet discovery was not restored")
    end
  end
  assert_upgraded_profile(tech)
  log("[mir-fixture] " .. state.source_version .. " to 3.2.2 upgrade proof complete archetype=" .. archetype)
end)
