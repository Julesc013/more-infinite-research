...
local function assert_no_level_four()
  local force = game.create_force("mir-f200-bob-tin-production-gain-cap")
...
local function create_case(level)
  local force = game.create_force("mir-f200-bob-tin-production-gain-" .. level)
...
script.on_init(function()
  if not script.active_mods.boblibrary or not script.active_mods.bobores or not script.active_mods.bobplates or script.active_mods.angelssmelting or script.active_mods.angelsrefining or script.active_mods.angelspetrochem then fail("fixture requires exact F200 Bob-only closure") end
  local caps = assert_cap_transport()
  local no_level_four = assert_no_level_four()
  storage.mir_f200_bob_tin_production_gain = {cases = {create_case(0), create_case(1), create_case(2), create_case(3)}, caps = caps, no_level_four = no_level_four, passed = false}
end)

script.on_event(defines.events.on_tick, function(event)
  local state = storage.mir_f200_bob_tin_production_gain
  ...
  if complete then
    state.passed = true
    log("[mir-f200-bob-tin-production-gain] RUNTIME PASS " .. summarize_runtime_observation(state.cases, state.caps, state.no_level_four))
