local technology_name = "recipe-prod-research_processing_unit-1"

local function fail(message)
  error("MIR Factorio 2.0 generated cap-transition validation failed: " .. message)
end
...
script.on_init(function()
  ...
  if not technology then fail("missing generated processing-unit productivity technology") end
  technology.level = 8
  ...
  storage.mir_generated_cap_transition_2_0 = {level = technology.level}
end)

script.on_configuration_changed(function()
  storage.mir_generated_cap_transition_2_0_pending = true
end)

script.on_event(defines.events.on_tick, function()
  if not storage.mir_generated_cap_transition_2_0_pending then return end
  storage.mir_generated_cap_transition_2_0_pending = nil
  local force = game.forces.player
  local technology = force.technologies[technology_name]
  if technology.prototype.max_level < 4294967295 then fail("prototype became finite") end
  if technology.level ~= storage.mir_generated_cap_transition_2_0.level then
  ...
  log("[mir-fixture] Factorio 2.0 lowered cap retained completed levels and removed invalid research")
end)
