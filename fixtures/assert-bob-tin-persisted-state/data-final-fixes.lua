...
if not technology then fail("published Tin anchor is absent") end
if technology.max_level ~= "infinite" then fail("Tin anchor prototype maximum level differs") end
local recipe = data.raw.recipe[recipe_name]
if not recipe or recipe.allow_productivity ~= true then
  fail("final Bob Tin recipe does not explicitly permit productivity")
end
...
if owners ~= 1 or change ~= 0.02 then
  fail("Tin anchor must have exactly one 0.02 productivity owner")
end

log("[mir-bob-tin-persisted-state] DATA JSON {\"technology\":\"" .. technology_name ..
  "\",\"recipe\":\"" .. recipe_name .. "\",\"owner_count\":" .. owners ..
  ",\"effect_change\":" .. tostring(change) .. ",\"prototype_max_level\":\"infinite\"}")
