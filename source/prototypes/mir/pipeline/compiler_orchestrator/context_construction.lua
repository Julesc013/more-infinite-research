...
local maximum_level_binding = target_line.feature_enabled("scripted_techs")
  and require("prototypes.mir.domain.technology." .. "maximum_level_binding") or nil
...
local function compile_active(context)
  ...
  local maximum_level_policy
  if maximum_level_binding then
    maximum_level_policy = maximum_level_binding.from_plan(latest, {
  ...
  if maximum_level_policy then context:set_state("maximum_level_policy", maximum_level_policy) end
...
function M.maximum_level_policy(context)
  if not maximum_level_binding then return nil end
  context = context or compiler_context.current()
  M.compile(context)
