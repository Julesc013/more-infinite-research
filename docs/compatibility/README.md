...
Use `Test-MIRLocalModLibraryCatalog.ps1` before a local sweep when you need a metadata-only gate. It reads local zip `info.json` files, catalogs available mod names, and fails if an external root named by the committed local-library scenarios is missing. Repository assertion fixtures are resolved by their checked-in `info.json` identities and are reported separately from external archives.
