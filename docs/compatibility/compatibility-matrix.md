...
| `atan-ash` | Supported in `2.2.0` | MIR-owned productivity for exact ash separation only | Static and runtime fixture with Ash-style sink recipes | Targets `atan-ash-seperation` only. Landfill, brick, nutrient, foundation, tile, and recovery-style ash sinks are excluded even when productivity-allowed. |
