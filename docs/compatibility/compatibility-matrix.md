...
| Capability policy and claim lints | Supported in `2.2.0` as static platform guard | Lints capability policy, generated stream manifest rows, support lanes, and compatibility claims | `validation\tests\tooling\Test-MIRPolicyLints.ps1` through static validation | Prevents generated streams without manifest entries and public claims without fixture backing. |
...
| `modules-t4` | Partial support in `3.0.5` | Existing `research_modules` discovers final module prototypes by tier | Tier-4 fixture plus `modules-t4_2.2.2` source-archive load check on Factorio 2.1.9 | Tier 4 and later recipes receive +1% per level. MIR does not rewrite module effects or add dynamic module-effect research. |
| `finite_prod_techs` | Cooperates in `3.0.5` | External load-after post-processing of MIR's final emitted productivity technologies | `finite_prod_techs_0.1.1` source-archive load check on Factorio 2.1.9 | Upstream declares the hidden optional dependency on MIR. MIR does not add a reverse dependency or own the finite-level formula. |
