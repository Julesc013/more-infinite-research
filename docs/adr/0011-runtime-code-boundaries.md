...
handlers through `script` and delegate to `control/` modules, but it must not
...
Runtime systems remain narrow and conservative. Runtime compatibility shims may
exist for old paths, but the maintained boundary is the stage control entrypoint
plus the `control/` runtime modules.
