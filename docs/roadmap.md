...
- The local static validation suite, package validation, locale validation, and Factorio runtime fixture load checks for both `reduce` and `skip` lab incompatibility policies, science-pack ingredient policy modes, the end-game prerequisite gate, base-only cargo skip behavior, Space Age cargo logistics shape, and finite vanilla-chain preservation are available.
