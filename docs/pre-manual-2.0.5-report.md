...
- `vanilla-locale-icons`: base game without Space Age; confirm Electric Shooting Speed falls back to discharge defense art/description and flamethrower/electric descriptions display.
- `space-age-electric-icon`: fresh Space Age save; confirm Electric Shooting Speed uses the electric-weapons-damage texture and has both `electric` and `tesla` effects.
