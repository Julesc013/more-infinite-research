...
```powershell
.\validation\tests\release\Test-MIRBackportManifest.ps1 -AllowPendingTags
```
...
The materializer verifies both tags and the 2.4.9 ancestry, creates a target-first/source-second merge, reapplies the exact reviewed Factorio 2.0 projection tree, checks the target profile and source lock, builds twice, verifies the expected archive and content hashes, and writes a reconstruction receipt. The merge author and committer date is deterministically one second after the newest parent, so independent rehearsals produce the same integration commit. The receipt stores `receipt_material_sha256` over every deterministic non-time field; `reconstructed_at` is an observation-only envelope field, so two rehearsals can prove identical reconstruction material without pretending their timestamps match. Validate both receipts with `validation/tests/release/Test-MIRBackportReconstruction.ps1`; pass `-OutputPath` to write a portable governed summary containing receipt hashes instead of machine-local receipt paths.
