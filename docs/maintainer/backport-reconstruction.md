...
Before integration, create and publish the manifest's annotated `archive/2.5-pre-3.2.2-q1` tag at the locked preintegration commit. This tag retains the complete reviewed target projection history even if `tmp/2.0` is later deleted.
