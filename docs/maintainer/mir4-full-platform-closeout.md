...
Open blockers are not release defects that this adapter may waive. The original closeout record correctly prohibited Dev integration within that evidence run and remains immutable. PR #151 was subsequently reviewed, passed the hosted MIR gates, and merged to `dev` at commit `3a58d3753bedcaf1107b76655117e0ff84dac960` under separate integration authority. `.mir/releases/waves/mir4-r0/MIR4-Release-Readiness-ReconciliationV1.json` remains the append-only reconciliation of that integration and is not rewritten.

The current pre-freeze release cut and execution queue are owned by `.mir/releases/waves/mir4-r0/MIR4-Pre-Freeze-Execution-ProgrammeV1.json`. It binds the later PR #153 baseline, records uploaded handoff material as planning evidence rather than command authority, makes release-workflow executor maturity an explicit blocker, and projects `dashboard.json`, `queue.json`, and `todo.md`. T02 adds a tested non-production phase kernel but does not upgrade any phase adapter. Historical M4 and MIR 3 records remain evidence, not the active queue.
