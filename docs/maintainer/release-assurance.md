...
Fast, targeted, and scheduled planners publish their plan-and-candidate transport under an attempt-qualified name, and each downstream job downloads only the exact planner output selected through `needs.plan.outputs`. Matrix workers likewise upload only their exact `test-id/fingerprint` subtree under `<prefix><run-id>-<run-attempt>-<safe-test-id>-<fingerprint>`. Every completed worker attempt, including a failed attempt, writes a receipt schema V3 object at `worker-receipts/<plan-material-sha256>/<receipt-sha256>.json`; `current.json` is only a locator for ordinary one-shot import. V3 binds the semantic plan and required-test-set digests, plan generation/source/target/profile, plan coordination producer, exact work row and freshness policy, contribution-preparation workflow/run/attempt/job identity, evidence producer and trust, completion state, immutable capsule hash, and result digest. On a GitHub partial rerun, the unchanged planner output permits the aggregate to download `<prefix><run-id>-*`, supply the independently observed current aggregate attempt, and import every bounded worker attempt at or below that attempt. A re-run of all jobs creates a new attempt-qualified plan/candidate transport. A well-formed, trusted worker receipt from the prior plan for the exact bounded transport and work row is reported as `stale-plan-transport` and ignored; it is never selected or published as proof for the new plan. Malformed prior-plan transport and every mismatch under the current plan fail closed. The importer validates every current-plan receipt, capsule, structured result, stdout/stderr, and declared artifact digest before publishing any destination object or pointer; it then records every trusted current-plan attempt for contradiction handling and selects the newest attempt only when all retained attempts agree. Same-attempt duplicates, stale or untrusted artifacts, a malformed newer attempt, and contradictory outcomes fail closed; contradictory exact evidence is quarantined and cannot yield a reusable pass. Missing and failed rows do not stop later independent rows, and the summary remains available even when proof closure fails. Whole-ledger artifact merging and `merge-multiple: true` are forbidden because creation or extraction order must never select evidence.
