...
- Version `2.1.0` also ships local compatibility-audit tooling for release maintainers: curated scenario execution, sharded Mod Portal audits, per-scenario timeouts, grouped expected/unexpected failure summaries, strict and exploratory wrapper modes, and review-only profile-stub generation.
