...
- Added local compatibility-audit automation for release work: executable curated scenarios, sharded/resumable Mod Portal audits, grouped failure summaries, review-only profile-stub generation, and a self-hosted extended-test workflow.
...
- Extended audit tooling can run static/runtime/smoke gates, top-25 base and Space Age audits, curated manual modpack scenarios, and opt-in sharded `downloads_count >= 10000` audits when a local Factorio binary and Mod Portal credentials are supplied.
