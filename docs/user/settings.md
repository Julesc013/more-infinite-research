...
Every generated research row has three cost controls: base cost, additive cost per later controlled level, and an exponential multiplier. For generated streams and recognized existing owners, base cost is the cost at the first controlled level. Base-continuation setting IDs retain their older profile meaning as the level-one coefficient; MIR projects that coefficient to the first new continuation level before applying the new additive increment. Additive cost defaults to `0`, so existing 3.2.3 continuation curves and saved profiles do not change unless you opt in. Multiplier `1` with additive cost `0` is fixed; multiplier `1` with a positive additive cost is linear; a multiplier above `1` can be purely exponential or combined with the additive value. MIR derives the curve from these numbers, so there is no separate model dropdown.
...
Leaving the settings at their defaults preserves the final existing technology exactly, including values changed by another mod. Turning Enable off also leaves the external technology untouched. A deliberate non-default value configures the recognized existing owner instead of silently ignoring the setting. Effect values use percentage points and change only recipe-productivity effects in that stream's product family; unrelated technology effects remain intact. Maximum level `0` means infinite. A positive maximum is the absolute highest technology level, not a number of extra levels. MIR hides Factorio's infinity badge and shows the exact selected maximum in the technology description while retaining an internal infinite prototype so completed levels are never lost when a cap is lowered. Invalid current or queued research is removed, and raising the maximum or returning it to `0` makes future levels available again.
