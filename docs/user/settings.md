...
The Automatic compiler mode controls structural family handling. `safe-attach` is the default and may attach structurally proven recipes to an existing stable MIR technology. `safe-generate` also enables the reviewed generic assembling-machine and lab technology families. `report` records decisions without attaching or generating, and `off` disables family handling. `exact-pack` keeps the safe attachment baseline and generates only a family explicitly authorized by an active exact-version, fixture-backed pack. Generation remains fail-closed when any hard safety proof is missing.
