...
- The pipeline extent multiplier is a startup-only prototype setting. Its default `1x` value leaves vanilla and modded fluid boxes unchanged. When enabled, the current implementation scales recognized fluid box fields across prototypes, not only pipe entities, so higher values are experimental for machines, tanks, thrusters, and modded prototypes that define their own fluid boxes.
