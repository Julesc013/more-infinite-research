...
Maximum level `0` means infinite. A positive integer is the absolute highest technology level permitted, not a count of additional levels. On script-capable targets, MIR keeps the managed prototype internally infinite so Factorio cannot discard completed levels before configuration-change recovery runs. For a positive cap, MIR hides the native infinity badge, keeps the technology visible when the cap disables it, and appends the exact selected maximum to the localized technology description. Runtime retains completed levels, removes invalid current or queued research, preserves fractional progress when research remains valid, and blocks future levels above the cap. Raising the cap or returning it to `0` restores future research. This contract is identical for generated streams, generated base continuations, native owners, direct startup values, and effective MIRSET1 imports.

If a later data-stage finalizer changes a managed prototype away from the required internal infinite maximum, MIR refuses runtime queue normalization and logs the setting, selected and planned cap, final observed maximum, binding operation, source, and conflict reason. It does not claim that the user's cap is active.
