...
| `technology_design` | Required schema-2 common design IR for every `emit` row, with typed subjects, per-field provenance, executable locks, authority-derived identity state, independent fingerprints, and multi-axis maturity. |
...
Whole-plan validation rejects duplicate stream keys, emitted manifest IDs, technology names, owner/recipe adoption identities, duplicate semantic effect identities across emission and adoption, any materializing row whose proof gate fails, and any emitted legacy projection that disagrees with `TechnologyDesign`. After all mutations, the compilation output validator compares full normalized effect values with a declared tolerance plus prerequisites, science ingredients, count formula, research time, maximum level, localized name, localized description, icons, order, level, enabled/hidden/upgrade state when declared, and generated-registry records. Stream adoption is checked as an exact expected-effect subset; emitted stream and base-extension technologies require exact planned shapes. Snapshots are deep copies.
