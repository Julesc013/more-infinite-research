...
| Pyanodons Post-processing can reconstruct stale technology unlocks during `data-final-fixes.lua` | `pypostprocessing-stale-unlock` lifecycle fixture reproducing `casting-mk02 → casting-gear` | Fixture-proven on Factorio 2.0.77 | The synthetic 2.0.77 fixture proves late stale-unlock removal; the exact Py 2.0 closure retains `casting-gear` and passes target integrity with zero external prunes. This is startup-integrity ordering, not broad Py support |
