...
| `EffectReceiver` exposes consumption, speed, and quality effect limits with documented bounds | Factorio `EffectReceiver` docs | Verified | MIR's efficiency, speed, and quality cap settings can mutate effect receiver limit tables during prototype loading |
