...
| Pyanodons Post-processing can reconstruct stale technology unlocks during `data-final-fixes.lua` | `pypostprocessing-stale-unlock` lifecycle fixture reproducing `casting-mk02 → casting-gear` | Fixture-proven on Factorio 2.0.77 | The 2.0 line declares `(?) pypostprocessing`, sanitizes after final Py reconstruction, removes exactly the missing unlock, and retains valid siblings in order; this is startup-integrity ordering, not broad Py support |
