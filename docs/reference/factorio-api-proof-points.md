...
| Factorio `1.1` generated direct-effect technologies can use target-era base technology art as effect badges | Runtime fixture `mir-fixture-assert-legacy-effect-icons` in the `factorio-1.1-direct-effects` validation scenario | Locally proven for `1.9.3` | Older rings should define target-line overlay mappings instead of bundling newer core constant graphics |
