...
| Pyanodons Post-processing can reconstruct stale technology unlocks during `data-final-fixes.lua` | `pypostprocessing-stale-unlock` lifecycle fixture reproducing `casting-mk02 → casting-gear` | Fixture-proven | The main line declares `(?) pypostprocessing`, sanitizes after the final Py reconstruction, removes exactly the missing unlock, and retains valid siblings in order; this is startup-integrity ordering, not broad Py support |
| Angel Smelting finalizes supported F200 Bob/Angel science-production routes during `data-final-fixes.lua` | Exact Factorio 2.0.77 `local-2-0-bob-angels` campaign and package-excluded final-state fixture | Exact-campaign proof required | F200 declares `(?) angelssmelting` so MIR plans against Angel's finalized recipe graph; the edge is load ordering, not a required dependency or an unbounded Angel support claim |
