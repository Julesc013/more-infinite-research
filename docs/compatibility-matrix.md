...
| Compatibility planner/registry | Started in `2.1.5`; broader policy planned for `2.2.0` | Diagnostics-only role rows, non-actions, and planner summary | Static validation and audit rows | Foundation before broad feature absorption. |
| Cap-aware diagnostics | Started in `2.1.5`; broader policy planned for `2.2.0` | Warning diagnostics for non-default recipe caps | Runtime diagnostics with default, raised, lowered, and extreme recipe caps | No silent cap mutation. |
...
| `atan-air-scrubbing` | Planned for `2.2.0` | Candidate MIR-owned productivity for clean pollution and spore filter recipes only | Needs exact recipe fixtures and targeted external load pass | Do not touch scrubbing or cleaning recipes; pollution/spore removal efficiency is not a MIR core feature. |
