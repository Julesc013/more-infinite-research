...
| `atan-air-scrubbing` | Requested, planned candidate | Candidate MIR-owned productivity for clean filter recipes only | Needs local zip, load test, and fixtures for `atan-pollution-filter` and `atan-spore-filter` | Do not touch scrubbing or cleaning recipes; pollution/spore removal efficiency is not a MIR core feature. |
| `better-bot-battery2` | Requested, native-overlap candidate | Prefer external or skip MIR worker robot battery stream when `worker-robots-battery-6` is present | Needs fixture proving finite levels survive and MIR duplicate is skipped | MIR already has worker robot battery, but Better Bot Battery uses different finite and infinite values. |
