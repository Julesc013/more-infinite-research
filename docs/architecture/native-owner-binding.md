...
The emitted schema-3 binding artifact carries compact, versioned input and output research-cost descriptors with separate semantic, provenance-bound authority, and qualification identities. On a configuration change, runtime state compares the previous output descriptor with the new output descriptor and preserves completed unit-equivalent work as `old fraction × old realized cost ÷ new realized cost`, clamped to Factorio's `[0,1]` progress range. It does not reparse formula strings. This preserves the player's level, current research selection, queue, and completed work while the normalized fraction changes with cost. Missing, malformed, digest-mismatched, or over-budget descriptors are logged and refused without mutating progress.
