...
`MIR4LocalPlaytestShadowAuthorizationV1` is the historical first private-lane record. `MIR4PrivateLaneAuthorizationV2` supersedes it for current construction, advances only f200 to the immutable 2.5.10 predecessor, and retains f110 and f100 unchanged beneath ignored `build/mir4/local-playtest-shadow`. It leaves release admission blocked by MIR 3 custody and EOL. It forbids public identity, tags, `dist/` writes, uploads, publication, production seals, public release or beta claims, broad support claims, wildcard targets, and generic gate waivers. Because the artifacts use intended numeric versions, transfer is restricted to maintainer-controlled private machines.
