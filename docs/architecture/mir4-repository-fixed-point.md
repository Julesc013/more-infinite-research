...
MIR 4 now migrates repository authorities one bounded family at a time while the proven player-package source remains fixed. `.mir/control/repository-fixed-point.json` owns the root state, `.mir/control/paths.yml` owns logical path resolution, and `.mir/control-plane/ownership.json` owns writer selection. `governance/repository/migrations/fixed-point-tooling-v1.json` is the separate authority for the first package-excluded tooling and test cutover. Canonicalization, diagnostics, target-key identity, whole-platform tooling, technology acceptance, target compilation, semantic compiler/policy tooling, runtime continuity, module/SDK/MEP, ProcessIR/exact tooling, Inspector/compatibility tooling, assurance/offline-custody tooling, and historical tooling are byte-pinned append-only predecessors. `governance/repository/migrations/release-tooling-v1.json` is the current fifteenth successor for the package-excluded read-only release-DAG validator, focused tests, current-consumer cutover, and release-history successor proof. It does not replace the release phase engine, compatibility policy, immutable T14 evidence, or any player, signing, sealing, promotion, tagging, or publication authority.
...
Run `./tools/mir.ps1 mir4 repository check` for root, historical-receipt, compatibility-forwarder, and inventory parity. `repository generate` may refresh only root-marker projections; it verifies but never rewrites an immutable receipt. Run the earlier migration `check` commands for their byte-pinned historical receipts, and `./tools/mir.ps1 mir4 inspector-compatibility-migration check` for the current successor and complete append-only chain. Run `initialize` only in the approved machine context to create and persist the external non-secret roots. None of these commands authorizes deletion, source freeze, signing, promotion, tagging, or publication.
