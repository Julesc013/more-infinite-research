...
MIR 4 now migrates repository authorities one bounded family at a time while the proven player-package source remains fixed. `.mir/control/repository-fixed-point.json` owns the root state, `.mir/control/paths.yml` owns logical path resolution, and `.mir/control-plane/ownership.json` owns writer selection. `governance/repository/migrations/fixed-point-tooling-v1.json` is the separate authority for the first package-excluded tooling and test cutover. Canonicalization and diagnostics are byte-pinned append-only successors. `governance/repository/migrations/target-key-tooling-v1.json` is the current successor for stable target-key identity. None replaces any player authority.
...
Canonical JSON has one implementation at `tools/mir/domain/canonicalization/CanonicalJsonV1.ps1` and one governed functional test at `tests/canonicalization/Test-MIR4CanonicalizationDiagnostics.ps1`. Stable diagnostics has one implementation at `tools/mir/domain/diagnostics/DiagnosticsV1.ps1` and a focused parity test at `tests/diagnostics/Test-MIR4DiagnosticsV1.ps1`. Both accepted migration commands now perform only byte, schema, digest, chain, and firewall verification; `generate` fails closed. Target-key identity has one implementation at `tools/mir/domain/targets/TargetKey.ps1`, a focused parity test at `tests/targets/Test-MIR4TargetKey.ps1`, and the current successor writer at `tools/mir/application/targets/TargetKeyMigration.ps1`. The former canonicalization, diagnostics, target-key, and T07 paths are read-only forwarders. All compatibility paths remain until every declared consumer uses the final paths, result parity and rollback have been rehearsed, and a separate sunset receipt is accepted.
...
Run `./tools/mir.ps1 mir4 repository check` for root, historical-receipt, compatibility-forwarder, and inventory parity. `repository generate` may refresh only root-marker projections; it verifies but never rewrites an immutable receipt. Run the canonicalization and diagnostics migration `check` commands for their byte-pinned historical receipts, and `./tools/mir.ps1 mir4 target-key-migration check` for the current successor and complete append-only chain. Run `initialize` only in the approved machine context to create and persist the external non-secret roots. None of these commands authorizes deletion, source freeze, signing, promotion, tagging, or publication.
