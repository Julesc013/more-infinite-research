...
`prototypes/mir/pipeline/compiler_context.lua` is the lifetime owner for mutable state derived from `data.raw`, startup settings, active mods, or the current compilation pass. CompilerContext schema 2 owns recipe, recipe-risk, relationship, science-pack, progression, lab-input, compatibility-policy, family-resolution, GenerationPlan, CompilationPlan, generated-technology, native-owner-adoption, graph-parity, coverage, telemetry, and diagnostic state. When generation diagnostics or automatic compiler preview/report mode requests lifecycle detail, it also owns the context-local schema-2 TechnologyCatalog and TechnologyQualification set; normal player loads do not construct those full internal artifacts. Modules may hold an internal read-only view into the active context during one data-final-fixes run, but may not keep those values in process-wide `canonical` or `latest` caches. Context snapshots are deep copies, and separate contexts cannot observe each other's derived state. The Factorio stage explicitly requests the live context because it discards the return value; tests and review tooling retain snapshot-by-default compatibility. Immutable built-in provider, FamilyRule, and registered operator catalogs may remain module-scoped because they are policy inputs rather than run-derived compiler state.
