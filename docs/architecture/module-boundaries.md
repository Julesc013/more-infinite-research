...
Research cost semantics live under `prototypes/mir/domain/research_cost/`. The schema-1 `ResearchCostModel` owns the anchor level, first-level base cost, non-negative additive increment, exponential multiplier of at least one, derived fixed/linear/exponential/hybrid kind, canonical Factorio formula, provenance, and separate semantic, authority, and qualification digests. Validation and formula compilation are pure; classification uses byte-, token-, and depth-bounded arithmetic parsing and never executes third-party Lua. `transition_descriptor.lua` is the compact versioned data/control boundary: emission publishes old and new descriptors, while runtime evaluates realized Factorio unit counts without reparsing formula text and preserves completed work as `old fraction × old cost ÷ new cost`. Unknown, malformed, digest-mismatched, or over-budget descriptors fail closed without changing progress. Generated streams, native-owner adoption, and vanilla continuations all consume the canonical model. An unchanged external formula is preserved verbatim; an explicit supported override projects to the canonical formula, while an unknown external formula rejects the override without mutating the technology.
