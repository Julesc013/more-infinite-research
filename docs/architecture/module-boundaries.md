...
The deterministic MIR32 transport identity is unchanged by the C16 performance work. Its canonical hash implementation reads eight bytes per Lua/C boundary crossing but applies the same recurrence and prime modulus after every byte, so existing source, design, evidence, and artifact fingerprints remain byte-for-byte stable.
