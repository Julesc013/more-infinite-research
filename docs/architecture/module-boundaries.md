...
The frozen historical presentation records preserve their exact scopes: baseline V1 covers PR #203/T14, V2 covers M41-05B/F2E, V3 covers the 447/439 byte-preserving composable-source layout receipt, and V4 is the immutable Factorio-1-convergence predecessor. The current package-presentation authority is the package-excluded `spec/distribution/mir4-current-package-presentation-v5.json`, generated and checked only by `tools/commands/mir4/Update-MIR4CurrentPackagePresentationV5Authority.ps1`. V5 binds immutable V4 to the canonical `source` and `targets` roots, exact materialized content identities, the F210/F200 progression application, and the explicit F110/F100 omission/nonclaim. F210/F200 remain blocked on exact-engine qualification; V5 keeps private build, qualification, seal, main-promotion, version allocation, tagging, signing, sealing and publication disabled. `tools/lib/mir4/PackagePresentation.ps1` routes current readers through V5; explicit historical readers retain immutable V1–V4 evidence.

The separate M41-to-M42 composable-source succession is also append-only. Its V3 receipt is immutable progression evidence, while V4 records the typed proof-input/control-plane successor, binds V3 and the exact V5 package identity, and leaves every transition gate except development merge false. A later package-presentation change must append a successor that binds V4; it must not rewrite V3 or V4 to make a newer package look historical.
