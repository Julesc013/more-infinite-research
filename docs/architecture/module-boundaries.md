...
`prototypes/mir/planner/compiler.lua` is the pure compiler entry point. Its complete arguments are `CompilationSnapshot` schema 1 and `PolicySnapshot` schema 1. It may require only deterministic core/domain/planner contracts; direct or transitive reads of `data.raw`, settings, loaded mods, logging, clocks, telemetry, or `CompilerContext` are forbidden. `pipeline/compilation_snapshot_adapter.lua` and `pipeline/policy_snapshot_adapter.lua` compose the outer Factorio and governance adapters.
