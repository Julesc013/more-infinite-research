...
`MIR4-Inspector-Compatibility-ProgrammeV1.json` owns only package-excluded copied DTO indexes, multidimensional private support assessment, the offline Inspector V1 workbench, and data-only compatibility-factory bundles. `SupportAssessment.ps1` cannot create or raise terminal claims, and `CompatibilityIndex.ps1` cannot expose `CompilerContext`, prototypes, operations, executors, callbacks, secrets, or unbounded source objects. `CompatibilityFactory.ps1` emits only its exact allowlisted evidence/data ZIP and cannot emit Lua, PowerShell, prototype, runtime, migration, or arbitrary-code content. `Inspector.ps1` owns V1 assets; `PlatformPreview.ps1` only composes them and preserves Inspector V0 unchanged. Terminal compatibility policy, claim registry, planner, emitter, runtime, state, and migration code remain authoritative. SOL07 evidence is historical, private, target-bound, and nontransferable; absent IR3/IR4/SE/Angel/Py closures remain review or extension blockers rather than support claims.
