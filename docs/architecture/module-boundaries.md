...
The frozen, historical `spec/distribution/mir4-package-presentation-baseline-v1.json` preserves the exact PR #203 and T14 package-presentation assertions. The current package-presentation authority is the package-excluded `spec/distribution/mir4-current-package-presentation-v2.json`, generated and checked only by `tools/commands/mir4/Update-MIR4CurrentPackagePresentationV2Authority.ps1`. It binds the accepted M41-05B documentation cutover, F2E package-authority cutover, and M41 current-product bridge-retirement receipts; it fails closed on the canonical `source|targets` fingerprint, materializer ABI, sole writer, manifest cross-binding, legacy-root retirement, and disabled allocation, tag, signing, sealing, and publication gates. `tools/lib/mir4/PackagePresentation.ps1` retains V1 solely for explicit historical source identities and uses V2 for the current checkout.
