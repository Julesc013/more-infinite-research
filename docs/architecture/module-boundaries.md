...
`prototypes/mir/index/recipe_facts.lua` is the build-once normalized recipe authority. It scans final recipe prototypes once after target-local mutation passes, resolves target-aware effective recipe properties, preserves the full product shape, stores private copied facts, and builds output/category indexes. Read-only consumers share the context-owned `view()`, `index_view()`, `for_each()`, cached `fingerprint()`, and `summary()` surfaces; complete deep-copied snapshots are reserved for callers that require ownership. `prototypes/mir/index/relationships.lua` owns separate immutable `input` and `output` indexes so post-emission assertions never reuse a pre-emission technology cache. Internal read-only consumers share `view(phase)`; `snapshot(phase)` is the defensive-copy boundary for exports and independently owned values. Recipe-productivity matching, science recipe-unlock facts, compatibility diagnostics, and the diagnostic registry query these authorities rather than rescanning `data.raw.recipe`. The shared owner lookup reuses the phase-scoped technology-effect view rather than deep-copying or scanning the complete relationship graph once per recipe.
