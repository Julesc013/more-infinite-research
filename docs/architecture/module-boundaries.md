...
The frozen, historical `spec/distribution/mir4-package-presentation-baseline-v1.json` preserves the exact PR #203 and T14 package-presentation assertions. V2 is also frozen evidence for the M41-05B documentation and F2E package-authority cutovers; its verifier must not reconstruct the retired `src/mod` layout. The current package-presentation authority is the package-excluded `spec/distribution/mir4-current-package-presentation-v3.json`, generated and checked only by `tools/commands/mir4/Update-MIR4CurrentPackagePresentationV3Authority.ps1`. V3 binds the immutable V2 receipt to the append-only composable-source successor: `source/package-source.json`, the registry and all four composition records, the source-layout authority/proof/receipt, the `source|targets` fingerprint, materializer ABI, sole writer, legacy-root retirement, and disabled private-build, qualification, seal, main-promotion, allocation, tag, signing, sealing, and publication gates. `tools/lib/mir4/PackagePresentation.ps1` retains V1/V2 solely for explicit historical source identities and uses V3 for the current checkout.
