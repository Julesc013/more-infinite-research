...
`prototypes/mir/pipeline/commands.lua` is the explicit execution boundary for data-final-fixes passes. Every pass has a stable command ID, kind, owned implementation, and positive target feature requirements. Stage wrappers preserve Factorio lifecycle order but do not invoke mutators directly. Emission commands are the only commands allowed to create generated technology prototypes; mutation commands may update existing prototypes only through their declared implementation. The exact-version compatibility repair remains separately bounded and audited.
