...
`prototypes/mir/index/recipe_facts.lua` is the build-once normalized recipe authority. It scans final recipe prototypes once after target-local mutation passes, stores private copied facts, and builds output/category indexes. Recipe-productivity matching, science recipe-unlock facts, compatibility diagnostics, and the diagnostic registry query it rather than rescanning `data.raw.recipe`. Mutation passes may still iterate live prototypes when applying commands; the recycler safety classifier is explicitly a phase-local pre-mutation index. Neither becomes a competing general fact authority.

`prototypes/mir/planner/generation_plan.lua` is the side-effect-free boundary between stream planning and emission. The compiler plans all fixed and automatic stream decisions, validates duplicate stream, manifest, technology, and adoption identities across the whole set, and only then applies the finalized plan. External-family adoption is also deferred until apply time; planning may inspect owners but may not mutate them.

`prototypes/mir/pipeline/commands.lua` is the explicit execution boundary for data-final-fixes passes. Every pass has a stable command ID, kind, phase, dependency list, owned implementation, and positive target feature requirements. The registry validates dependencies, rejects cycles, exposes a deterministic topological order, and fails if a stage invokes a command before its dependencies. Stage wrappers preserve Factorio lifecycle order but do not invoke mutators directly. Emission commands are the only commands allowed to create generated technology prototypes; mutation commands may update existing prototypes only through their declared implementation. The exact-version compatibility repair remains separately bounded and audited.
