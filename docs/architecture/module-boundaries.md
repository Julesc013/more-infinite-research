...
`prototypes/mir/pipeline/compiler_context.lua` is the lifetime owner for mutable state and services derived from `data.raw`, startup settings, active mods, or the current compilation pass. CompilerContext schema 3 owns recipe, recipe-risk, relationship, science-pack, graph-wide researchability, lab-input, compatibility-policy, family-resolution, TechnologyCatalog, qualifications, GenerationPlan, CompilationPlan, generated-technology, native-owner-adoption, graph-parity, coverage, telemetry, diagnostics, services, freeze state, and state epochs. Every compilation constructs the schema-2 TechnologyCatalog; diagnostics control detailed publication, not semantic existence. Services are registered per context and may be frozen before planning; science modules never configure process-global callbacks. State is set once unless the caller performs an explicit matching-epoch replacement, frozen state cannot be replaced, snapshots are observationally pure deep copies, and separate contexts cannot observe each other's derived state. Immutable built-in provider, FamilyRule, promotion, and registered operator catalogs may remain module-scoped because they are source-owned policy inputs rather than run-derived compiler state.
...
`prototypes/mir/planner/generation_plan.lua` is the side-effect-free boundary between stream planning and emission. The compiler plans all fixed and automatic stream decisions, validates duplicate stream, manifest, technology, adoption, and semantic effect identities across the whole set, and only then applies the finalized plan. Evidence-bearing gates have explicit `not-applicable`, `pending`, `passed`, `failed`, and `superseded` states. The context-owned schema-2 TechnologyCatalog is always constructed with every applicable materializing and safe-diagnostic alternative and its independent SafetyQualification before deterministic current selection. GenerationPlan is an exact projection of that selection. The catalog has no mutation or promotion authority, and a rejected alternative cannot become current. External-family adoption is deferred until apply time; planning may inspect owners but may not mutate them. Every accepted adoption carries a schema-2 `patch-existing` TechnologyDesign that binds the immutable owner input, complete expected output, target, operation, and configured fields. `prototypes/mir/planner/compilation_plan.lua` also plans base continuations before the first generated technology is emitted, converts accepted continuation operations to schema-2 `TechnologyDesign`, and re-derives their projection after sanitation and cross-operation policy.
