...
`src/mod/families/modern/prototypes/mir/domain/technology/maximum_level_binding.lua` is the canonical F210/F200 player-package maximum-level authority across generated streams, base continuations, and native owners. The retained root copy is characterization only and cannot be emitted by a player package. Its schema-3 `MaximumLevelBinding` records exact technology and semantic identities, deterministic exact-technology through global precedence, setting/profile provenance, requested and effective absolute caps, target-specific prototype/runtime/presentation strategies, migration behavior, proof, and finalizer observations. Planning keeps managed technologies prototype-infinite on script-capable targets so Factorio cannot discard completed levels before configuration-change recovery runs. After output validation, the compiler-owned presentation emitter consumes those bindings, records the known data-final-fixes observation, hides the native infinity badge for a positive effective direct or MIRSET1 cap, keeps the technology visible at the cap, and appends the exact localized setting value to its description. `src/mod/families/modern/prototypes/mir/runtime/maximum_level_control.lua` consumes the same transported registry, retains completed levels and bonuses, filters invalid current and queued research, and scopes ordinary research and force events to their affected force. Visibility is restored independently of enablement; enablement is restored only after an exact force-scoped V3 MIR enabled-to-disabled record still matches the binding transition. Pre-disabled, legacy, blocked, and mismatched state remains untouched. A finite late-finalizer prototype, equal-precedence conflict, missing observation, or unknown finalizer adapter emits a stable blocking diagnostic and cannot activate runtime normalization. Compatibility policy files cannot perform this mutation.
