...
`prototypes/mir/planner/generation_plan.lua` is the side-effect-free boundary between stream planning and emission. The compiler plans all fixed and automatic stream decisions, validates duplicate stream, manifest, technology, adoption, and semantic effect identities across the whole set, and only then applies the finalized plan. Evidence-bearing gates have explicit `not-applicable`, `pending`, `passed`, `failed`, and `superseded` states. `prototypes/mir/planner/compilation_plan.lua` is a pure finalizer over immutable CompilerInput. It plans base continuations, converts accepted operations to schema-2 TechnologyDesign, applies sanitation and graph policy through planner-safe integrity services, and finalizes the schema-3 TechnologyCatalog with every accepted and rejected design plus total authoritative gates. GenerationPlan and CompilationPlan are exact projections of the catalog selection, and CompilerResult binds all identities. The planner imports no emitter. `prototypes/mir/pipeline/compiler_orchestrator.lua` owns sequencing, context state, apply, output assertion, and exact artifact publication.
...
## C8 compiler boundary

C8 separates pure planning from lifecycle orchestration. `pipeline/compiler_orchestrator.lua` owns compile, apply, output assertion, and artifact publication. `planner/compilation_plan.lua` accepts immutable `CompilerInput` material and returns an immutable `CompilerResult`; it cannot import `emit/`. Planner-safe technology-effect checks live under `integrity/`.
