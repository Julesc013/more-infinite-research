...
The technology graph semantics no longer belong separately to planner and emitter code. `prototypes/mir/graph/` is the shared non-mutating kernel for canonical snapshots, iterative strongly connected components, condensation, reachability, researchability, the graph-wide researchability index, qualification, diff, and proof evidence. A completed graph qualification constructs one trusted immutable passing proof for its exact graph identity and shares that record across every accepted planned technology; rejected technologies retain their distinct causal evidence. Researchability consumes the snapshot-owned stable node view, `technology_map()` remains the defensive-copy boundary for callers requiring ownership, and a snapshot constructed by qualification shares the already sorted internal prerequisite arrays. The SCC kernel has an exact acyclic fast path that emits the same singleton component identities; cyclic input falls through to the complete iterative SCC algorithm. Condensation deduplicates edges with nested component membership instead of materializing concatenated edge keys. Fixed graph-snapshot and condensation schemas build their canonical bytes directly and feed them through `fingerprint.of_prebuilt_canonical()`; compiler fixtures prove those bytes remain identical to the general canonical serializer, and the fingerprint module records the same work-volume metrics. `planner/technology_graph.lua` and `emit/technology_graph_safety.lua` are adapters: virtual and realized graphs bind exact graph, component-assignment, condensation, and proof fingerprints before parity is accepted.
