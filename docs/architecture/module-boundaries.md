...
The current package-presentation baseline is the visible, package-excluded `spec/distribution/mir4-package-presentation-baseline-v1.json`. It appends the exact package fingerprint and `README.md` blob transition introduced by PR #203 to the immutable T14 predecessor; it does not amend T14 or any MIR 4.0.0 release record. `tools/lib/mir4/PackagePresentation.ps1` admits the pre-T14, T14, and current baselines while failing closed for any unrecorded package source. The current record confirms that executable source, gameplay, emitter, release, signing, promotion, and publication authority did not change. Root-README/package coupling remains an explicit temporary condition until package-membership characterization and repository/package README separation close it.
