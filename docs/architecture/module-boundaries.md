...
The frozen historical presentation records preserve their exact scopes: baseline V1 covers PR #203/T14, V2 covers M41-05B/F2E, and V3 covers the 447/439 byte-preserving composable-source layout receipt. The current package-presentation authority is the package-excluded `spec/distribution/mir4-current-package-presentation-v4.json`, generated and checked only by `tools/commands/mir4/Update-MIR4CurrentPackagePresentationV4Authority.ps1`. V4 binds immutable V3 and the Factorio-1 convergence authority/receipt to the current source manifest, target authority and all four materialized content identities. F210/F200 changed presentation README bytes only while retaining executable content; F110/F100 changed semantically and remain explicitly blocked on exact-engine proof. V4 keeps private build, qualification, seal, main-promotion, version allocation, tagging, signing, sealing and publication disabled. `tools/lib/mir4/PackagePresentation.ps1` routes current readers through V4; explicit historical readers retain the immutable V1–V3 evidence.
