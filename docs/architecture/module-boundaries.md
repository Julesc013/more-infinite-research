...
`prototypes/mir/planner/generation_plan.lua` is the side-effect-free boundary between stream planning and emission. The compiler plans all fixed and automatic stream decisions, validates duplicate stream, manifest, technology, owner binding, adoption, and semantic effect identities across the whole set, and only then applies the finalized plan. Evidence-bearing gates cover target, effect, owner, science, lab, prerequisite, loop, progression, migration, and output identity. External-family adoption and native-owner configuration are deferred until apply time; planning may inspect owners but may not mutate them. `prototypes/mir/planner/compilation_plan.lua` also plans base continuations before the first generated technology is emitted and makes the existing emitter consume those operations without rediscovery.
