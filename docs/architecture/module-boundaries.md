...
`prototypes/mir/runtime/maximum_level_control.lua` owns the lossless absolute maximum-level contract on script-capable targets. Planning keeps managed generated streams, base continuations, and native owners prototype-infinite so Factorio cannot discard completed levels before configuration-change recovery runs. After output validation, the compiler-owned presentation emitter hides the native infinity badge for a positive effective direct or MIRSET1 cap, keeps the technology visible at the cap, and appends the exact localized setting value to its description. Runtime retains completed levels, filters invalid current and queued research, preserves still-valid progress, and records only the technologies it disabled so cap relaxation does not overwrite unrelated mod state. Its declared `runtime -> streams` dependency reads the canonical stream registry only to map stable setting IDs onto runtime technology names; it does not reopen compiler state or mutate prototypes. A finite late-finalizer prototype conflicts with lossless recovery and is diagnosed rather than treated as active. Compatibility policy files cannot perform this mutation.
