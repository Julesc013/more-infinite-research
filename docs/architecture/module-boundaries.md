...
The W03 semantic compiler is a package-excluded shadow reference aggregate owned by `MIR4-Semantic-Compiler-ProgrammeV1.json`. `tools/lib/mir4/CompilationRun.ps1` may bind current authorities into `CompilationRunV1`, map the legacy data-only provider contract to concern protocols, and test registered merge-law adapters. It cannot evaluate a second semantic planner, import Factorio globals, authorize an executor, mutate prototypes or runtime state, execute migrations, or publish. Subscription and migration laws remain W04-owned; ProcessIR semantics remain W06-owned. The terminal `CompilerContext`, planner, transformation plan, executors, mutation journal, compiler result, and bounded public artifacts remain player-authoritative.
