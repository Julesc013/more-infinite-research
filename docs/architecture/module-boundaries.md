...
MIR 4 V1 developer records cross one package-excluded canonicalization boundary before hashing or transport. `spec/canonicalization/mir-canonical-json-v1.json` owns `mir-canonical-json/1`; `tools/mir/domain/canonicalization/CanonicalJsonV1.ps1` is the sole repository implementation, the old `tools/lib/mir4/CanonicalJsonV1.ps1` path is a read-only compatibility forwarder, and the independent Python implementation replays the same positive and negative corpus. `spec/api/mir4-v1/schema-namespace.json` owns permanent schema IDs, `spec/api/mir4-v1/diagnostics.json` owns stable non-reusable diagnostic codes and ordering, and `spec/api/mir4-v1/compatibility.json` confines V0 to explicit migration readers. `ModuleEcosystem.ps1` consumes these contracts for V1 API and MEP data only. This boundary cannot mutate player prototypes or runtime state, confer public support, or perform a release transition.

The W01 visible-root fixed point is governed by `.mir/control/repository-fixed-point.json`. Logical path resolution remains owned by `.mir/control/paths.yml`, writer selection remains owned by `.mir/control-plane/ownership.json`, and current package source paths remain authoritative. The first bounded migration activates governance, contracts, assurance, tests, `tools/mir`, and releases for the repository fixed-point tooling family. Its domain, port, Git adapter, application writer, CLI, and test live under their final roots; the former library, command, and W01 test paths are thin compatibility forwarders. Its exact receipt is an immutable predecessor. The canonicalization family follows through a separate authority, proof policy, sole successor-receipt writer, and append-only receipt; it evolves current authority hashes without rewriting earlier acceptance. No part of either migration can move player source, mutate the package, authorize deletion, or perform a release transition; any unknown path still fails the cutover boundary.
