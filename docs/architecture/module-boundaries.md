...
The deterministic MIR32 transport identity is unchanged by the C16 performance work. Canonical encoding keeps arrays allocation-free, captures ordinary string-key maps in one traversal, and preserves the existing typed-key ordering and byte representation; sparse or mixed numeric maps retain the exact fallback classification. Its canonical hash implementation reads eight bytes per Lua/C boundary crossing but applies the same recurrence and prime modulus after every byte, so existing source, design, evidence, and artifact fingerprints remain byte-for-byte stable.
