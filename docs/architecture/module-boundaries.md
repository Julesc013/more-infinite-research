...
`prototypes/mir/index/recipe_facts.lua` is the build-once normalized recipe authority. It scans final recipe prototypes once after target-local mutation passes, resolves target-aware effective recipe properties, preserves the full product shape, stores private copied facts, and builds output/category indexes. Read-only consumers share the context-owned `index_view()`, `for_each()`, cached `fingerprint()`, and `summary()` surfaces; complete deep-copied snapshots are reserved for callers that require ownership. `prototypes/mir/index/relationships.lua` owns separate immutable `input` and `output` snapshots so post-emission assertions never reuse a pre-emission technology cache. Recipe-productivity matching, science recipe-unlock facts, compatibility diagnostics, and the diagnostic registry query these authorities rather than rescanning `data.raw.recipe`. The shared owner lookup uses the snapshot-scoped technology effect index rather than scanning every technology once per recipe.

`prototypes/mir/pipeline/compiler_context.lua` is the lifetime owner for mutable state derived from `data.raw`, startup settings, active mods, or the current compilation pass. CompilerContext schema 2 owns recipe and relationship indexes, active compatibility packs, the composed compatibility-policy authority, family resolution, GenerationPlan and CompilationPlan artifacts, generated-technology and native-owner-adoption records, coverage, telemetry, and diagnostics. When generation diagnostics or automatic compiler preview/report mode requests lifecycle detail, it also owns the context-local TechnologyCandidate and TechnologyQualification catalogs; normal player loads do not construct those full internal artifacts. Modules may hold an internal read-only view into the active context during one data-final-fixes run, but may not keep those values in process-wide `canonical` or `latest` caches. Context snapshots are deep copies, and separate contexts cannot observe each other's derived state. The Factorio stage explicitly requests the live context because it discards the return value; tests and review tooling retain snapshot-by-default compatibility. Immutable built-in provider, FamilyRule, and registered operator catalogs may remain module-scoped because they are policy inputs rather than run-derived compiler state.

`prototypes/mir/report/public_compiler_artifacts.lua` owns compact schema-1 projections for normal-load GenerationPlan, coverage, and CompilerEvidence publication. Full stream specifications, `TechnologyDesign` objects, recipe rows, and sanitation ledgers remain internal compiler state and are emitted through separately named `*-internal` prototypes only when generation diagnostics or automatic compiler preview/report mode requests them.

`prototypes/mir/planner/generation_plan.lua` is the side-effect-free boundary between stream planning and emission. The compiler plans all fixed and automatic stream decisions, validates duplicate stream, manifest, technology, adoption, and semantic effect identities across the whole set, and only then applies the finalized plan. Evidence-bearing gates cover target, effect, owner, science, lab, prerequisite, loop, progression, migration, and output identity. When explicitly requested for diagnostics or preview/report mode, the context-owned TechnologyCatalog records the normalized candidate, selected alternative, and exact-context qualification for every finalized row without publishing or promoting it. This is still a projection of the compatibility-preserving stream planner, not yet a pre-selection alternative generator. External-family adoption is deferred until apply time; planning may inspect owners but may not mutate them. Every accepted adoption carries a schema-2 `patch-existing` TechnologyDesign that binds the immutable owner input, complete expected output, target, operation, and configured fields. `prototypes/mir/planner/compilation_plan.lua` also plans base continuations before the first generated technology is emitted, converts accepted continuation operations to schema-2 `TechnologyDesign`, and re-derives their projection after sanitation and cross-operation policy.
