...
Research cost semantics live under `prototypes/mir/domain/research_cost/`. The schema-1 `ResearchCostModel` owns the anchor level, first-level base cost, non-negative additive increment, exponential multiplier of at least one, derived fixed/linear/exponential/hybrid kind, canonical Factorio formula, provenance, and fingerprint. Validation and formula compilation are pure; classification uses a bounded arithmetic parser and never executes third-party Lua. Generated streams, native-owner adoption, and vanilla continuations all consume this model. An unchanged external formula is preserved verbatim; an explicit supported override projects to the canonical formula, while an unknown external formula rejects the override without mutating the technology.
