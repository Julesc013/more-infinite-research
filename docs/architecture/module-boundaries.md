...
`prototypes/mir/planner/generation_plan.lua` is the side-effect-free boundary between stream planning and emission. The compiler plans all fixed and automatic stream decisions, validates duplicate stream, manifest, technology, adoption, and semantic effect identities across the whole set, and only then applies the finalized plan. Evidence-bearing gates cover target, effect, owner, science, lab, prerequisite, loop, progression, migration, and output identity. External-family adoption is deferred until apply time; planning may inspect owners but may not mutate them. `prototypes/mir/planner/compilation_plan.lua` also plans base continuations before the first generated technology is emitted, converts accepted continuation operations to schema-2 `TechnologyDesign`, and re-derives their projection after sanitation and cross-operation policy.
