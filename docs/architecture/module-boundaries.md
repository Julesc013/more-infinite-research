...
W09 historical succession and successor-host proof implementations are package-excluded applications under `tools/mir/application/history`, with record export under `tools/mir/cli/Export-MIR4HistoricalSuccessionRecords.ps1`. Their former `tools/lib/mir4` and `tools/commands/mir4` entrypoints are compatibility forwarders only. This placement grants no historical-product construction, museum admission, custody-rights, player mutation, prototype write, runtime migration, source-freeze, signing, promotion, upload, publication, or public-support authority. `tools/lib/mir4/ReleaseDag.ps1` remains a separate release-tooling family and is not owned by the historical migration.
