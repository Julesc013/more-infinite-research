...
The deterministic MIR32 transport identity is unchanged by the C16 performance work. Canonical map encoding captures and classifies keys in one traversal while preserving the existing typed-key ordering and byte representation. Its canonical hash implementation reads eight bytes per Lua/C boundary crossing but applies the same recurrence and prime modulus after every byte, so existing source, design, evidence, and artifact fingerprints remain byte-for-byte stable.
