...
The W01 visible-root skeleton is a generated, package-excluded shadow projection of `.mir/control/repository-fixed-point.json`. Logical path resolution remains owned by `.mir/control/paths.yml`, writer selection remains owned by `.mir/control-plane/ownership.json`, and current package source paths remain authoritative. `tools/lib/mir4/RepositoryFixedPoint.ps1` inventories and validates the projection but cannot move source, change a writer, mutate the package, or authorize deletion; any unknown path fails the cutover boundary.
