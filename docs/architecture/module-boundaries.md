...
`prototypes/mir/pipeline/compiler_context.lua` is the lifetime owner for mutable state derived from `data.raw`, startup settings, active mods, or the current compilation pass. CompilerContext schema 2 owns recipe and relationship indexes, active compatibility packs, family resolution, GenerationPlan and CompilationPlan artifacts, generated-technology and native-owner-adoption records, coverage, telemetry, and diagnostics. Modules may hold an internal read-only view into the active context during one data-final-fixes run, but may not keep those values in process-wide `canonical` or `latest` caches. Context snapshots are deep copies, and separate contexts cannot observe each other's derived state. Immutable built-in provider and FamilyRule catalogs may remain module-scoped because they are policy inputs rather than run-derived compiler state.

`prototypes/mir/planner/generation_plan.lua` is the side-effect-free boundary between stream planning and emission. The compiler plans all fixed and automatic stream decisions, validates duplicate stream, manifest, technology, adoption, and semantic effect identities across the whole set, and only then applies the finalized plan. Evidence-bearing gates cover target, effect, owner, science, lab, prerequisite, loop, progression, migration, and output identity. External-family adoption is deferred until apply time; planning may inspect owners but may not mutate them. `prototypes/mir/planner/compilation_plan.lua` also plans base continuations before the first generated technology is emitted and makes the existing emitter consume those operations without rediscovery.
