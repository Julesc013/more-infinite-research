...
The MIR 4 whole-platform consolidation is package-excluded and owned by `MIR4-Whole-Platform-ProgrammeV1.json`. `tools/lib/mir4/WholePlatform.ps1` proves that every former 4.0 through 4.17 area has one 4.0.0 destination, executable implementation paths, verification ownership, explicit maturity, and a cutover condition. `tools/mir/domain/targets/TargetKey.ps1` makes uppercase `FNNN` the canonical human-facing target form while accepting lowercase target IDs only as compatibility input and preserving immutable historical evidence; the old library path is a read-only forwarder. `tools/lib/mir4/TechnologyAcceptance.ps1` may project a final schema-3 `TechnologyCatalog` into a non-mutating per-technology review queue; it cannot approve, promote, emit, mutate, transfer compatibility claims, or publish.
