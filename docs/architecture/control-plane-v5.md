...
Each context directory is named by a digest over ten exact members: plan, candidate descriptor, release transition, expanded tasks, expanded scenarios, package-domain manifest, target profile, environment locks, control-plane lock, and `candidate.zip`. A manifest binds every member's byte length and SHA-256; `context-digest.txt` repeats the reconstructed context identity. Existing contexts are validated and reused byte-for-byte, never overwritten. Executable context ABI 2 additionally locks the controller commit, tracked-worktree diff identity, absence of untracked governed files, and every control-plane implementation file. An executor refuses the context if any of those controller inputs drift after materialization. Fresh-calibration proof further requires that the locked controller checkout was clean and committed.
...
Performance campaign authority is also intentionally controller scoped because C24's package-source commit predates the final C24 campaign row. The executor verifies that `.mir/performance-campaign.json` binds the context's exact candidate, C21 baseline, target, and Factorio version, clones the immutable source into a context-addressed staging repository, and overlays only that package-excluded authority file. It then proves the staged package-source fingerprint is unchanged before running the source-side measurement and validation scripts. The emitted schema-3 artifact must independently bind C21, C24, the source commit, the Factorio binary, every passing paired lane, and the third-party closure before v5 writes its observation, pure status evaluation, and TaskNode result.
