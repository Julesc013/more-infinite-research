...
Every native Factorio worker resolves the selected executable before launch and compares its binary digest, byte length, reported version, and official-data tree digest with the immutable environment locks in the verification context. No runtime, upgrade, ecosystem, approved-delta, or performance observation may be emitted from an installation that fails that exact comparison.
