...
`tools/commands/control/Invoke-MIRControlPlane.ps1 plan` supports `changed`, `qualify-incremental`, `calibrate-fresh`, and `rerun-failure` modes. Every selected row includes its semantic impact reason, freshness class, resource class, prerequisites, and effective-input digest. Unknown paths select the complete graph and fail governance until ownership is added.
...
Exact historical source checkouts may predate the canonical `validation/scenarios/runtime.json` layout. Registry construction and validation accept only its governed exact predecessor, `fixtures/compat-matrix/expected-scenarios.json`, when the canonical file is absent; they never search or guess. The catalog content hash still binds the resulting registry.
...
The historical adapter converts each of the 130 schema-4 v4 evidence rows for C24 into a canonical `legacy-v4-adapter` observation. A versioned `status-equals` assertion then runs through the same pure evaluator used for native v5 captures. `validation/baselines/control/3.2.2-v5-replay.json` records independent capture, observation, evaluation, and evidence digests for every row; it is deterministic and contains no wall-clock identity material.
...
Local evidence copies are stored by SHA-256 under `build/results/evidence/objects/sha256/`. Indexes and leases are rebuildable coordination data; durable authorities are promoted to tracked or external release custody. Revocation may target a producer ABI, evaluator ABI, canonicalization ABI, digest set, or time range without discarding unrelated observations.
