...
This proves structural shadow equivalence only. The operational shadow gate remains closed until every candidate-specific outcome is admitted through immutable v5 evidence. For 3.2.2, the historical v4 outcomes are replayable but the calibration-only release exception is not a schema-4 seal and cannot satisfy the seal-input dimension. For P9, approved delta, upgrade, performance, manual acceptance, aggregate verdict, and seal inputs remain pending. Observed files outside the v5 evidence store are diagnostic input, not admitted proof. `.mir/control-plane/shadow-analysis.json` records these distinctions and `scripts/Test-MIRControlPlaneShadow.ps1` fails closed unless all dimensions are accepted.
