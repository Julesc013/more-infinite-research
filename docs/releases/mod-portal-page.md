...
| Landfill productivity | `+10%` for landfill; `+5%` for foundation; `+2%` for ice platform; `+1%` for space platform foundation | On when recipes exist | Grants exact productivity permission to the two Space Age platform recipes. Uses landfill technology art and adds metallurgic and space science when available. |
