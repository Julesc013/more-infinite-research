...
- Final package hygiene check found 119 entries and zero forbidden
...
- The non-zero power floor is separate and default off. When enabled, it changes
  explicit `0W` active-use prototypes to `1W` to avoid unwanted low-power
  warning icons in affected packs.
