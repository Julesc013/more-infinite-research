...
| Factorio 2.0 | 2.4.5 | `tmp/2.0` | Direct projection of `9c8f400` / canonical package source `79df29b` | Pending version-renumbered candidate | Retain 94/94 behavior proof; rerun version-sensitive Factorio 2.0.77 checks | Qualified 2.4.1 candidate superseded before publication | Rebuild, exact-ZIP/upgrade validation, seal, then maintainer review/tag/release |
...
Push the sealed 2.4.5 archive on `tmp/2.0` and stop before tagging or publication.
