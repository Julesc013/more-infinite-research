...
The `.10` implementation kept managed technology prototypes infinite and enforced positive settings only at runtime. Research stopped at the selected number, but Factorio still displayed the native infinity badge. A finite-prototype experiment on exact Factorio 2.1.14 proved unsafe: a save above a newly lowered prototype maximum exited before MIR's configuration-change handler could recover it. The `.11` correction therefore retains the lossless internal infinite prototype, hides the infinity badge for a positive cap, keeps capped technologies visible, and appends the exact configured maximum to the localized technology description. Runtime remains responsible for queue normalization and cap relaxation. The same policy covers generated streams, generated base continuations, native Space Age owners, direct startup values, and MIRSET1 imports. If another finalizer replaces the required internal infinite maximum, MIR refuses to claim the runtime cap is active and logs the selected value, planned cap, observed cap, binding operation, setting, and conflict reason.
