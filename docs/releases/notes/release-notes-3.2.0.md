...
MIR 3.2.0 uses exact C20 package-source commit `303de261629149af5f50bd210368e61423f1a299`. The release archive is `dist/more-infinite-research_3.2.0.zip`: 1,029,464 bytes, 290 entries, and SHA-256 `35372EE6D16DA6765E8C30AEAAF5DA4A5D300F02C0A0A03648C80893A5394F32`. Its normalized package-content and package-source SHA-256 is `26CBE7A12FA30C3352343B77A0062FE07426284992EFCBA8DCB82C221CE2DD18`. Publish this recorded ZIP without rebuilding it, then verify every downloaded public copy against the archive hash.

The maintainer selected C20 for `main` staging after fresh release-authority, documentation, focused runtime, static, composition, and deterministic-package validation. C20 replaces C19 because long validation exposed a real post-compile prerequisite-rewire graph mismatch; the now-real multi-replacement fixture also exposed a shadowed competing-productivity preparation table. Longer exact-C20 validation is run and recorded separately before tagging; superseded evidence is not relabelled as exact C20 evidence.
...
Competing-technology removal now records the removed node and every dependent prerequisite rewrite in an exact fingerprinted journal. The unchanged graph retains the fast authority-reuse path; a rewritten graph must equal the journal projection and pass a fresh complete graph qualification. MIR also fixes recognized competing productivity owners that were previously blocked by a shadowed eligibility variable.
