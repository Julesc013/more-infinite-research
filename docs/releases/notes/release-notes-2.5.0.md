...
P11 is the locally automated-qualified and manually accepted release candidate. All 126 machine-verifiable tasks pass against the exact archive, including exact runtime, upgrade, ecosystem, approved-delta, AdvancedBeltsSA, and six-lane paired performance evidence. The schema-2 maintainer attestation binds the same exact archive, content, package source, and Factorio 2.0.77 binary. The maintainer authorized publication at a fixed cutoff while protected run 30716314749 remained active. That run's scenario process passed, but its artifact upload failed during the network outage; protected retry 30740083663 remains post-publication follow-up. P9 and P10 results remain superseded historical evidence and are not current P11 proof.
