...
- Use `docs/archive/2.x/post-2.0-feature-plan.md`, `docs/archive/2.x/legacy-backport-cadence.md`, and `docs/maintainer/backporting.md` as supporting notes. Durable future tasks from those files should also be mirrored into root `todo.md`.
...
expanded legacy releases = older Factorio line backports recorded in docs/archive/2.x/legacy-backport-cadence.md
...
`1.9.3+` becomes the Factorio `1.1` line, `1.8.0` is the one-time Factorio
`0.18` bridge/archive package, `1.8.1+` becomes the Factorio `1.0` line,
`1.7.x` through `1.3.x` map to Factorio `0.17` through `0.13`, and
...
Current `dev` is the `v3.0.0` Factorio `2.1` hardening line. The shipped
architecture now uses required Factorio root lifecycle wrappers, active MIR
implementation under `prototypes/mir/`, and declarative stream data tables under
`prototypes/streams/`. The old shipped compatibility, library, legacy, planner,
control-folder, and broad root-helper shim paths are absent from the 3.x layout.

The remaining 3.0 work is release hardening: preserve zero-inventory evidence,
run runtime and targeted release gates, compare planner/report rows with the
recorded regression baseline, update player-facing release metadata, and build
only the unpublished `3.0.0` archive.
...
The detailed release gate lives in `docs/archive/2.x/release-plan-2.1.0.md`, but the durable task list lives in root `todo.md`.
...
`docs/archive/2.x/2.2.0-feature-intake.md`. Its useful input is the feature-family risk
...
The expanded older-line ladder is recorded in `docs/archive/2.x/legacy-backport-cadence.md`.
...
3. `v1.9.2` has been released from `legacy` as the Factorio `2.0` transition
   port of the tested `2.2.0` source point.
4. Finish `v3.0.0` on `dev` as the compatibility-compiler architecture release
   by running final validation, preserving regression evidence, updating
   release metadata, and building only the unpublished `3.0.0` archive.
5. Use `docs/architecture/compatibility-compiler-charter.md`,
   `docs/architecture/module-boundaries.md`, and the focused capability,
   policy, decision, manifest, claim, testing, maintainer, and ADR docs as the
   scope boundary. The 3.0 line hardens the Factorio shell plus
   `prototypes/mir/` compiler namespace, not new broad gameplay.
6. Use the locked post-transition target-line scheme: `3.x.x` for Factorio `2.1`, `2.x.x` for Factorio `2.0`, `1.9.3+` for Factorio `1.1`, `1.8.0` as the one-time Factorio `0.18` bridge/archive package, `1.8.1+` for Factorio `1.0`, `1.7.x` through `1.3.x` for Factorio `0.17` through `0.13`, and `0.12.x` through `0.6.x` for Factorio `0.12` through `0.6`.
