...
- Use `docs/archive/superseded/post-2.0-feature-plan.md`, `docs/archive/superseded/legacy-backport-cadence.md`, and `docs/maintainer/backporting.md` as supporting notes. Durable future tasks from those files should also be mirrored into root `todo.md`.
...
post-transition releases = target-line version ranges recorded in docs/maintainer/backporting.md
expanded legacy releases = older Factorio line backports recorded in docs/archive/superseded/legacy-backport-cadence.md
...
| `1.8.0` | `0.18.x` bridge | Bridge archive | one-time Factorio `0.18` bridge package, validated in both `0.18` and `1.0`, then frozen unless a severe load/package defect appears |
| `1.8.1+` | `1.0.x` | Compatibility port | maintained Factorio `1.0` line under the locked mapping |
...
After `1.9.2`, the versioning model is locked: `3.x.x` becomes the Factorio `2.1` line, `2.x.x` becomes the Factorio `2.0` line starting at `2.3.0`, `1.9.3+` becomes the Factorio `1.1` line, `1.8.0` is the one-time Factorio `0.18` bridge/archive package, `1.8.1+` becomes the Factorio `1.0` line, `1.7.x` through `1.3.x` map to Factorio `0.17` through `0.13`, and `0.12.x` through `0.6.x` map directly to Factorio `0.12` through `0.6`. The published `1.9.0` through `1.9.2` archives remain historical transition exceptions for Factorio `2.0`.

The `1.9.7`, `1.9.8`, and `1.9.9` cutoffs are superseded by the post-`1.9.2` target-line reset unless the maintainer explicitly revives them. If revived, they should still be treated as release triggers, not API assumptions.

## Current Baseline

Current `dev` is the `v3.0.0` Factorio `2.1` hardening line. The shipped architecture now uses required Factorio root lifecycle wrappers, active MIR implementation under `prototypes/mir/`, and declarative stream data tables under `prototypes/streams/`. The old shipped compatibility, library, legacy, planner, control-folder, and broad root-helper shim paths are absent from the 3.x layout.

The remaining 3.0 work is release hardening: preserve zero-inventory evidence, run runtime and targeted release gates, compare planner/report rows with the recorded regression baseline, update player-facing release metadata, and build only the unpublished `3.0.0` archive.

The `2.2.0` baseline included:

- the compatibility planner, typed fact registry, capability diagnostics, manifest/claim linting, and fixture-backed proof slices for Air Scrubbing, ATAN Ash, ATAN Nuclear Science, AAI Loaders, and Big Mining Drill;
...
The historical detailed release gate lives in `docs/archive/superseded/release-plan-2.1.0.md`, but the durable task list lives in root `todo.md`.
...
`v2.1.5` may pull in low-risk diagnostics-only planner work because it does not change generation behavior. `v2.2.0` is not the release that absorbs every useful idea-mod behavior. Most ideamods are compatibility signals, not planned MIR features. The release's job is to turn a small number of remaining audit signals into designed MIR-owned behavior, with fixtures proving recipe IDs, ownership, value matching, lab compatibility, and non-replacement of balance-distinct chains.

The July 2026 suggestions transcript is tracked in `docs/archive/superseded/2.2.0-feature-intake.md`. Its useful input is the feature-family risk split, the startup-vs-runtime setting boundary, the loop-risk requirement, and the reminder that disabled features must do no work. MIR does not adopt the transcript's separate product names or "Extended" branding. There is one mod, and any future opt-in setting should use plain, Factorio-style wording.
...
Use `docs/compatibility/support-lanes.md` as the decision framework for roles such as exact replacement, MIR-owned stream candidate, compatibility adapter, diagnostic-only, companion scope, docs-only, or rejected from core.
...
The compatibility matrix in `docs/compatibility/compatibility-matrix.md` is the claim ledger for these decisions. A row may be planned or future, but public compatibility claims require an explicit tested profile, validation artifact, role enum, save-compatibility notes, and notes about what MIR refuses to own.

Future overhaul work should be staged as campaigns after the narrow `v2.2.0` proof ladder, not folded into a broad stream batch. The proof ladder is Air Scrubbing, the ATAN follow-ups, Big Mining Drill, Fluid Must Flow, Robot Attrition, Jetpack, Equipment Gantry, AAI Containers, AAI Loaders, and AAI Industry. The procedural compatibility kernel in `docs/architecture/procedural-compatibility-kernel.md` is the control surface for that ladder: loaders, drills, native modifiers, machine manufacturing, ore processing, science/lab compatibility, and rule surfaces should be capability lanes before they become new streams. Outdated upstream Factorio-version metadata does not block fixture-backed support or planned legacy backports; it only changes the public claim lane until a real external load profile passes on the target Factorio line. Krastorio 2 is the first large overhaul campaign target after that ladder, before Bob's, Angel's, Space Exploration, or Pyanodons suite work. Keep standalone K2, K2 plus Space Exploration, K2 Spaced Out / Space Age, AAI suite combinations, Bob's focused subsets, Space Exploration, and Pyanodons as separate matrices because their Factorio lines, dependency shapes, progression rules, and productivity restrictions differ.
...
The expanded older-line ladder is recorded as historical context in `docs/archive/superseded/legacy-backport-cadence.md`. Those Factorio `1.1` through `0.6` releases are separate target-line ports and must not be treated as full-parity releases until each target line is validated.

The daily backport celebration window should draw from that table and the expanded ladder. If a daily slot cannot be validated honestly, skip or reorder the slot rather than publishing an archive with unclear support claims.
...
The detailed proof ledger and unknowns are maintained in `docs/reference/factorio-api-proof-points.md`. Named manual save scenarios are maintained in `docs/maintainer/manual-test-plan.md`.
...
3. `v1.9.2` has been released from `legacy` as the Factorio `2.0` transition port of the tested `2.2.0` source point.
4. Finish `v3.0.0` on `dev` as the compatibility-compiler architecture release by running final validation, preserving regression evidence, updating release metadata, and building only the unpublished `3.0.0` archive.
5. Use `docs/architecture/compatibility-compiler-charter.md`, `docs/architecture/module-boundaries.md`, and the focused capability, policy, decision, manifest, claim, testing, maintainer, and ADR docs as the scope boundary. The 3.0 line hardens the Factorio shell plus `prototypes/mir/` compiler namespace, not new broad gameplay.
