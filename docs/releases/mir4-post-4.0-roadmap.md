...
| M42-00 | Establish one package source, target overlays, one materializer, and four independently generated targets inside the 4.1 completion boundary | Active: F1/F2A/F2B/F2C complete; all four F2D target lanes and the independent aggregate are complete; F2E package-authority cutover is dependency-ready |
...
The accepted F2D chain is complete for F210, F200, F110, and F100, including the independent four-target aggregate. The aggregate revalidated every receipt and predecessor link, replay commit and tree, engine and package identity, fresh load, upgrade and two reloads, independent verifier, resource receipt, custody manifest, and redaction boundary. This remains a package-excluded result: the old writer is still authoritative and only F2E package-authority cutover is now dependency-ready.
