...
| M42-00 | Establish one package source, target overlays, one materializer, and four independently generated targets inside the 4.1 completion boundary | Active: F1/F2A/F2B/F2C complete; F2D F210, F200, and F110 complete; F100 is next; the F2D aggregate is pending and F2E remains blocked |
...
The accepted F2D chain is now F210 complete, F200 complete, F110 complete, and F100 pending. F110 passed against the exact governed Factorio 1.1.110.62357 executable and the 1.9.9 predecessor, including the selected exact-package load, first upgraded-save reload, and second reload. This is a package-excluded replay result: the old writer remains authoritative, the four-target aggregate remains pending, and package cutover is still blocked.
