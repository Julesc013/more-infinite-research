...
| M42-00 | Establish one package source, target overlays, one materializer, and four independently generated targets inside the 4.1 completion boundary | Active: F1/F2A/F2B/F2C complete; all four F2D target lanes are complete; the F2D aggregate is next and F2E remains blocked |
...
The accepted F2D target chain is now complete for F210, F200, F110, and F100. F100 passed against the exact governed Factorio 1.0.0.54889 executable and the 1.8.9 predecessor, including the selected exact-package load, first upgraded-save reload, and second reload. These remain package-excluded target results: the old writer remains authoritative, the four-target aggregate is the next gate, and package cutover is still blocked.
