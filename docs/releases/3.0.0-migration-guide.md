...
This guide records the save-compatibility boundary for the MIR 3 architecture
release. Its purpose is to keep the 3.0 architecture compatible with existing
saves and existing player expectations.
...
3.0 preserves existing generated technology IDs unless a tested migration
exists. The architecture changes internally, but a player should not lose
research progress because a stream moved behind `StreamSpec`, the Factorio
platform adapter, or the MIR emitter boundary.
...
- `prototypes/streams/` remains the declarative stream-data source for this
  release, while emission and active implementation stay under `prototypes/mir/`;
