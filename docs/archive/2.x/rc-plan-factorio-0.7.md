---
title: "Factorio 0.7 RC Planning Report"
status: archived
applies_to: "0.7"
audience: maintainer
doc_type: archive
...
---
# Factorio 0.7 RC Planning Report

Updated: 2026-07-06
Branch: `tmp/0.7`
Target Factorio line: `0.7.x`
Prototype snapshot reviewed: `wube/factorio-data` tag `0.7.5`
Planned MIR release slot(s): `1.7.1`
Risk rating: Extreme
Change type in this commit: documentation and planning only; no code behavior changes.

This is a tentative maintainer-authorized planning note for the temporary experimental branch. It does not make this branch a release candidate by itself. A stable RC for Factorio `0.7` still requires target-line code edits, metadata edits, package construction, and a load test against a matching Factorio binary. If those gates fail, the plan must change rather than shipping a misleading archive.
...
API status: No official versioned API page was listed for 0.7; FFF #3 explicitly mentions Lua API refactoring around 0.7.1.

Science-pack surface observed for the target: science-pack-1, science-pack-2, science-pack-3, alien-science-pack.

- factorio-data 0.7.5 did not show max_level/count_formula.
- Observed effects include inserter stack size, turret attack, gun speed, ammo damage, and toolbelt.
- FFF #3 reports Lua API refactoring around 0.7.1, making modern assumptions unsafe.
...
- Treat nearly all current architecture as non-portable until proven.
- Use finite static technologies only for first proof.
- Audit package structure, locale, and technology prototype schema against 0.7.5 binary.
- Keep branch docs-first until a tiny proof package loads.

## Minimum RC Plan

- Use this as a celebratory historical port, not stable parity line.
- First RC should contain the smallest useful set of old native effects.
- Every feature beyond combat/inserter/toolbelt needs target-data evidence.

## Stable RC Readiness

Status: not ready.

This branch is suitable as an experimental planning branch only. The present source tree should not be packaged as a Factorio `0.7` RC by changing metadata alone. The RC threshold is a target-specific compatibility patch with unsupported technology modifiers removed, target science packs resolved, locale/package structure verified, and the package loaded by a matching Factorio `0.7.x` executable.

## Implementation Non-Goals For The First RC

- Do not promise full feature parity with the current Factorio `2.1` line.
- Do not ship any technology effect whose exact modifier name is not proven for Factorio `0.7`.
...
- [ ] Load the package in a matching Factorio 0.7.x binary; if unavailable, release notes must say validation was not run.
...
- [ ] Select the exact MIR 2.x.x source snapshot for planned releases 1.7.1.
- [ ] Create a target-line API/effect whitelist from official docs where available and factorio-data 0.7.5.
- [ ] Patch info.json factorio_version, version, dependencies, and optional-mod ordering for Factorio 0.7.
...
- [ ] Run a matching Factorio 0.7.x binary load test before RC approval.
- [ ] Find or reconstruct the 0.7 technology prototype schema.
- [ ] Create a one-technology 0.7.5 smoke package.