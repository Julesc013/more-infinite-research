...
Use `scripts/Convert-MIRCompatAuditResults.ps1` after load-test runs to group failures into actionable buckets and emit `compat-failures.grouped.json`, `compat-summary.md`, `profile-candidates.json`, and `missing-dependencies.*`. The grouped output records total, expected, and unexpected failure counts. Expected failures come only from reviewed rules in `fixtures/compat-matrix/expected-failures.json`.
