...
local function rejected_candidate(key, reason, failed_gate)
  return nil, {
    schema = 1,
...
local function accepted_gate_vector(key, technology_name)
...
local function plan_chain(key)
  ...
  if not is_enabled(key, spec) then
    ...
    return rejected_candidate(key, "disabled")
  end
  ...
  if has_infinite or #levels == 0 then
    ...
    return rejected_candidate(key, has_infinite and "already_infinite" or "no_vanilla_chain", "target_supported")
  end
  table.sort(levels)
  ...
  if detected_highest < min_level - 1 then
    ...
    return rejected_candidate(key, "vanilla_chain_below_minimum", "target_supported")
  end
  ...
  if not base_tech or not base_tech.unit then
    ...
    return rejected_candidate(key, "missing_base_unit", "progression_safe")
  end
  if base_tech.max_level == "infinite" then
    D.extension(D.extension_fields(key, "skipped", "base_already_infinite"))
    return rejected_candidate(key, "base_already_infinite", "output_identity_safe")
  end
  ...
  if base_researchability_reason then
    ...
    return rejected_candidate(key, "unresearchable_base_technology_" .. base_researchability_reason, "science_compatible")
  end
  ...
  if data_raw.technology(new_name) then
    ...
    return rejected_candidate(key, "target_exists", "output_identity_safe")
  end
  ...
  if not target_line.feature_enabled("scripted_techs")
  ...
  if not prefer_this_mod_for_competing_techs() then
    ...
    if other_choice then
      ...
      return rejected_candidate(key, "competing_infinite_kept", "owner_conflict_free")
    end
  end
  ...
  if existing then
    ...
    return rejected_candidate(key, "equivalent_infinite_exists", "owner_conflict_free")
  end
  ...
  if not resolved_ingredients or #resolved_ingredients == 0 then
    ...
    return rejected_candidate(key, "no_lab_compatible_science", "lab_compatible")
  end
  new.unit = {
  ...
  if prerequisite_reason then
    ...
    return rejected_candidate(key, prerequisite_reason, "prerequisites_acyclic")
  end
