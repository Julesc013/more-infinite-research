...
local function extend_chain(key)
  ...
  local linear_increment = 0
  local increment_provenance = "target-finite-substitute"
  if target_line.supports_native_infinite_technology() then
    linear_increment = sanitize_number(startup_setting(cost_contract.base_name("linear_increment", key)))
    increment_provenance = cost_contract.base_name("linear_increment", key)
  ...
  if target_line.supports_native_infinite_technology() then
    new.unit.count_formula = cost_model.count_formula
  else
    new.unit.count = math.max(1, math.floor(research_cost_model.evaluate(cost_model, desired_new_level) + 0.5))
  end
