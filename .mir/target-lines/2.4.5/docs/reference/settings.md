...
Recognized cost shapes are `growth^L*base`, `base*growth^(L-1)`, and fixed count for base-only changes. An unknown formula remains valid for default preservation but rejects explicit cost changes. Plans carry immutable input and expected-output fingerprints, whole-plan validation rejects duplicate owner bindings, and emission verifies the input before applying one transaction. Emitted input and output unit models let configuration-change runtime preserve the current owner research, level, and fractional progress; the Factorio 2.0 adapter also reconstructs 2.4.0's former independently applied cost model during upgrade. See [native owner binding](../architecture/native-owner-binding.md) and `.mir/native-owner-cost-models.json`.
