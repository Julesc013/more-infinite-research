local from_version = "1.8.1"
local to_version = "1.8.2"
local expected_mode = "off"
local technology_name = "recipe-prod-research_lab_productivity-1"
local expected_progress = 0.42
local epsilon = 0.000001

local function fail(message)
  error("MIR 1.8.1 to 1.8.2 off-retention validation failed: " .. message)
end
...
script.on_init(function()
  ...
  global.mir.target_upgrade_fixture = {
    ...
    runtime_marker = "factorio-1.0-global-state"
  }
  log("[mir-fixture] 1.8.1 upgrade source proof complete")
end)
...
script.on_configuration_changed(function()
  ...
  if not state or state.runtime_marker ~= "factorio-1.0-global-state" then
  ...
  log("[mir-fixture] 1.8.1 to 1.8.2 upgrade proof complete")
end)
