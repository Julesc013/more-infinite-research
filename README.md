...
| `prototypes/mir/settings/pipeline_extent.lua` | Owns pipeline extent dropdown values and parsing. The old `prototypes/pipeline-extent-settings.lua` file is a compatibility shim. |
| `prototypes/mir/pipeline/extent.lua` | Applies the opt-in startup-only pipeline extent multiplier to fluid boxes. The old `prototypes/pipeline-extent.lua` file is a compatibility shim. |
| `prototypes/mir/streams/registry.lua` | Assembles shared stream config and compatibility profile overlays. The old `prototypes/config.lua` file is a compatibility shim. |
| `prototypes/mir/planner/stream_compiler.lua` | Owns the transitional generated stream loop. The old `prototypes/tech-gen.lua` file is a compatibility shim. |
| `prototypes/mir/emit/base_extensions.lua` | Extends finite vanilla technology chains. The old `prototypes/base-tech-extensions.lua` file is a compatibility shim. |
| `prototypes/mir/emit/effect_safety.lua` | Blocks unsafe native effect types from MIR-generated technologies. The old `prototypes/technology-effect-safety.lua` file is a compatibility shim. |
| `prototypes/mir/policy/weapon_speed.lua` | Optionally removes rocket/cannon-shell overlap from MIR's generated weapon speed continuation while preserving finite vanilla bonuses. The old `prototypes/weapon-speed-adjustments.lua` file is a compatibility shim. |
| `prototypes/mir/policy/max_level.lua` | Applies stream max levels after generation. The old `prototypes/max-level-control.lua` file is a compatibility shim. |
| `prototypes/mir/report/diagnostics_sink.lua` | Structured generation report logging. The old `prototypes/diagnostics.lua` file is a compatibility shim. |
...
| `prototypes/mir/platform/factorio/prototype_lookup.lua` | Shared prototype lookup helpers for items, fluids, technologies, ammo categories, mods, and labs. Old `prototypes/lib/*` paths are compatibility shims. |
...
| `prototypes/mir/compatibility/profiles.lua` | Mod-specific stream patch scaffolding. Old `prototypes/compat/*` paths are compatibility shims. |
