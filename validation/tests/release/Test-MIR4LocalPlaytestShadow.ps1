...
$authorityRelative = '.mir/releases/waves/mir4-r0/MIR4-Local-Playtest-Shadow-AuthorizationV1.json'
...
foreach ($target in @($authority.authorized_targets)) {
  ...
  Assert-True ($planRows.Count -eq 1 -and $readinessRows.Count -eq 1) "Local-playtest target is not uniquely imported: $($target.target_key)"
  $planRow = $planRows[0]
  ...
  Assert-True (-not [bool]$readinessRows[0].local_construction_admitted) "Release readiness admitted $($target.target_key); the local lane must remain orthogonal."
}
Assert-True ((@($readiness.entry_state.blocked_targets) -join '|') -ceq 'f200|f110|f100') 'Release readiness no longer blocks all private-lane targets.'

$packageFiles = @(Get-MIRPackageSourceFiles -RepoRoot $RepoRoot)
foreach ($relative in @($authorityRelative, 'spec/schemas/mir4-local-playtest-shadow-authorization.schema.json', 'spec/schemas/mir4-local-playtest-candidate-manifest.schema.json')) {
