...
try {
  ...
  $planPath = (Resolve-Path (Join-Path $RepoRoot ".mir/releases/waves/mir4-r0/MIR4-Bootstrap-Local-Candidate-PlanV1.json")).Path
  $plan = Assert-MIR4GovernedBootstrapRecordFileV1 -Path $planPath -SchemaPath (Join-Path $schemas "mir4-bootstrap-local-candidate-plan.schema.json")
  ...
  $tamperedEvidencePayloadPath=Join-Path $testRoot "synthetic-evidence-payload-tampered.json";$tamperedEvidencePayload=Get-Content -Raw $evidencePayloadPath|ConvertFrom-Json -Depth 100;$tamperedEvidencePayload.candidate.candidate_id="tampered-synthetic-probe";$null=Write-MIR4CustodyRecordV1 -Record $tamperedEvidencePayload -Path $tamperedEvidencePayloadPath
  ...
  $differentDry=Join-Path $testRoot "synthetic-dry-different.json";$different=Get-Content -Raw $dryA|ConvertFrom-Json -Depth 100;$different.release.title="Divergent synthetic pair probe";$null=Write-MIR4CustodyRecordV1 -Record $different -Path $differentDry
