...
$authority=Get-Content -Raw -LiteralPath (Join-Path $repo "$root/MIR4-M4C01-Implementation-AuthorizationV1.json")|ConvertFrom-Json
...
$registry=Get-Content -Raw -LiteralPath (Join-Path $repo "$root/MIR4-Target-RegistryV5.json")|ConvertFrom-Json
...
$presentation=$presentationText|ConvertFrom-Json -Depth 100
