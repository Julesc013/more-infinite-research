...
foreach ($row in @($profiles.targets)) {
  ...
  if ($matrixRow.Count -ne 1 -or $admissionRow.Count -ne 1 -or
      ...
      [string]$record.state -notin @(
        "package-built", "focused-qualified", "candidate-qualified", "automated-qualified-awaiting-human-review",
        "manually-accepted", "protected-qualified", "sealed", "promoted", "tagged", "published", "publicly-verified"
      ) -or [string]$record.candidate_id -ne [string]$record.candidate_allocation.assigned_id) {
