...
$commands = [ordered]@{
  "scripts/Get-MIRLegacyInventory.ps1" = "tools/commands/workspace/Get-MIRLegacyInventory.ps1"
  "scripts/Remove-MIRStaleArtifacts.ps1" = "tools/commands/workspace/Remove-MIRStaleArtifacts.ps1"
}
...
foreach ($legacyRelative in $commands.Keys) {
  ...
  if (-not $legacyText.Contains($marker)) {
    if (Test-Path -LiteralPath $canonicalPath -PathType Leaf) {
      throw "Refusing duplicate workspace-command implementations: $legacyRelative and $canonicalRelative"
    }
    $moveCount++
    ...
    if ($Apply) {
      ...
      if ($canonicalText -ceq $legacyText) {
        throw "Workspace command root bootstrap was not rewritten: $legacyRelative"
      }
      [IO.File]::WriteAllText($canonicalPath, $canonicalText, $utf8NoBom)

      $bodyIndex = $legacyText.IndexOf('$ErrorActionPreference')
      if ($bodyIndex -lt 1) { throw "Workspace command has no stable parameter/body boundary: $legacyRelative" }
...
"@
    ...
    throw "Legacy workspace-command wrapper has no canonical implementation: $legacyRelative"
  }
}

[ordered]@{
  schema = 1
  migration = "mir-workspace-commands-v1"
