...
function New-MIR4A08GitHubRestPolicyProvider {
  ...
  $provider = {
    ...
    if ($sourceIds.Count -eq 0) { throw '[mir4-a08-policy-ruleset-source]' }
    $rulesets = @(
...
function Read-MIR4A08QualificationRecord {
  ...
  $recordFullPath=(Resolve-Path -LiteralPath $RecordPath).Path;try{$record=Get-Content -Raw -LiteralPath $recordFullPath|ConvertFrom-Json -Depth 100 -DateKind String}catch{throw '[mir4-a08-qualification-record]'};Assert-MIR4A08PropertyNames $record @('schema','kind','status','candidate','materializer','authority','expected_targets','targets','release_authority','publication_authority','record_sha256') '[mir4-a08-qualification-record]';Assert-MIR4A08PropertyNames $record.candidate @('ref','commit','tree') '[mir4-a08-qualification-record]';Assert-MIR4A08PropertyNames $record.materializer @('path','git_blob') '[mir4-a08-qualification-record]'
  if ([int]$record.schema-ne1-or[string]$record.kind-cne'MIR42ProtectedMainQualificationBindingV2'-or[string]$record.status-cne'independently-qualified'-or[string]$record.record_sha256-cne(Get-MIR4A08SelfSha256 $record 'record_sha256')-or[string]$record.candidate.ref-cne[string]$Candidate.ref-or[string]$record.candidate.commit-cne[string]$Candidate.commit-or[string]$record.candidate.tree-cne[string]$Candidate.tree-or[string]$record.materializer.path-cne'tools/mir/application/package/TargetMaterializer.ps1'-or[string]$record.materializer.git_blob-cne(Resolve-MIR4A08Blob -RepoRoot $RepoRoot -Commit ([string]$Candidate.commit) -Path ([string]$record.materializer.path))-or(@($record.expected_targets|Sort-Object)-join'|')-cne'F200|F210'-or[bool]$record.release_authority-or[bool]$record.publication_authority){throw '[mir4-a08-qualification-record]'};$authority=Assert-MIR4A08TrustedIssuer -Authority $record.authority -Candidate $Candidate -CanonicalRepository ([string]$Plan.policy.repository) -QualificationAuthorityProvider $QualificationAuthorityProvider -Rehearsal:$Rehearsal;$observed=@();if (@($record.targets).Count-ne2){throw '[mir4-a08-qualification-record]'};foreach ($row in @($record.targets)){Assert-MIR4A08PropertyNames $row @('target','distribution_version','relative_path','archive_sha256','content_sha256','bytes','entry_count','root','factorio_version','worker_receipt','independent_receipt') '[mir4-a08-qualification-record]';if ([string]$row.target-notin@($Plan.qualification.expected_targets)-or[string]$row.distribution_version-eq''){throw '[mir4-a08-qualification-record]'};$path=Resolve-MIR4A08ContainedFile -Root $PackageRoot -RelativePath ([string]$row.relative_path) -Code '[mir4-a08-qualification-record]';$actual=Get-MIR4A08PackageObservation -PackagePath $path -Target ([string]$row.target) -RelativePath ([string]$row.relative_path);foreach ($field in @('archive_sha256','content_sha256','bytes','entry_count','root','factorio_version')){if ([string]$actual.$field-cne[string]$row.$field){throw '[mir4-a08-qualified-package-mismatch]'}};Assert-MIR4A08QualificationReceipts -Row $row -Candidate $Candidate -Authority $authority -Observed $actual;$observed+=$actual};$setSha=Get-MIR4A08PackageSetSha256 $observed;return [pscustomobject][ordered]@{record_path=$recordFullPath;record_sha256=(Get-MIR4A08FileSha256 $recordFullPath);record_binding_sha256=[string]$record.record_sha256;package_root=([IO.Path]::GetFullPath((Resolve-Path -LiteralPath $PackageRoot).Path));candidate=[pscustomobject][ordered]@{ref=[string]$Candidate.ref;commit=[string]$Candidate.commit;tree=[string]$Candidate.tree};materializer=$record.materializer;authority=$authority;package_set_sha256=$setSha;packages=@($observed|Sort-Object target)}
}
...
function New-MIR4A08PromotionIntention {
  ...
  $repo=(Resolve-Path -LiteralPath $RepoRoot).Path;$null=Assert-MIR4A08Plan $Plan;$localCandidate=Assert-MIR4A08CandidateAgainstPlan -RepoRoot $repo -Plan $Plan -CandidateRef ([string]$Candidate.ref) -CandidateCommit ([string]$Candidate.commit);if ([string]$Candidate.tree-cne[string]$localCandidate.tree-or[string]$Candidate.parent_commit-cne[string]$localCandidate.parent_commit){throw '[mir4-a08-candidate-caller-forged]'};$null=Assert-MIR4A08ConfiguredRemote -RepoRoot $repo -RemoteName $RemoteName -Plan $Plan -Rehearsal:$Rehearsal;if ((Get-MIR4A08RemoteRef -RepoRoot $repo -RemoteName $RemoteName -Ref 'refs/heads/main')-cne[string]$Plan.main_before.commit){throw '[mir4-a08-remote-main-ref-drift]'};if ((Get-MIR4A08RemoteRef -RepoRoot $repo -RemoteName $RemoteName -Ref $RemoteSourceRef)-cne[string]$Plan.source.commit){throw '[mir4-a08-remote-source-ref-drift]'};$qualification=Read-MIR4A08QualificationRecord -RepoRoot $repo -Plan $Plan -Candidate $localCandidate -RecordPath $QualificationRecordPath -PackageRoot $PackageRoot -Rehearsal:$Rehearsal;$intention=[pscustomobject][ordered]@{schema=1;kind='MIR42ProtectedMainPromotionIntentionV2';rehearsal=[bool]$Rehearsal;plan=$Plan;candidate=$localCandidate;qualification=$qualification;remote=[pscustomobject][ordered]@{name=$RemoteName;canonical_remote_url=[string]$Plan.policy.canonical_remote_url;main_ref='refs/heads/main';source_ref=$RemoteSourceRef;candidate_ref=[string]$localCandidate.ref};intention_sha256=''};$intention.intention_sha256=Get-MIR4A08SelfSha256 $intention 'intention_sha256';$path=Join-Path ([IO.Path]::GetFullPath($StateRoot))("$($intention.intention_sha256.ToLowerInvariant()).promotion-intention.json");$null=Write-MIR4A08ImmutableJson -Path $path -Value $intention -Code '[mir4-a08-intention-conflict]';return [pscustomobject][ordered]@{path=$path;intention_sha256=$intention.intention_sha256;candidate_commit=$localCandidate.commit;package_set_sha256=$qualification.package_set_sha256;remote_push_performed=$false;main_mutation_performed=$false;tag_created=$false;publication_authorized=$false}
}
