...
function New-MIRCPTodoLines {
  param(
    [Parameter(Mandatory)]$MIR4OperatingProgramme,
    [Parameter(Mandatory)]$MIR4ExecutionProgramme,
  ...
  $lines = [Collections.Generic.List[string]]::new()
  $mir4ReviewDate = if ($MIR4OperatingProgramme.recorded_at -is [DateTime]) {
    ([DateTime]$MIR4OperatingProgramme.recorded_at).ToString("yyyy-MM-dd")
  } else {
    Get-MIRCPAuthoredDate -Timestamp ([string]$MIR4OperatingProgramme.recorded_at)
  }
  foreach ($line in @(
    "# MIR Executable Queue", "", "> Generated by Control Plane v5 from typed records. Edit the records, not this file.", "",
    "Generated: $mir4ReviewDate", "", "## Active MIR 4.x operating programme", "",
    "Status: $(Format-MIRCPCode $MIR4OperatingProgramme.status). MIR 4.1 completes the repository, package, tooling, test, documentation, and maintainability fixed point; no source or distribution version is allocated by this queue.", "",
    "| Work package | Completion boundary | State | Dependencies | Outcome |", "| --- | --- | --- | --- | --- |"
  )) { $lines.Add($line) }
  foreach ($workPackage in @($MIR4OperatingProgramme.work_packages)) {
  ...
  foreach ($turn in @($MIR4ExecutionProgramme.turns)) {
    $lines.Add("| $(Format-MIRCPCode $turn.id) | $($turn.name) | $(Format-MIRCPCode $turn.state) | $(@($turn.depends_on) -join ', ') | $(Format-MIRCPCode ([string][bool]$turn.human_required).ToLowerInvariant()) |")
  }
  $lines.Add("")
  $lines.Add("### Current blockers")
  $lines.Add("")
  $lines.Add("| Blocker | Scope | State | Blocks |")
  $lines.Add("| --- | --- | --- | --- |")
  foreach ($blocker in @($MIR4ExecutionProgramme.blockers)) {
    $lines.Add("| $(Format-MIRCPCode $blocker.id) | $(Format-MIRCPCode $blocker.scope) | $(Format-MIRCPCode $blocker.state) | $(@($blocker.blocks) -join ', ') |")
  }
  $lines.Add("")
  $lines.Add("### MIR 3 residual disposition")
  $lines.Add("")
  $lines.Add("| Residual | Observed state | Required disposition | Release blocking |")
  $lines.Add("| --- | --- | --- | --- |")
  foreach ($residual in @($MIR4ExecutionProgramme.mir3_residuals)) {
    $lines.Add("| $(Format-MIRCPCode $residual.id) | $($residual.observed_state) | $($residual.required_disposition) | $(Format-MIRCPCode ([string][bool]$residual.release_blocking).ToLowerInvariant()) |")
  }
  $lines.Add("")
  $lines.Add("## Published terminal release history")
  $lines.Add("")
  $lines.Add("| Release | Candidate | Namespace / minimum next ordinal | State | Next required state |")
  $lines.Add("| --- | --- | --- | --- | --- |")
  ...
  $lines.Add("")
  $lines.Add("## Retained MIR 3 terminal programme")
  $lines.Add("")
  $lines.Add("The historical terminal release train, target dispositions, qualification boundaries, stop conditions, and MIR 4 handoff remain defined in the [MIR 3 Terminal .9 Programme](docs/releases/mir-3-terminal-dot-9-programme.md). This retained programme does not supersede the active MIR 4 queue above.")
  $lines.Add("")
...
function Update-MIRCPViews {
  ...
  $mir4OperatingProgramme = Read-MIRCPJson -Path "spec/programmes/mir4-4x-operating-programme-v1.json" -RepoRoot $repo
  $mir4ExecutionProgramme = Read-MIRCPJson -Path ".mir/releases/waves/mir4-r0/MIR4-Pre-Freeze-Execution-ProgrammeV1.json" -RepoRoot $repo
  ...
  Set-MIRCPGeneratedText -Path ([string]$policy.outputs.todo) -Lines (New-MIRCPTodoLines -MIR4OperatingProgramme $mir4OperatingProgramme -MIR4ExecutionProgramme $mir4ExecutionProgramme -TerminalReleases $terminalReleases -Changes $changes -Incidents $incidents -IncidentReconciliation $incidentReconciliation -Tasks $tasks -BaselineQueue $baselineQueue -ReviewDate $reviewDate -RepoRoot $repo) -RepoRoot $repo -Check:$Check
