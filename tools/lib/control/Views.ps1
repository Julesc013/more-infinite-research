...
function New-MIRCPTodoLines {
  ...
  $lines = [Collections.Generic.List[string]]::new()
  $mir4ReviewDate = if ($MIR4ExecutionProgramme.recorded_at -is [DateTime]) {
    ([DateTime]$MIR4ExecutionProgramme.recorded_at).ToString("yyyy-MM-dd")
  } else {
    Get-MIRCPAuthoredDate -Timestamp ([string]$MIR4ExecutionProgramme.recorded_at)
  }
  foreach ($line in @(
    "# MIR Executable Queue", "", "> Generated by Control Plane v5 from typed records. Edit the records, not this file.", "",
    "Generated: $mir4ReviewDate", "", "## Active MIR 4.0 pre-freeze programme", "",
...
function Update-MIRCPViews {
  ...
  Set-MIRCPGeneratedText -Path ([string]$policy.outputs.todo) -Lines (New-MIRCPTodoLines -MIR4ExecutionProgramme $mir4ExecutionProgramme -TerminalReleases $terminalReleases -Changes $changes -Incidents $incidents -IncidentReconciliation $incidentReconciliation -Tasks $tasks -BaselineQueue $baselineQueue -ReviewDate $reviewDate -RepoRoot $repo) -RepoRoot $repo -Check:$Check
