...
function New-MIRCPTodoLines {
  ...
  $lines = [Collections.Generic.List[string]]::new()
  $mir4ReviewDate = Get-MIRCPAuthoredDate -Timestamp ([string]$MIR4OperatingProgramme.recorded_at)
  foreach ($line in @(
...
function Update-MIRCPViews {
  ...
  $mir4OperatingProgramme = Read-MIRCPJson -Path "spec/programmes/mir4-4x-operating-programme-v1.json" -RepoRoot $repo -PreserveTimestamps
