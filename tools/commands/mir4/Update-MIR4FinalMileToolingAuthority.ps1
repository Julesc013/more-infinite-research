  ...
  if ($state.authority_hashes.ContainsKey($path)) {
    ...
    if ($actual -cne $previous) {
      $evolved.Add([ordered]@{
        ...
        reason = 'Repair the governed post-publication legacy alias controller, advance local-playtest assurance to the exact V3 authority, and prepare complete release copy without granting a release transition.'
