...
$migrationPath = 'governance/repository/migrations/fixed-point-tooling-v1.json'
$migrationReceiptPath = 'releases/migrations/MIR4-Repository-Fixed-Point-Tooling-MigrationV1.json'
...
$repositoryMigration = Read-RepoJson $migrationPath
$repositoryMigrationReceipt = Read-RepoJson $migrationReceiptPath
$ownership = Read-RepoJson $ownershipPath
$fixedResult = & (Join-Path $repo 'tools/mir/cli/Invoke-MIR4RepositoryFixedPoint.ps1') -Command check -RepoRoot $repo | Out-String | ConvertFrom-Json -Depth 100
$fixedRefs = @((New-AuthorityRef $fixedPath 'governing-authority'), (New-AuthorityRef $migrationPath 'migration-authority'), (New-AuthorityRef $migrationReceiptPath 'migration-receipt'), (New-AuthorityRef $pathsPath 'path-authority'), (New-AuthorityRef $ownershipPath 'ownership-authority'), (New-AuthorityRef 'tests/repository/Test-MIR4RepositoryFixedPoint.ps1' 'validator'))
Add-Record 'MIR4_AUTHORITY_MAP_FINAL.json' (New-MIR4FullPlatformRecord -SourceIdentity $source -Kind 'MIR4AuthorityMapFinalV1' -Status 'accepted-one-writer-authority-map' -Maturity 'stable-governance' -AuthorityId 'mir4-control-plane-ownership' -AuthorityMode canonical -AuthoritySourcePaths @($ownershipPath,$modulesPath,$pathsPath,$migrationPath) -EvidenceRefs (@($fixedRefs)+(New-AuthorityRef $modulesPath 'module-boundary-authority')) -Payload ([ordered]@{ownership=$ownership;one_writable_authority_required=$true;repository_migration=$repositoryMigration;physical_cutover=$false}))
Add-Record 'MIR4_REPOSITORY_FIXED_POINT.json' (New-MIR4FullPlatformRecord -SourceIdentity $source -Kind 'MIR4RepositoryFixedPointFinalV1' -Status ([string]$fixed.state) -Maturity 'mixed' -AuthorityId 'mir4-repository-fixed-point-v2' -AuthorityMode canonical -AuthoritySourcePaths @($fixedPath,$migrationPath) -EvidenceRefs $fixedRefs -Payload ([ordered]@{authority=$fixed;migration=$repositoryMigration;receipt=$repositoryMigrationReceipt;observation=$fixedResult}))
$activatedRoots = @($repositoryMigration.activated_roots | ForEach-Object { [string]$_ })
$pathRows = @($fixed.visible_roots | ForEach-Object { [ordered]@{root=[string]$_.id;destination=[string]$_.path;mode=[string]$_.mode;current_authorities=@($_.current_authorities);writer_cutover=([string]$_.id -in $activatedRoots);deletion_authorized=$false} })
Add-Record 'MIR4_PATH_MIGRATION_MATRIX.json' (New-MIR4FullPlatformRecord -SourceIdentity $source -Kind 'MIR4PathMigrationMatrixFinalV1' -Status 'package-excluded-tooling-cutover-active-player-source-deferred' -Maturity 'mixed' -AuthorityId 'mir4-repository-fixed-point-v2' -AuthorityMode canonical -AuthoritySourcePaths @($fixedPath,$migrationPath,$pathsPath) -EvidenceRefs $fixedRefs -Payload ([ordered]@{rows=$pathRows;path_map=@($repositoryMigration.path_map);move_gate=@($fixed.move_gate);physical_cutover=$false;rollback=$repositoryMigration.rollback}))
$shimRows = @(
  @($repositoryMigration.compatibility_entrypoints | ForEach-Object { [ordered]@{root='repository-tooling';shim_path=[string]$_.path;mode=[string]$_.role;debt='compatibility-forwarder';removal_gate=@($repositoryMigration.sunset.required_gates)} })
  @($fixed.visible_roots | Where-Object { [string]$_.mode -match '^shadow' } | ForEach-Object { [ordered]@{root=[string]$_.id;shim_path=[string]$_.path;mode=[string]$_.mode;debt='bounded-shadow-reader';removal_gate=@($fixed.move_gate)} })
)
Add-Record 'MIR4_SHIM_DEBT_REGISTER.json' (New-MIR4FullPlatformRecord -SourceIdentity $source -Kind 'MIR4ShimDebtRegisterFinalV1' -Status 'compatibility-readers-retained-and-remaining-shadow-debt-bounded' -Maturity 'mixed' -AuthorityId 'mir4-repository-fixed-point-v2' -AuthorityMode canonical -AuthoritySourcePaths @($fixedPath,$migrationPath) -EvidenceRefs $fixedRefs -Payload ([ordered]@{rows=$shimRows;remaining_move=$fixed.remaining_move;writer_cutover_authorized_for=@($activatedRoots);package_visible_cutover_authorized=$false}))
