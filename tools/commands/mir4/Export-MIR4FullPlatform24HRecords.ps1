...
Add-Record 'MIR4_PUBLISHER_CONFINEMENT.json' (New-MIR4FullPlatformRecord -SourceIdentity $source -Kind 'MIR4PublisherConfinementFinalV1' -Status ([string]$readiness.publisher.status) -Maturity 'stable-governance' -AuthorityId 'mir4-release-governance-v1' -AuthorityMode canonical -AuthoritySourcePaths @($governancePath) -EvidenceRefs $governanceRefs -Payload ([ordered]@{publisher=$governance.publisher;observation=$readiness.publisher;publication_authorized=$false}))
...
$waveRows = @(
  ...
  [ordered]@{wave='W06';status='partial-with-bounded-blocker';blocking_ids=@('BLOCKED-EXACT-TARGET-PROCESSIR-SNAPSHOT')},
  [ordered]@{wave='W07';status='accepted-private-inspector-compatibility';blocking_ids=@('BLOCKED-INDEPENDENT-PRODUCTION-CONSUMER')},
  [ordered]@{wave='W08';status='partial-with-bounded-blocker';blocking_ids=@('BLOCKED-EXACT-TARGET-PROCESSIR-SNAPSHOT','BLOCKED-MISSING-TRUSTED-TIMING-CAPACITY-EVIDENCE')},
