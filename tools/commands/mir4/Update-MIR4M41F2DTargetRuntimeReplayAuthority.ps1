...
if ($Check) {
  ...
  Require-MIR4F2DTarget ([string]$proof.target -ceq $Target -and [string]$fresh.target -ceq $Target -and [string]$upgrade.target -ceq $Target -and [string]$verification.target -ceq $Target -and [string]$resource.target -ceq $Target -and [string]$custody.target -ceq $Target) 'mir4-m41-f2d-target-wrong-target-evidence'
