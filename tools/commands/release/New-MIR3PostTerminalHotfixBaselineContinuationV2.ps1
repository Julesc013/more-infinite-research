...
$releaseRecordPath = ".mir/releases/records/3.2.11.json"
$publicationPath = ".mir/evidence/terminal-publication/2026-08-18/github/3.2.11.json"
$qualificationPath = ".mir/releases/emergency/MIR3PostTerminalEmergencyHotfixLocalQualificationV2.json"
$sourceFreezePath = ".mir/releases/emergency/MIR3PostTerminalEmergencyHotfixSourceFreezeV2.json"
$reconstructionPath = ".mir/releases/emergency/MIR3PostTerminalEmergencyHotfixCandidateReconstructionV2.json"
$overridePath = ".mir/releases/emergency/MIR3PostTerminalEmergencyHotfixMaintainerReleaseOverrideV2.json"
...
$foundationManifestPath = ".mir/releases/terminal/baselines/3.2.10/baseline-manifest.json"
$foundationSnapshotPath = ".mir/releases/terminal/baselines/3.2.10/normalized-snapshot.json"
...
$finding33 = Read-Json $finding33Path
$finding34 = Read-Json $finding34Path
$distRelativePath = "dist/more-infinite-research_3.2.11.zip"
$distPath = Join-Path $RepoRoot $distRelativePath
if (-not (Test-Path -LiteralPath $distPath -PathType Leaf)) { throw "Published 3.2.11 distribution is absent." }
...
if ([string]$releaseRecord.state -cne "publicly-verified" -or [string]$publication.status -cne "published-and-verified" -or
  ...
  throw "Published 3.2.11 identity does not match its append-only release record."
}
...
$snapshotRecord = Add-RecordSha256 ([ordered]@{
  ...
  predecessors = @(
    [ordered]@{release="3.2.10";role="immutable-terminal-predecessor";upgrade_id="3.2.10-to-3.2.11"},
    [ordered]@{release="3.2.9";role="older-governed-predecessor";upgrade_id="3.2.9-to-3.2.11"},
    [ordered]@{release="3.2.5";role="older-governed-predecessor";upgrade_id="3.2.5-to-3.2.11"}
  )
  foundation = [ordered]@{release="3.2.10";manifest=$foundationManifestPath;root_sha256=[string]$foundationManifest.input_root_sha256}
  inventories = [ordered]@{
    features = [ordered]@{foundation_snapshot=$foundationSnapshotPath;package_composition=(Get-RelativePath $compositionPath);disposition="3.2.10-plus-approved-emergency-contract-v2-delta"}
    technologies = [ordered]@{foundation_snapshot=$foundationSnapshotPath;emergency_delta=".mir/releases/emergency/MIR3PostTerminalEmergencyHotfixChangeSetV2.json";closed_findings=@($finding33Path,$finding34Path);stable_ids_preserved=$true}
    settings = [ordered]@{foundation_snapshot=$foundationSnapshotPath;emergency_delta=".mir/releases/emergency/MIR3PostTerminalEmergencyHotfixChangeSetV2.json";released_id_or_type_changes=0;codec="MIRSET1"}
    owners_aliases_tombstones = [ordered]@{foundation_snapshot=$foundationSnapshotPath;emergency_delta=".mir/releases/emergency/MIR3PostTerminalEmergencyHotfixChangeSetV2.json";transactional_replacement=$true;open_follow_up=$null}
    migrations = [ordered]@{foundation_snapshot=$foundationSnapshotPath;watermark=[ordered]@{from=@("3.2.10","3.2.9","3.2.5");stable_ids=$true;to="3.2.11"}}
...
$authorityInputs = @(
  ...
  $finding33Path,
  $finding34Path,
  ".mir/releases/emergency/MIR3PostTerminalEmergencyHotfixAuthorizationV2.json",
  ".mir/releases/emergency/MIR3PostTerminalEmergencyHotfixProgrammeV2.json",
  ".mir/releases/emergency/MIR3PostTerminalEmergencyHotfixChangeSetV2.json",
  ".mir/releases/emergency/MIR3PostTerminalEmergencyHotfixTargetMatrixV2.json",
  ".mir/releases/transitions/3.2.11-c35-source-frozen.json",
  ".mir/releases/transitions/3.2.11-c35-package-built.json",
  "docs/releases/notes/release-notes-3.2.11.md",
...
$manifestRecord = Add-RecordSha256 ([ordered]@{
  schema = 1
  kind = "MIR3PostTerminalHotfixBaselineContinuationV2"
  release = $release
  target = "2.1"
  status = "captured-public-custody-pending-mir4-refresh"
  package_visible = $false
  capture_tool = [ordered]@{path="tools/commands/release/New-MIR3PostTerminalHotfixBaselineContinuationV2.ps1";version="2";sha256=(Get-FileBinding "tools/commands/release/New-MIR3PostTerminalHotfixBaselineContinuationV2.ps1" "capture-tool").sha256}
  foundation = [ordered]@{release="3.2.10";manifest=$foundationManifestPath;root_sha256=[string]$foundationManifest.input_root_sha256;retained_as_immutable_historical_snapshot=$true}
  ...
  continuation = [ordered]@{supersedes_for_factorio_2_1_predecessor="3.2.10";preserves_historical_baseline=$true;closed_findings=@("MIR3-TERM-0033","MIR3-TERM-0034");factorio_2_0_predecessor="2.5.11"}
...
$schemaPath = Join-Path $RepoRoot "spec\schemas\mir3-post-terminal-hotfix-baseline-continuation-v2.schema.json"
if (-not ((Get-Content -Raw -LiteralPath $manifestPath) | Test-Json -SchemaFile $schemaPath -ErrorAction Stop)) {
  throw "Generated 3.2.11 continuation manifest failed schema validation."
}
Write-Host "[ok] MIR 3.2.11 terminal baseline continuation: $($manifestRecord.record_sha256)"
