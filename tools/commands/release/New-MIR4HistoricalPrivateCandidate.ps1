...
$packageSourceSha256=Get-MIRPackageSourceFingerprint -RepoRoot $RepoRoot
...
foreach($row in $targets){
  ...
  $manifest=[pscustomobject][ordered]@{schema=1;kind='MIR4HistoricalPrivateCandidateManifestV1';status='built-unqualified-private-experimental';maturity='private-experimental-inventory';candidate_id='M4C02-09-24H';target_key=[string]$row.target_key;target_disposition=$(if([string]$row.target_key-eq'f018'){'private-experimental-bridge'}else{'private-experimental'});factorio_line=[string]$row.factorio_line;source_version='4.0.0';source_commit=$sourceCommit;source_tree=$sourceTree;package_source_sha256=$packageSourceSha256;distribution_version=[string]$row.distribution_version;package_root="more-infinite-research_$([string]$row.distribution_version)";predecessor=[ordered]@{release=[string]$row.predecessor_release;archive=[string]$row.predecessor_archive;sha256=[string]$row.predecessor_archive_sha256};projection=[string]$row.projection;builds=@($letters|ForEach-Object{[ordered]@{id=$_;sha256=$hashes[0]}});distribution=[ordered]@{path=[IO.Path]::GetRelativePath($RepoRoot,$distribution).Replace('\','/');sha256=$hashes[0];bytes=(Get-Item $distribution).Length;entries=@($inventories[0].entries).Count};runtime=[ordered]@{status='not-run';engine=$row.engine};public_claim=$false;publication_authorized=$false;source_frozen=$false;record_sha256=''}
