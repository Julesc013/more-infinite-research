...
function ConvertTo-MIRScenarioAuthorityValue {
  ...
  if ($Value -is [Collections.IDictionary]) {
    ...
    foreach ($key in @($Value.Keys | ForEach-Object { [string]$_ } | Sort-Object)) {
      $result[$key] = ConvertTo-MIRScenarioAuthorityValue -Value $Value[$key]
    }
    return $result
  }
  if ($Value -is [pscustomobject]) {
    ...
    foreach ($property in @($Value.PSObject.Properties | Sort-Object Name)) {
      $result[[string]$property.Name] = ConvertTo-MIRScenarioAuthorityValue -Value $property.Value
    }
    return $result
  }
  if ($Value -is [Collections.IEnumerable] -and $Value -isnot [string]) {
    return ,@($Value | ForEach-Object { ConvertTo-MIRScenarioAuthorityValue -Value $_ })
  }
  return $Value
}

function Get-MIRLiteralInvocations {
  param(
  ...
  if ($functionAst.Count -ne 1) {
    throw "Expected exactly one $FunctionName definition; found $($functionAst.Count)."
  }
  ...
  $calls = @($Ast.FindAll({
    param($node)
    $node -is [Management.Automation.Language.CommandAst] -and $node.GetCommandName() -eq $FunctionName
  }, $true))
  foreach ($call in $calls) {
    ...
    if ($isStatic -and $bound.Contains("ScenarioName")) {
      ...
      $invocations[[string]$bound["ScenarioName"]] = $bound
    }
  }
...
$runtime = Get-MIRLiteralInvocations -Ast $ast -FunctionName "Invoke-RuntimeScenario"
$configuration = Get-MIRLiteralInvocations -Ast $ast -FunctionName "Invoke-RuntimeConfigurationChangeScenario"
$manifest = Get-Content -Raw -LiteralPath $manifestPath | ConvertFrom-Json -Depth 100
if ([int]$manifest.schema -ne 3) { throw "Expected runtime scenario authority schema 3." }
$records = @($manifest.profiles.($factorioVersion))
if ($records.Count -eq 0) { throw "No runtime scenario authority exists for Factorio $factorioVersion." }

$runtimeNames = @($records | Where-Object kind -eq "runtime" | ForEach-Object { [string]$_.name })
$expectedLiteralOnly = @("reduced-settings-surface")
$literalOnly = @($runtime.Invocations.Keys | Where-Object { $_ -notin $runtimeNames } | Sort-Object)
...
$literalConfigurationNames = @($configuration.Invocations.Keys | Sort-Object)
...
$dynamicNames = @($runtimeNames | Where-Object { -not $runtime.Invocations.Contains($_) } | Sort-Object)
...
foreach ($record in $records) {
  if ([string]$record.kind -eq "configuration-change") {
    ...
    $invocation = $configuration.Invocations[$scenarioName]
    $record.fixtures = @(
    ...
    $settings = [ordered]@{}
    foreach ($parameterName in $configuration.Parameters.Values) {
      if ($parameterName -in @("ScenarioName", "EnableSpaceAge", "InitialFixtureNames", "ChangedFixtureNames")) { continue }
      if ($invocation.Contains($parameterName)) {
        $settings[$parameterName] = ConvertTo-MIRScenarioAuthorityValue -Value $invocation[$parameterName]
      }
    }
  ...
  $scenarioName = [string]$record.name
  if (-not $runtime.Invocations.Contains($scenarioName)) {
    $preservedDynamic++
    continue
  }
  $invocation = $runtime.Invocations[$scenarioName]
  $record.fixtures = @($invocation["EnabledFixtureNames"] | ForEach-Object { [string]$_ } | Sort-Object -Unique)
  $settings = [ordered]@{}
  foreach ($parameterName in $runtime.Parameters.Values) {
...
if ($Check) {
  if ($actual -cne $expected) {
    throw "Runtime scenario authority differs from the static target-native runner. Run tools/commands/targets/Sync-MIRRuntimeScenarioAuthority.ps1."
  }
