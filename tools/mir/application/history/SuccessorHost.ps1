...
function Get-MIR4W09SyntheticArchiveContentIdentity {
    ...
    $entries=@(
      foreach($entry in @($zip.Entries|Sort-Object FullName)){
        ...
        try{
          ...
          try{$sha256=[Convert]::ToHexString($algorithm.ComputeHash($stream))}finally{$algorithm.Dispose()}
        }finally{$stream.Dispose()}
        [pscustomobject][ordered]@{path=[string]$entry.FullName;bytes=[long]$entry.Length;sha256=$sha256}
      }
    )
  }finally{$zip.Dispose()}
  $material=[pscustomobject][ordered]@{schema=1;kind='MIR4SyntheticSuccessorArchiveContentV1';entries=$entries}
  $json=ConvertTo-MIR4PlatformCanonicalJson $material
  ...
  [long]$uncompressedBytes=0;foreach($entry in $entries){$uncompressedBytes+=[long]$entry.bytes}
  return [ordered]@{content_sha256=$digest;entry_count=$entries.Count;uncompressed_bytes=$uncompressedBytes}
}
