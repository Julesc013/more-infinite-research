...
function Get-MIR4ProcessIRInputSha256 {
  ...
  catch { throw "[mir4-processir-input-encoding] $Path" }
  ...
  $channels = @(
    foreach ($source in @($Authority.effect_channels | Sort-Object id -CaseSensitive)) {
      ...
      $row = [ordered]@{
        ...
        owner_ref=[ordered]@{path=[string]$source.owner_path;sha256=(Get-MIR4ProcessIRInputSha256 $ownerPath)}
...
function New-MIR4W06Records {
  ...
  $exactRefs=@()
  foreach($property in @('receipt','manifest')){$relative=[string]$exact[$property];$path=Join-Path $repo $relative;if(-not(Test-Path -LiteralPath $path -PathType Leaf)){throw "[mir4-w06-exact-reference-missing] $relative"};$exactRefs+=[ordered]@{path=$relative;sha256=(Get-MIR4ProcessIRInputSha256 $path)}}
