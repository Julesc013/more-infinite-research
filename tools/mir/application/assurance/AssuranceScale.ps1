...
function New-MIR4W08ProofCover {
  ...
  while($uncovered.Count){
    ...
    foreach($candidate in @($Candidates|Sort-Object id)){
      if([string]$candidate.id-in@($selected|ForEach-Object{[string]$_.id})){continue}
      $eligible=@()
...
function New-MIR4W08AssuranceScaleResult {
  ...
  $impact=Get-MIR4W08ImpactProjection -ChangedPaths @('tools/mir/application/assurance/AssuranceScale.ps1') -RepoRoot $repo
