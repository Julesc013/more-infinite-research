if (-not (Get-Command Get-MIR4ModuleDigest -ErrorAction SilentlyContinue)) {
  . (Join-Path $PSScriptRoot '../extensions/ModuleEcosystem.ps1')
}
...
function Get-MIR4InspectorCompatibilityAuthority {
  ...
  if (-not ($text | Test-Json -SchemaFile (Join-Path $repo 'spec/schemas/mir4-inspector-compatibility-programme-v1.schema.json'))) { throw '[mir4-w07-authority-schema]' }
  ...
  $expectedApplications = @('tools/mir/application/inspection/SupportAssessment.ps1','tools/mir/application/inspection/CompatibilityIndex.ps1','tools/mir/application/inspection/CompatibilityFactory.ps1','tools/mir/application/inspection/Inspector.ps1')
  $expectedCommands = @('tools/mir/cli/Export-MIR4InspectorCompatibilityRecords.ps1')
  $expectedCompatibility = @('tools/lib/mir4/SupportAssessment.ps1','tools/lib/mir4/CompatibilityIndex.ps1','tools/lib/mir4/CompatibilityFactory.ps1','tools/lib/mir4/Inspector.ps1','tools/commands/mir4/Export-MIR4InspectorCompatibilityRecords.ps1')
  if ((@($authority.tooling.applications) -join "`n") -cne ($expectedApplications -join "`n") -or
      (@($authority.tooling.commands) -join "`n") -cne ($expectedCommands -join "`n") -or
      (@($authority.tooling.compatibility_entrypoints) -join "`n") -cne ($expectedCompatibility -join "`n") -or
      [string]$authority.tooling.migration_authority -cne 'governance/repository/migrations/inspector-compatibility-tooling-v1.json' -or
      [bool]$authority.tooling.package_visible -or [bool]$authority.tooling.release_transition_authority) { throw '[mir4-w07-tooling-authority]' }
  foreach ($flag in @(
