if (-not (Get-Command Get-MIR4T12RecordDigest -ErrorAction SilentlyContinue)) {
  . (Join-Path $PSScriptRoot '../processir/ExactProcessIR.ps1')
}
if (-not (Get-Command Invoke-FactorioProcess -ErrorAction SilentlyContinue)) {
  . (Join-Path $PSScriptRoot '../../../lib/validation/FactorioProcess.ps1')
}
...
function Get-MIR4T13Authority {
  ...
  if(-not($text|Test-Json -SchemaFile (Join-Path $repo 'spec/schemas/mir4-release-compatibility-canaries-t13-v1.schema.json'))){throw '[mir4-t13-authority-schema]'}
  ...
  $expectedApplications=@('tools/mir/application/inspection/CompatibilityCanary.ps1');$expectedCommands=@('tools/mir/cli/Export-MIR4CompatibilityCanaryRecords.ps1');$expectedCompatibility=@('tools/lib/mir4/CompatibilityCanary.ps1','tools/commands/mir4/Export-MIR4CompatibilityCanaryRecords.ps1')
  if((@($value.tooling.applications)-join"`n")-cne($expectedApplications-join"`n")-or(@($value.tooling.commands)-join"`n")-cne($expectedCommands-join"`n")-or(@($value.tooling.compatibility_entrypoints)-join"`n")-cne($expectedCompatibility-join"`n")-or[string]$value.tooling.migration_authority-cne'governance/repository/migrations/inspector-compatibility-tooling-v1.json'-or[string]$value.tooling.reference_mode-cne'read-only-exact-historical-evidence'-or[bool]$value.tooling.package_visible-or[bool]$value.tooling.release_transition_authority){throw '[mir4-t13-tooling-authority]'}
