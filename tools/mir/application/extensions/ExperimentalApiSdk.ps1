...
function Test-MIR4ApiRecord {
  ...
  $extensionNamespaces = if ($Record.extensions -is [Collections.IDictionary]) {
    @($Record.extensions.Keys | ForEach-Object { [string]$_ })
  } else {
    @($Record.extensions.PSObject.Properties | ForEach-Object { [string]$_.Name })
  }
  foreach ($namespace in @($extensionNamespaces | Where-Object { $_ })) {
...
function Get-MIR4ModuleEcosystemSdkFiles {
  ...
  . (Join-Path $RepoRoot 'tools/mir/application/extensions/SdkV1.ps1')
