...
$receipt=if($Command-eq'generate'){Invoke-MIR4AssuranceOfflineCustodyMigrationProjectionV1 -RepoRoot $RepoRoot}elseif($Command-eq'check'){Invoke-MIR4AssuranceOfflineCustodyMigrationProjectionV1 -RepoRoot $RepoRoot -Check}else{Get-MIR4RepositoryJsonV1 -RepoRoot $RepoRoot -Path $script:MIR4AssuranceOfflineCustodyMigrationReceiptPath}
