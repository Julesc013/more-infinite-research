...
$receipt=if($Command-eq'generate'){
  Invoke-MIR4ModuleSdkMepMigrationProjectionV1 -RepoRoot $RepoRoot
}elseif($Command-eq'check'){
  Invoke-MIR4ModuleSdkMepMigrationProjectionV1 -RepoRoot $RepoRoot -Check
}else{
  Get-MIR4RepositoryJsonV1 -RepoRoot $RepoRoot -Path $script:MIR4ModuleSdkMepMigrationReceiptPath
}
$result=[ordered]@{
  schema=1;kind='MIR4ModuleSdkMepMigrationResultV1';migration_id=[string]$receipt.migration_id;state=[string]$receipt.state
  predecessor_receipt=[string]$receipt.predecessor_receipt.path;predecessor_sha256=[string]$receipt.predecessor_receipt.sha256
  receipt=$script:MIR4ModuleSdkMepMigrationReceiptPath;digest=[string]$receipt.digest
