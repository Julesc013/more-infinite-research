...
function Get-MIR4RepositoryFixedPointAuthority {
  ...
  if (@($authority.migration_sequence).Count -ne 16 -or
      ...
      [string]$authority.migration_sequence[9].state -cne 'accepted-immutable-predecessor' -or
      [string]$authority.migration_sequence[10].migration_id -cne 'MIR4-PROCESSIR-EXACT-TOOLING-V1' -or
      [string]$authority.migration_sequence[10].state -cne 'accepted-immutable-predecessor' -or
      [string]$authority.migration_sequence[11].migration_id -cne 'MIR4-INSPECTOR-COMPATIBILITY-TOOLING-V1' -or
      [string]$authority.migration_sequence[11].state -cne 'accepted-immutable-predecessor' -or
      [string]$authority.migration_sequence[12].migration_id -cne 'MIR4-ASSURANCE-OFFLINE-CUSTODY-TOOLING-V1' -or
      [string]$authority.migration_sequence[12].state -cne 'accepted-immutable-predecessor' -or
      [string]$authority.migration_sequence[13].migration_id -cne 'MIR4-HISTORICAL-TOOLING-V1' -or
      [string]$authority.migration_sequence[13].state -cne 'accepted-immutable-predecessor' -or
      [string]$authority.migration_sequence[14].migration_id -cne 'MIR4-RELEASE-TOOLING-V1' -or
      [string]$authority.migration_sequence[14].state -cne 'accepted-immutable-predecessor' -or
      [string]$authority.migration_sequence[15].migration_id -cne 'M41-03-CHANGE-AND-RELEASE-AUTHORITY-V1' -or
      [string]$authority.migration_sequence[15].state -cne 'current-append-only-successor') {
