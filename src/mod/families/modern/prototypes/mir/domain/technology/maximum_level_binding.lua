...
local function new_binding(input, options)
  ...
  local binding = {
    ...
    -- Retain scalar fields only as a migration projection. Player consumers
    -- must read the normalized records above, so a blocked V3 policy cannot
    -- be mistaken for an older permissive binding.
