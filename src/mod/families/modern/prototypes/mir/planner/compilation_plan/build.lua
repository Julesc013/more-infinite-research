...
local function sanitize_stream_artifact(stream_artifact, target_inventory)
  ...
  for _, source_row in ipairs(stream_artifact.rows or {}) do
    local row = model.derive_stream_row(source_row)
    if row.action == "emit" then
      ...
      if #removed > 0 then
        ...
        row.gates.effect_valid = model.shared_planning_gate(
...
local function apply_graph_decisions(stream_artifact, graph_summary)
    ...
    if row.action == "emit" then
      ...
      if rejection then
        ...
        if row.diagnostics then
          row.diagnostics = model.shallow_copy(row.diagnostics)
        ...
        row.gates.output_identity_safe = model.shared_planning_gate(
  ...
  return model.rebuild_stream_artifact(stream_artifact, rows)
end
...
local function apply_base_graph_decisions(base_operations, graph_summary)
  ...
  for _, operation in ipairs(base_operations or {}) do
    local updated = model.copy_operation_with_design_view(operation)
    updated.gates = {}
...
function M.finalize(stream_plan, base_plan, compiler_inputs)
  ...
  if not trusted_stream_artifact then validation.admit_stream_artifact(stream_artifact) end
  local exact_input = compiler_inputs and compiler_inputs.compiler_input
    or model.default_compiler_input(stream_artifact, base_plan, (compiler_inputs or {}).input_sanitation_ledger)
  ...
  for _, operation in ipairs(normalized_base) do
    table.insert(operations, model.copy_operation_with_design_view(operation))
  end
  ...
  local validation_summary = validation.operations(operations)
  ...
  artifact.compilation_fingerprint = plan_fingerprint.compilation(artifact)
  ...
  telemetry.count("technology_catalog_canonical_bytes",
    #fingerprint.canonical(technology_catalog.authority_projection(artifact.technology_catalog)))
  artifact.qualification_fingerprint = plan_fingerprint.qualification(artifact)
  artifact.semantic_fingerprint = artifact.qualification_fingerprint
  artifact.fingerprint = artifact.compilation_fingerprint
  local operation_fingerprints = plan_fingerprint.operation_fingerprints(operations)
  ...
  return plan_fingerprint.attach_run_evidence(artifact)
end
