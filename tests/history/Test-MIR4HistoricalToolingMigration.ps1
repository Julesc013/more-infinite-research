...
Assert-MIR4HistoricalToolingMigrationV1 ((Get-FileHash -LiteralPath (Join-Path $repo 'tools/lib/mir4/ReleaseDag.ps1') -Algorithm SHA256).Hash-ceq$script:MIR4HistoricalToolingReleaseDagSha256) 'mir4-historical-tooling-migration-release-dag'
...
foreach($binding in @($receipt.evolved_bindings)){$item=[string]$binding.path;Assert-MIR4HistoricalToolingMigrationV1 ($prior.authority_hashes.ContainsKey($item)-and[string]$binding.previous_sha256-ceq[string]$prior.authority_hashes[$item]) 'mir4-historical-tooling-migration-evolved-prior' $item;Assert-MIR4HistoricalToolingMigrationV1 ([string]$binding.current_sha256-ceq(Get-MIR4PreFreezeFileSha256 -Path (Join-Path $repo $item) -Mode ([string]$binding.hash_mode))) 'mir4-historical-tooling-migration-evolved-current' $item}
foreach($binding in @($receipt.current_authorities)){$actual=Get-MIR4PreFreezeFileSha256 -Path (Join-Path $repo ([string]$binding.path)) -Mode ([string]$binding.hash_mode);Assert-MIR4HistoricalToolingMigrationV1 ([string]$binding.sha256-ceq$actual) 'mir4-historical-tooling-migration-current-authority' ([string]$binding.path)}
...
Assert-MIR4HistoricalToolingMigrationV1 ($LASTEXITCODE-eq0-and$releaseHistoryOutput-match'append-only-historical-tooling-successor') 'mir4-historical-tooling-migration-release-history-successor' $releaseHistoryOutput
...
$generateResult=Invoke-MIR4HistoricalToolingMigrationCommandProbeV1 generate;$checkResult=Invoke-MIR4HistoricalToolingMigrationCommandProbeV1 check;$showResult=Invoke-MIR4HistoricalToolingMigrationCommandProbeV1 show
Assert-MIR4HistoricalToolingMigrationV1 ((ConvertTo-MIR4CanonicalJsonV1 $generateResult)-ceq(ConvertTo-MIR4CanonicalJsonV1 $checkResult)-and(ConvertTo-MIR4CanonicalJsonV1 $checkResult)-ceq(ConvertTo-MIR4CanonicalJsonV1 $showResult)) 'mir4-historical-tooling-migration-cli-parity'
