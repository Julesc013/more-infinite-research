...
if(Test-Path -LiteralPath $supplyChainSuccessorPath -PathType Leaf){
  ...
  Assert-MIR4M4202OfflineCustody ($supplyChainSuccessorRaw|Test-Json -SchemaFile (Join-Path $repo 'contracts/repository/mir4-m42-02-supply-chain-decomposition-v1.schema.json')) 'mir4-m42-02-offline-custody-supply-chain-successor-schema'
  $supplyChainSuccessor=$supplyChainSuccessorRaw|ConvertFrom-Json -Depth 100 -DateKind String
  Assert-MIR4M4202OfflineCustody (Test-MIR4BootstrapRecordHash -Record $supplyChainSuccessor) 'mir4-m42-02-offline-custody-supply-chain-successor-record'
  ...
  Assert-MIR4M4202OfflineCustody ((Get-FileHash -LiteralPath $supplyChainPredecessorPath -Algorithm SHA256).Hash-ceq[string]$supplyChainSuccessor.predecessor.receipt_sha256-and[string]$supplyChainPredecessor.record_sha256-ceq[string]$supplyChainSuccessor.predecessor.record_sha256) 'mir4-m42-02-offline-custody-supply-chain-successor-predecessor'
...
Assert-MIR4M4202OfflineCustody ((Test-MIR4M4202PackageSourceSuccession -RepoRoot $repo -PredecessorSha256 ([string]$receipt.preservation.package_source_sha256) -CurrentSha256 $packageBefore) -and @($receipt.preservation.package_visible_delta).Count -eq 0 -and (Get-MIR4CanonicalPackageSourceFingerprint -RepoRoot $repo) -ceq $packageBefore) 'mir4-m42-02-offline-custody-package-firewall'
