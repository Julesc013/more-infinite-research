...
if ($testMigrationPreview.mode -ne "historical-complete" -or $testMigrationPreview.changed -ne 0 -or
    $testMigrationPreview.tests -ne 65 -or $testMigrationPreview.canonical_root -ne "tests" -or
    $testMigrationPreview.compatibility_root -ne "validation/tests") {
...
$canonicalExecutableTests = @(Get-ChildItem -LiteralPath (Join-Path $repo "tests") -Filter "*.ps1" -Recurse -File)
$validationCompatibilityTests = @(Get-ChildItem -LiteralPath (Join-Path $repo "validation/tests") -Filter "*.ps1" -Recurse -File)
if ($legacyTestWrappers.Count -ne 65 -or $canonicalExecutableTests.Count -lt 167 -or $validationCompatibilityTests.Count -ne 3) {
  throw "Canonical test/wrapper inventory drifted: canonical=$($canonicalExecutableTests.Count), validation-forwarders=$($validationCompatibilityTests.Count), legacy-wrappers=$($legacyTestWrappers.Count)."
}
...
foreach ($wrapper in $validationCompatibilityTests) {
  $text = Get-Content -Raw -LiteralPath $wrapper.FullName
  if ($text -notmatch 'MIR4-.*-COMPATIBILITY-TEST' -or $text.Split([char]10).Count -gt 10 -or $text -notmatch 'tests[/\\]') {
...
$deprecatedReferenceExclusions = @(
  ...
  (Join-Path $repo "tests/tooling/Test-MIRLayout.ps1"),
  (Join-Path $repo "tests/tooling/Test-MIRPowerShellQuality.ps1")
)
...
$canonicalTest = Invoke-MIRCliProbe -Entrypoint (Join-Path $repo "tests/tooling/Test-MIRVerificationSchemas.ps1") -Arguments @("-RepoRoot", $repo)
