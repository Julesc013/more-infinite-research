...
foreach($file in $decompositionFiles){
  ...
  $expectedBindingSha[$path]=if($path-ceq[string]$receipt.decomposition.facade.path){[string]$receipt.decomposition.facade.previous_sha256}else{[string]$file.sha256}
}
foreach($binding in @($receipt.evolved_bindings)){
  $path=[string]$binding.path
  if($expectedBindingSha.ContainsKey($path)){
    Assert-MIR4CommandRouterDecompositionV1 ([string]$binding.previous_sha256-ceq[string]$expectedBindingSha[$path]) 'mir4-m42-02-command-router-binding-origin' $path
  }
