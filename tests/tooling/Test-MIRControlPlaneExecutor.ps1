...
$historicalInputTask = [pscustomobject][ordered]@{
  ...
  effective_inputs = @(
    "source:tests/compiler/Test-MIRResearchCostModels.ps1",
...
$historicalAbsence = @($historicalInputManifest.rows | Where-Object {
  [string]$_.input -eq "source:tests/compiler/Test-MIRResearchCostModels.ps1" -and
...
$context = New-MIRCPVerificationContext -Mode calibrate-fresh -Target "2.1" -Release "3.2.2" -CandidatePath $candidate `
  -SourceRepoRoot $performanceSource -OutputRoot "build/results/control-plane-v5-self-test/executor-contexts" -RepoRoot $repo
...
$evidenceRoot = "build/results/control-plane-v5-self-test/executor-evidence/$([guid]::NewGuid().ToString('N'))"
