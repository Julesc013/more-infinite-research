...
Assert-MIR4M4202StreamCompiler (@($manifest.bindings).Count-eq[int]$currentPresentation.package_source.binding_count) 'manifest-binding-count'
Assert-MIR4M4202StreamCompiler (@($manifest.bindings|ForEach-Object{"$($_.layer)|$($_.output_path)"}|Sort-Object -Unique).Count-eq[int]$currentPresentation.package_source.unique_binding_count) 'manifest-binding-uniqueness'
