...
foreach($binding in @($receipt.evolved_bindings)){$item=[string]$binding.path;Assert-MIR4AssuranceOfflineCustodyMigrationV1 ($prior.authority_hashes.ContainsKey($item)-and[string]$binding.previous_sha256-ceq[string]$prior.authority_hashes[$item]) 'mir4-assurance-offline-custody-migration-evolved-prior' $item}
...
Assert-MIR4AssuranceOfflineCustodyMigrationV1 ($LASTEXITCODE-eq0-and$releaseHistoryOutput-match'append-only-historical-tooling-successor') 'mir4-assurance-offline-custody-migration-release-history-successor' $releaseHistoryOutput
...
Assert-MIR4AssuranceOfflineCustodyMigrationV1 ((ConvertTo-MIR4CanonicalJsonV1 $checkResult)-ceq(ConvertTo-MIR4CanonicalJsonV1 $showResult)) 'mir4-assurance-offline-custody-migration-cli-parity'
