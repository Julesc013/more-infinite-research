...
$result=Test-MIR4M41ToM42ComposableSourceSuccessionV3 -RepoRoot $repo
...
if([string]$record.current.tooling_inventory_predecessor_sha256-cne'CC28AA34B139C0B082BAF07E81829D797F4A2B303AA31A7C3A6FD3EC526F834A'-or
   [string]$record.current.tooling_inventory.sha256-cne'59F98A68D36A085DEDF47B9B60AA8C4157BF6BB9A7A0D3FB4331B0DC772BF35D'-or
   [string]$record.current.tooling_inventory.digest-cne'sha256:5d29c319fe855bd1a9a051884dba5f3b36eb24dcdd2e297a4173fa8cb12b6efe'){throw '[mir4-m41-m42-succession-v3-tooling-inventory]'}
...
$rejected=$false;try{Test-MIR4M41ToM42ComposableSourceSuccessionV3 -RepoRoot $repo -SuccessionRecord $tampered|Out-Null}catch{$rejected=$_.Exception.Message -match 'v3-(historical-integrity|gate)'}
...
$rejected=$false;try{Test-MIR4M41ToM42ComposableSourceSuccessionV3 -RepoRoot $repo -SuccessionRecord $tampered|Out-Null}catch{$rejected=$_.Exception.Message -match 'v3-historical-integrity'}
