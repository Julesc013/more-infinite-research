...
$currentRegistryRecord = Get-JsonRecord ".mir/releases/waves/mir4-r0/MIR4-Target-RegistryV4.json"
...
$currentRefreshRecord = Get-JsonRecord ".mir/releases/waves/mir4-r0/MIR4-Terminal-Predecessor-RefreshV3.json"
...
Assert-True ([string]$currentRefreshRecord.Value.kind -ceq "MIR4-Terminal-Predecessor-RefreshV3" -and
  [string]$currentRefreshRecord.Value.status -ceq "accepted-current-pre-eol-package-excluded" -and
  [string]$currentRefreshRecord.Value.payload.factorio_2_1.predecessor_release -ceq "3.2.11" -and
  [string]$currentRefreshRecord.Value.payload.factorio_2_0.predecessor_release -ceq "2.5.11" -and
...
Assert-True ($currentF200RegistryRows.Count -eq 1 -and
  [string]$currentF200RegistryRows[0].mir3_predecessor -ceq "2.5.11") "Target Registry V4 does not advance the current f200 predecessor to 2.5.11."
...
Write-Host "[ok] MIR 4 historical readiness and current 3.2.11 / 2.1.14 predecessor refresh are fail-closed"
