...
foreach ($row in $rows) {
  ...
  foreach ($reference in @($row.current_tests) + @($row.current_evidence)) {
    ...
    if (@($pathPrefixes | Where-Object { $text.StartsWith($_, [StringComparison]::Ordinal) }).Count -gt 0) {
      $resolved = $text
      if (-not (Test-Path -LiteralPath (Join-Path $RepoRoot $resolved))) {
        $resolved = [string](Resolve-MIRRepoPath -RepoRoot $RepoRoot -Path $text).relative_path
      }
      if (-not (Test-Path -LiteralPath (Join-Path $RepoRoot $resolved))) {
