...
if (Test-Path -LiteralPath $activeReleasePath -PathType Leaf) {
  ...
  if ($activeVersion -eq "3.2.10" -and [string]$activeRelease.state -in $admittedEmergencyStates) {
    ...
    if ([string]$changeSet.status -ne "one-finding-admitted" -or
      ...
      throw "The 3.2.10 emergency change set, maintainer override, or exact candidate delta boundary is invalid."
    }
...
if ($activeVersion -eq "3.2.2") {
  ...
  & (Join-Path $repo "tests\release\Test-MIRApprovedPatchDelta.ps1") @arguments
  exit $LASTEXITCODE
}
