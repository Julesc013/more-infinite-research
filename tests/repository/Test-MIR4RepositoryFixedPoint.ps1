...
Assert-MIR4RepositoryMigrationV1 ([string]$authority.remaining_move.classification -ceq 'none-progression-source-succession-static-complete-exact-engine-qualification-required') 'mir4-repository-migration-debt-class'
Assert-MIR4RepositoryMigrationV1 (@($authority.migration_sequence).Count -eq 22) 'mir4-repository-migration-sequence-count'
...
Assert-MIR4RepositoryMigrationV1 ([string]$authority.migration_sequence[19].migration_id -ceq 'MIR4-M41-TO-M42-COMPOSABLE-SOURCE-SUCCESSION-V2' -and [string]$authority.migration_sequence[19].state -ceq 'accepted-immutable-predecessor') 'mir4-repository-migration-sequence-factorio-one-predecessor'
Assert-MIR4RepositoryMigrationV1 ([string]$authority.migration_sequence[20].migration_id -ceq 'MIR4-M41-TO-M42-COMPOSABLE-SOURCE-SUCCESSION-V3' -and [string]$authority.migration_sequence[20].state -ceq 'accepted-immutable-predecessor') 'mir4-repository-migration-sequence-progression-predecessor'
Assert-MIR4RepositoryMigrationV1 ([string]$authority.migration_sequence[21].migration_id -ceq 'MIR4-M41-TO-M42-COMPOSABLE-SOURCE-SUCCESSION-V4' -and [string]$authority.migration_sequence[21].authority -ceq 'spec/distribution/mir4-current-package-presentation-v5.json' -and [string]$authority.migration_sequence[21].receipt -ceq 'assurance/repository/mir4-m41-to-m42-composable-source-succession-v4.json' -and [string]$authority.migration_sequence[21].state -ceq 'current-append-only-successor') 'mir4-repository-migration-sequence-proof-input-successor'
...
$preBridgeReceipt = Get-MIR4RepositoryJsonV1 -RepoRoot $repo -Path 'releases/migrations/MIR4-M42-02-Supply-Chain-DecompositionV1.json'
$preBridgePackageSourceSha256 = [string]$preBridgeReceipt.preservation.package_source_sha256
      ...
      if(Test-Path -LiteralPath (Join-Path $repo $m4202L4Path) -PathType Leaf){
        ...
        if($m4202L5Exists){
          ...
          if(Test-Path -LiteralPath (Join-Path $repo $m4202L6Path) -PathType Leaf){
            ...
            Assert-MIR4RepositoryMigrationV1 ([string]$m4202L6.predecessor.package_source_sha256-ceq[string]$m4202L5.package_authority.package_source_sha256-and[string]$m4202L6.package_authority.package_source_sha256-ceq$preBridgePackageSourceSha256) 'mir4-repository-migration-m42-02-l6-successor-fingerprint'
            ...
            if(Test-Path -LiteralPath (Join-Path $repo $m4202PowerShellPath) -PathType Leaf){
              ...
              Assert-MIR4RepositoryMigrationV1 ([string]$m4202PowerShell.preservation.package_source_sha256-ceq$preBridgePackageSourceSha256-and@($m4202PowerShell.preservation.package_visible_delta).Count-eq0) 'mir4-repository-migration-m42-02-powershell-characterization-package-firewall'
              ...
              if(Test-Path -LiteralPath (Join-Path $repo $m4202CommandRouterPath) -PathType Leaf){
                ...
                Assert-MIR4RepositoryMigrationV1 ([string]$m4202CommandRouter.preservation.package_source_sha256-ceq$preBridgePackageSourceSha256-and@($m4202CommandRouter.preservation.package_visible_delta).Count-eq0) 'mir4-repository-migration-m42-02-command-router-package-firewall'
                ...
                if(Test-Path -LiteralPath (Join-Path $repo $m4202ValidationRunnerPath) -PathType Leaf){
                  ...
                  Assert-MIR4RepositoryMigrationV1 ([string]$m4202ValidationRunner.preservation.package_source_sha256-ceq$preBridgePackageSourceSha256-and@($m4202ValidationRunner.preservation.package_visible_delta).Count-eq0) 'mir4-repository-migration-m42-02-validation-runner-package-firewall'
                  ...
                  if(Test-Path -LiteralPath (Join-Path $repo $m4202AssuranceEvidencePath) -PathType Leaf){
                    ...
                    Assert-MIR4RepositoryMigrationV1 ([string]$m4202AssuranceEvidence.preservation.package_source_sha256-ceq$preBridgePackageSourceSha256-and@($m4202AssuranceEvidence.preservation.package_visible_delta).Count-eq0) 'mir4-repository-migration-m42-02-assurance-evidence-package-firewall'
                    ...
                    if(Test-Path -LiteralPath (Join-Path $repo $m4202PreFreezeReleasePath) -PathType Leaf){
                      ...
                      Assert-MIR4RepositoryMigrationV1 ([string]$m4202PreFreezeRelease.preservation.package_source_sha256-ceq$preBridgePackageSourceSha256-and@($m4202PreFreezeRelease.preservation.package_visible_delta).Count-eq0) 'mir4-repository-migration-m42-02-pre-freeze-release-package-firewall'
                      ...
                      if(Test-Path -LiteralPath (Join-Path $repo $m4202BootstrapMaterializationPath) -PathType Leaf){
                        ...
                        Assert-MIR4RepositoryMigrationV1 ([string]$m4202BootstrapMaterialization.preservation.package_source_sha256-ceq$preBridgePackageSourceSha256-and@($m4202BootstrapMaterialization.preservation.package_visible_delta).Count-eq0) 'mir4-repository-migration-m42-02-bootstrap-materialization-package-firewall'
                        ...
                        if(Test-Path -LiteralPath (Join-Path $repo $m4202AssuranceReleasePath) -PathType Leaf){
                          ...
                          Assert-MIR4RepositoryMigrationV1 ([string]$m4202AssuranceRelease.preservation.package_source_sha256-ceq$preBridgePackageSourceSha256-and@($m4202AssuranceRelease.preservation.package_visible_delta).Count-eq0) 'mir4-repository-migration-m42-02-assurance-release-package-firewall'
                          ...
                          if(Test-Path -LiteralPath (Join-Path $repo $m4202CompatibilityAuditPath) -PathType Leaf){
                            ...
                            Assert-MIR4RepositoryMigrationV1 ([string]$m4202CompatibilityAudit.preservation.package_source_sha256-ceq$preBridgePackageSourceSha256-and@($m4202CompatibilityAudit.preservation.package_visible_delta).Count-eq0) 'mir4-repository-migration-m42-02-compatibility-audit-package-firewall'
                            ...
                            if(Test-Path -LiteralPath (Join-Path $repo $m4202OfflineCustodyPath) -PathType Leaf){
                              ...
                              Assert-MIR4RepositoryMigrationV1 ([string]$m4202OfflineCustody.preservation.package_source_sha256-ceq$preBridgePackageSourceSha256-and@($m4202OfflineCustody.preservation.package_visible_delta).Count-eq0) 'mir4-repository-migration-m42-02-offline-custody-package-firewall'
                              ...
                              $m4202ReleaseCapsulePath='releases/migrations/MIR4-M42-02-Release-Capsule-DecompositionV1.json'
                              if(Test-Path -LiteralPath (Join-Path $repo $m4202ReleaseCapsulePath) -PathType Leaf){
                                $m4202ReleaseCapsuleRaw=Get-Content -Raw -LiteralPath (Join-Path $repo $m4202ReleaseCapsulePath)
                                Assert-MIR4RepositoryMigrationV1 ($m4202ReleaseCapsuleRaw|Test-Json -SchemaFile (Join-Path $repo 'contracts/repository/mir4-m42-02-release-capsule-decomposition-v1.schema.json')) 'mir4-repository-migration-m42-02-release-capsule-schema'
                                $m4202ReleaseCapsule=$m4202ReleaseCapsuleRaw|ConvertFrom-Json -Depth 100 -DateKind String
                                Assert-MIR4RepositoryMigrationV1 (Test-MIR4BootstrapRecordHash -Record $m4202ReleaseCapsule) 'mir4-repository-migration-m42-02-release-capsule-record'
                                Assert-MIR4RepositoryMigrationV1 ((Get-MIR4RepositoryFileSha256V1 -Path (Join-Path $repo $m4202OfflineCustodyPath))-ceq[string]$m4202ReleaseCapsule.predecessor.receipt_sha256-and[string]$m4202ReleaseCapsule.predecessor.record_sha256-ceq[string]$m4202OfflineCustody.record_sha256) 'mir4-repository-migration-m42-02-release-capsule-predecessor'
                                Assert-MIR4RepositoryMigrationV1 ([string]$m4202ReleaseCapsule.status-ceq'M42-02-PS9-RELEASE-CAPSULE-DECOMPOSED'-and[string]$m4202ReleaseCapsule.decomposition.responsibility-ceq'release-capsule'-and[string]$m4202ReleaseCapsule.next_fixed_point-ceq'M42-02-PS10-CONTROL-EXECUTOR') 'mir4-repository-migration-m42-02-release-capsule-scope'
                                Assert-MIR4RepositoryMigrationV1 ([bool]$m4202ReleaseCapsule.public_contract.unchanged-and[int]$m4202ReleaseCapsule.public_contract.function_count-eq19-and@($m4202ReleaseCapsule.decomposition.modules).Count-eq8-and[bool]$m4202ReleaseCapsule.semantic_contract.ordered_source_slices_preserved-and[bool]$m4202ReleaseCapsule.semantic_contract.capsule_construction_unchanged-and[bool]$m4202ReleaseCapsule.semantic_contract.capsule_verification_unchanged-and[bool]$m4202ReleaseCapsule.semantic_contract.offline_restore_unchanged) 'mir4-repository-migration-m42-02-release-capsule-contract'
                                Assert-MIR4RepositoryMigrationV1 ([string]$m4202ReleaseCapsule.preservation.package_source_sha256-ceq$preBridgePackageSourceSha256-and@($m4202ReleaseCapsule.preservation.package_visible_delta).Count-eq0) 'mir4-repository-migration-m42-02-release-capsule-package-firewall'
                                Assert-MIR4RepositoryMigrationV1 (@($m4202ReleaseCapsule.transition_gate.PSObject.Properties|Where-Object{[bool]$_.Value}).Count-eq0) 'mir4-repository-migration-m42-02-release-capsule-release-firewall'
                                $m4202ControlExecutorPath='releases/migrations/MIR4-M42-02-Control-Executor-DecompositionV1.json'
                                if(Test-Path -LiteralPath (Join-Path $repo $m4202ControlExecutorPath) -PathType Leaf){
                                  $m4202ControlExecutorRaw=Get-Content -Raw -LiteralPath (Join-Path $repo $m4202ControlExecutorPath)
                                  Assert-MIR4RepositoryMigrationV1 ($m4202ControlExecutorRaw|Test-Json -SchemaFile (Join-Path $repo 'contracts/repository/mir4-m42-02-control-executor-decomposition-v1.schema.json')) 'mir4-repository-migration-m42-02-control-executor-schema'
                                  $m4202ControlExecutor=$m4202ControlExecutorRaw|ConvertFrom-Json -Depth 100 -DateKind String
                                  Assert-MIR4RepositoryMigrationV1 (Test-MIR4BootstrapRecordHash -Record $m4202ControlExecutor) 'mir4-repository-migration-m42-02-control-executor-record'
                                  Assert-MIR4RepositoryMigrationV1 ((Get-MIR4RepositoryFileSha256V1 -Path (Join-Path $repo $m4202ReleaseCapsulePath))-ceq[string]$m4202ControlExecutor.predecessor.receipt_sha256-and[string]$m4202ControlExecutor.predecessor.record_sha256-ceq[string]$m4202ReleaseCapsule.record_sha256) 'mir4-repository-migration-m42-02-control-executor-predecessor'
                                  Assert-MIR4RepositoryMigrationV1 ([string]$m4202ControlExecutor.status-ceq'M42-02-PS10-CONTROL-EXECUTOR-DECOMPOSED'-and[string]$m4202ControlExecutor.decomposition.responsibility-ceq'control-executor'-and[string]$m4202ControlExecutor.next_fixed_point-ceq'M42-02-PS11-SUPPLY-CHAIN') 'mir4-repository-migration-m42-02-control-executor-scope'
                                  Assert-MIR4RepositoryMigrationV1 ([bool]$m4202ControlExecutor.public_contract.unchanged-and[int]$m4202ControlExecutor.public_contract.function_count-eq27-and@($m4202ControlExecutor.decomposition.modules).Count-eq6-and[bool]$m4202ControlExecutor.semantic_contract.ordered_current_source_slices_preserved-and[bool]$m4202ControlExecutor.semantic_contract.runtime_measurements_unchanged-and[bool]$m4202ControlExecutor.semantic_contract.aggregate_gate_unchanged-and[bool]$m4202ControlExecutor.semantic_contract.ps7_source_evolution_preserved) 'mir4-repository-migration-m42-02-control-executor-contract'
                                  Assert-MIR4RepositoryMigrationV1 ([string]$m4202ControlExecutor.preservation.package_source_sha256-ceq$preBridgePackageSourceSha256-and@($m4202ControlExecutor.preservation.package_visible_delta).Count-eq0) 'mir4-repository-migration-m42-02-control-executor-package-firewall'
                                  Assert-MIR4RepositoryMigrationV1 (@($m4202ControlExecutor.transition_gate.PSObject.Properties|Where-Object{[bool]$_.Value}).Count-eq0) 'mir4-repository-migration-m42-02-control-executor-release-firewall'
                                  $m4202SupplyChainPath='releases/migrations/MIR4-M42-02-Supply-Chain-DecompositionV1.json'
                                  if(Test-Path -LiteralPath (Join-Path $repo $m4202SupplyChainPath) -PathType Leaf){
                                    $m4202SupplyChainRaw=Get-Content -Raw -LiteralPath (Join-Path $repo $m4202SupplyChainPath)
                                    Assert-MIR4RepositoryMigrationV1 ($m4202SupplyChainRaw|Test-Json -SchemaFile (Join-Path $repo 'contracts/repository/mir4-m42-02-supply-chain-decomposition-v1.schema.json')) 'mir4-repository-migration-m42-02-supply-chain-schema'
                                    $m4202SupplyChain=$m4202SupplyChainRaw|ConvertFrom-Json -Depth 100 -DateKind String
                                    Assert-MIR4RepositoryMigrationV1 (Test-MIR4BootstrapRecordHash -Record $m4202SupplyChain) 'mir4-repository-migration-m42-02-supply-chain-record'
                                    Assert-MIR4RepositoryMigrationV1 ((Get-MIR4RepositoryFileSha256V1 -Path (Join-Path $repo $m4202ControlExecutorPath))-ceq[string]$m4202SupplyChain.predecessor.receipt_sha256-and[string]$m4202SupplyChain.predecessor.record_sha256-ceq[string]$m4202ControlExecutor.record_sha256) 'mir4-repository-migration-m42-02-supply-chain-predecessor'
                                    Assert-MIR4RepositoryMigrationV1 ([string]$m4202SupplyChain.status-ceq'M42-02-PS11-SUPPLY-CHAIN-DECOMPOSED-POWERSHELL-SEQUENCE-COMPLETE'-and[string]$m4202SupplyChain.decomposition.responsibility-ceq'supply-chain'-and[string]$m4202SupplyChain.next_fixed_point-ceq'M41-BRIDGE-RETIREMENT') 'mir4-repository-migration-m42-02-supply-chain-scope'
                                    Assert-MIR4RepositoryMigrationV1 ([bool]$m4202SupplyChain.public_contract.unchanged-and[int]$m4202SupplyChain.public_contract.function_count-eq28-and@($m4202SupplyChain.decomposition.modules).Count-eq5-and[bool]$m4202SupplyChain.semantic_contract.ordered_current_source_slices_preserved-and[bool]$m4202SupplyChain.semantic_contract.component_inventory_unchanged-and[bool]$m4202SupplyChain.semantic_contract.spdx_attestation_unchanged-and[bool]$m4202SupplyChain.semantic_contract.slsa_provenance_unchanged-and[bool]$m4202SupplyChain.semantic_contract.powershell_decomposition_sequence_complete) 'mir4-repository-migration-m42-02-supply-chain-contract'
                                    Assert-MIR4RepositoryMigrationV1 ([string]$m4202SupplyChain.programme_transition.current_state-ceq'complete'-and[string]$m4202SupplyChain.programme_transition.next_programme_state-ceq'queued'-and[bool]$m4202SupplyChain.programme_transition.mir41_qualification_still_required) 'mir4-repository-migration-m42-02-supply-chain-programme-transition'
                                    Assert-MIR4RepositoryMigrationV1 ([string]$m4202SupplyChain.preservation.package_source_sha256-ceq$preBridgePackageSourceSha256-and@($m4202SupplyChain.preservation.package_visible_delta).Count-eq0) 'mir4-repository-migration-m42-02-supply-chain-package-firewall'
                                    Assert-MIR4RepositoryMigrationV1 (@($m4202SupplyChain.transition_gate.PSObject.Properties|Where-Object{[bool]$_.Value}).Count-eq0) 'mir4-repository-migration-m42-02-supply-chain-release-firewall'
                                  }
                                }
                              }
                            }
...
$currentPackageContract = Assert-MIR4CurrentPackageContract -RepoRoot $repo -RequiredPackageSourceSha256 $currentPackageSourceSha256
Assert-MIR4RepositoryMigrationV1 ([string]$sourceSuccession.record_sha256 -ceq 'F4C9D487B5E1A304978020391069DC7EE8BF84C0FB5946C87E78B45C033CCEFB' -and [string]$currentPackageContract.package_source_sha256 -ceq $currentPackageSourceSha256 -and [bool]$sourceSuccession.factorio_one_exact_engine_proof_required -and -not [bool]$sourceSuccession.current_release_operations_authorized) 'mir4-repository-migration-factorio-one-current-successor'
