...
if(@($manifest.bindings).Count-ne448-or@($manifest.bindings|ForEach-Object{"$($_.layer)|$($_.output_path)"}|Sort-Object -Unique).Count-ne448-or@($manifest.bindings.source_path|Sort-Object -Unique).Count-ne448){throw '[mir4-editable-source-binding-uniqueness]'}
...
foreach($target in @('f210','f200','f110','f100')){
  ...
  $expectedDelta=if($modern){33}else{0}
