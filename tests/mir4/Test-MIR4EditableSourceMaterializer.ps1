...
if(@($manifest.bindings).Count-ne434-or@($manifest.bindings|ForEach-Object{"$($_.layer)|$($_.output_path)"}|Sort-Object -Unique).Count-ne434-or@($manifest.bindings.source_path|Sort-Object -Unique).Count-ne434){throw '[mir4-editable-source-binding-uniqueness]'}
