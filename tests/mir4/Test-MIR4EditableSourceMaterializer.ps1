...
if(@($manifest.bindings).Count-ne429-or@($manifest.bindings|ForEach-Object{"$($_.layer)|$($_.output_path)"}|Sort-Object -Unique).Count-ne429-or@($manifest.bindings.source_path|Sort-Object -Unique).Count-ne429){throw '[mir4-editable-source-binding-uniqueness]'}
