...
if($packageBefore-cne$preT14Package){
  if($packageBefore-ceq$t14Package){
    ...
    try{Assert-MIR4CurrentPackagePresentation -RepoRoot $RepoRoot -PackageSourceSha256 $packageBefore|Out-Null}catch{throw '[mir4-t12-package-source-unknown-evolution]'}
  }
}
