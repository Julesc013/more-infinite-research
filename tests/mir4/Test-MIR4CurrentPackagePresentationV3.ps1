...
if (
  ...
  [string]$rows[0].source_identity.commit -cne 'b6cf5f19d24f4474ee9dbb0473e59ce9caa8b75d' -or
  [string]$rows[0].source_identity.tree -cne '35b1c9bfbd734ccbddac9b217d15ef7f8d4b9780' -or
  [int]$rows[0].package_source.binding_count -ne 448 -or
  [int]$rows[0].package_source.unique_binding_count -ne 448 -or
  [string]$current.source_identity.commit -cne [string]$rows[-1].ledger_predecessor.commit -or
  [string]$current.source_identity.tree -cne [string]$rows[-1].ledger_predecessor.tree -or
  [int]$rows[-1].ledger_predecessor.row_count -ne ($rows.Count - 1) -or
  [string]$rows[-1].ledger_predecessor.final_row_sha256 -cne [string]$rows[-2].row_sha256
...
$secondRow = $twoRowLedger.rows[-1] | ConvertTo-Json -Depth 100 | ConvertFrom-Json -Depth 100 -DateKind String
$secondRow.sequence = @($twoRowLedger.rows).Count + 1
$secondRow.row_id = 'mir4-current-package-presentation-v3-{0:D4}' -f [int]$secondRow.sequence
$secondRow.previous_row_sha256 = [string]$twoRowLedger.rows[-1].row_sha256
...
$twoRowLedger.rows = @($twoRowLedger.rows) + @($secondRow)
...
if ([string](Get-MIR4CurrentPackagePresentationV3FinalRow -Ledger $twoRowLedger).row_id -cne [string]$secondRow.row_id) {
...
$trailingDeletion.rows = @($trailingDeletion.rows | Select-Object -SkipLast 1)
