...
if($packageBefore-cne$preT14Package){
  if($packageBefore-ceq$t14Package){
    ...
    try{Assert-MIR4CurrentPackagePresentationV3 -RepoRoot $RepoRoot -PackageSourceSha256 $packageBefore|Out-Null}catch{throw '[mir4-t13-package-source-unknown-evolution]'}
  }
}
