...
function New-A08Qualification {
  ...
  $issuer='synthetic-independent-verifier';$digest=('A'*64)
  ...
  $producer=[pscustomobject][ordered]@{repository='Julesc013/more-infinite-research';workflow='synthetic-independent-verification';run_id='synthetic-run';run_attempt='1';job=$issuer;actor='synthetic';workflow_commit=[string]$Candidate.commit;workflow_ref='refs/heads/main';source_commit=[string]$Candidate.commit;source_tree=[string]$Candidate.tree;event='synthetic-merge';environment='synthetic';runner_identity='synthetic';trust_class='synthetic-protected-release';verifier_sha256=('E'*64);policy_sha256=('F'*64)}
  ...
  $record=[pscustomobject][ordered]@{
    schema=1
    kind='MIR42ProtectedMainQualificationBindingV3'
    ...
    authority=[pscustomobject][ordered]@{kind='MIR4GovernedIndependentQualificationAuthorityV1';issuer_id=$issuer;trust_policy='validation/trust.json';producer=$producer}
...
'@
  ...
  $badEngine=$qualification|ConvertTo-Json -Depth 100|ConvertFrom-Json -Depth 100;$badEngine.targets[0].worker_receipt.engine.sha256='not-a-sha256';$badEngine.targets[0].worker_receipt.record_sha256=Get-MIR4A08SelfSha256 $badEngine.targets[0].worker_receipt 'record_sha256';$badEngine.record_sha256=Get-MIR4A08SelfSha256 $badEngine 'record_sha256';$badEnginePath=Join-Path $testRoot 'bad-engine.json';Write-A08Json $badEnginePath $badEngine;Expect-A08Failure {Read-MIR4A08QualificationRecord -RepoRoot $testRoot -Plan $plan -Candidate $candidate -RecordPath $badEnginePath -PackageRoot $packageRoot -Rehearsal} '[mir4-a08-qualification-record]'
  $differentEngine=$qualification|ConvertTo-Json -Depth 100|ConvertFrom-Json -Depth 100;$differentEngine.targets[0].independent_receipt.engine_sha256=('A'*64);$differentEngine.targets[0].independent_receipt.record_sha256=Get-MIR4A08SelfSha256 $differentEngine.targets[0].independent_receipt 'record_sha256';$differentEngine.record_sha256=Get-MIR4A08SelfSha256 $differentEngine 'record_sha256';$differentEnginePath=Join-Path $testRoot 'different-engine.json';Write-A08Json $differentEnginePath $differentEngine;Expect-A08Failure {Read-MIR4A08QualificationRecord -RepoRoot $testRoot -Plan $plan -Candidate $candidate -RecordPath $differentEnginePath -PackageRoot $packageRoot -Rehearsal} '[mir4-a08-qualification-record]'
  ...
  $productionAuthority=$qualification.authority|ConvertTo-Json -Depth 100|ConvertFrom-Json -Depth 100
  $productionAuthority.issuer_id='verification-gate';$productionAuthority.producer.workflow='MIR Control Plane v5';$productionAuthority.producer.run_id='123';$productionAuthority.producer.run_attempt='1';$productionAuthority.producer.job='verification-gate';$productionAuthority.producer.actor='synthetic';$productionAuthority.producer.workflow_commit=$source;$productionAuthority.producer.workflow_ref='refs/heads/dev';$productionAuthority.producer.source_commit=[string]$candidate.commit;$productionAuthority.producer.source_tree=[string]$candidate.tree;$productionAuthority.producer.event='workflow_dispatch';$productionAuthority.producer.environment='release-candidate';$productionAuthority.producer.runner_identity='self-hosted-windows';$productionAuthority.producer.trust_class='protected-release';$productionAuthority.producer.policy_sha256=Get-MIR4A08FileSha256 (Join-Path $repo 'validation/trust.json')
  ...
  $artifactName='mir-v5-qualification-source-123-attempt-1';$goodArtifacts=[pscustomobject]@{total_count=1;artifacts=@([pscustomobject][ordered]@{id=42;name=$artifactName;size_in_bytes=1024;expired=$false;workflow_run=[pscustomobject]@{id=123}})}
  $goodAttestation=[pscustomobject][ordered]@{schema=1;kind='MIR4QualificationSourceAttestationV1';repository='Julesc013/more-infinite-research';workflow='MIR Control Plane v5';workflow_run_id='123';workflow_run_attempt='1';workflow_job='verification-gate';workflow_ref='refs/heads/dev';workflow_commit=$source;source_ref=[string]$candidate.commit;source_commit=[string]$candidate.commit;source_tree=[string]$candidate.tree;context_id=('A'*64)}
...
if "%~1"=="run" (
  ...
  >"%~9\qualification-source-attestation.json" echo %A08_ATTESTATION_JSON%
  exit /b 0
)
...
'@,[Text.UTF8Encoding]::new($false))
  $qualificationProvider=New-MIR4A08GitHubRestQualificationAuthorityProvider -GhExecutable $fakeGh;$null=Assert-MIR4A08TrustedIssuer -Authority $productionAuthority -Candidate $candidate -CanonicalRepository 'Julesc013/more-infinite-research' -QualificationAuthorityProvider $qualificationProvider
  $productionAuthority.producer.run_id='124';Expect-A08Failure {Assert-MIR4A08TrustedIssuer -Authority $productionAuthority -Candidate $candidate -CanonicalRepository 'Julesc013/more-infinite-research' -QualificationAuthorityProvider $qualificationProvider} '[mir4-a08-qualification-authority-provider]';$productionAuthority.producer.run_id='123'
  $badRunner=$goodJobs|ConvertTo-Json -Depth 10|ConvertFrom-Json -Depth 10;$badRunner.jobs[0].labels=@('Windows');$env:A08_JOBS_JSON=$badRunner|ConvertTo-Json -Depth 10 -Compress;Expect-A08Failure {Assert-MIR4A08TrustedIssuer -Authority $productionAuthority -Candidate $candidate -CanonicalRepository 'Julesc013/more-infinite-research' -QualificationAuthorityProvider $qualificationProvider} '[mir4-a08-qualification-authority-provider]';$env:A08_JOBS_JSON=$goodJobs|ConvertTo-Json -Depth 10 -Compress
  $badRef=$goodRun|ConvertTo-Json -Depth 10|ConvertFrom-Json -Depth 10;$badRef.head_branch='side';$env:A08_RUN_JSON=$badRef|ConvertTo-Json -Depth 10 -Compress;Expect-A08Failure {Assert-MIR4A08TrustedIssuer -Authority $productionAuthority -Candidate $candidate -CanonicalRepository 'Julesc013/more-infinite-research' -QualificationAuthorityProvider $qualificationProvider} '[mir4-a08-qualification-authority-provider]';$env:A08_RUN_JSON=$goodRun|ConvertTo-Json -Depth 10 -Compress
  $badSource=$goodAttestation|ConvertTo-Json -Depth 10|ConvertFrom-Json -Depth 10;$badSource.source_commit=('0'*40);$env:A08_ATTESTATION_JSON=$badSource|ConvertTo-Json -Depth 10 -Compress;Expect-A08Failure {Assert-MIR4A08TrustedIssuer -Authority $productionAuthority -Candidate $candidate -CanonicalRepository 'Julesc013/more-infinite-research' -QualificationAuthorityProvider $qualificationProvider} '[mir4-a08-qualification-authority-provider]';$env:A08_ATTESTATION_JSON=$goodAttestation|ConvertTo-Json -Depth 10 -Compress
