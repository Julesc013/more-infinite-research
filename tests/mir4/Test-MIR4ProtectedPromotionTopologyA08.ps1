...
'@
  ...
  $presentation=[pscustomobject][ordered]@{
    ...
    package_source=[pscustomobject][ordered]@{fingerprint_sha256=('C'*64);materializer_abi='mir4-target-materializer/1';roots=@('source','targets');sole_writer='tools/mir/application/package/TargetMaterializer.ps1';legacy_root_state='retired-historical-read-only'}
  ...
  $badRunnerGh=Join-Path $testRoot 'bad-runner-gh.cmd';$oldCandidate=$env:A08_CANDIDATE;$env:A08_CANDIDATE=[string]$candidate.commit;[IO.File]::WriteAllText($badRunnerGh,"@echo off`r`nsetlocal EnableExtensions`r`nif `"%~2`"==`"-X`" (`r`n  echo {`"jobs`":[{`"name`":`"MIR / verification-gate`",`"status`":`"completed`",`"conclusion`":`"success`",`"runner_name`":`"synthetic-runner`",`"labels`":[`"Windows`"]}]}`r`n  exit /b 0`r`n)`r`necho {`"id`":123,`"name`":`"MIR Control Plane v5`",`"event`":`"workflow_dispatch`",`"head_sha`":`"%A08_CANDIDATE%`",`"run_attempt`":1,`"status`":`"completed`",`"conclusion`":`"success`",`"actor`":{`"login`":`"synthetic`"}}`r`n",[Text.UTF8Encoding]::new($false));$badRunnerProvider=New-MIR4A08GitHubRestQualificationAuthorityProvider -GhExecutable $badRunnerGh;Expect-A08Failure {Assert-MIR4A08TrustedIssuer -Authority $productionAuthority -Candidate $candidate -CanonicalRepository 'Julesc013/more-infinite-research' -QualificationAuthorityProvider $badRunnerProvider} '[mir4-a08-qualification-authority-provider]';if($null-eq$oldCandidate){Remove-Item Env:A08_CANDIDATE}else{$env:A08_CANDIDATE=$oldCandidate}
