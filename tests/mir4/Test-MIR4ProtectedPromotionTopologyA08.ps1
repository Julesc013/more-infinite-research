...
'@
  ...
  $productionAuthority.issuer_id='verification-gate';$productionAuthority.producer.workflow='MIR Control Plane v5';$productionAuthority.producer.run_id='123';$productionAuthority.producer.run_attempt='1';$productionAuthority.producer.job='verification-gate';$productionAuthority.producer.actor='synthetic';$productionAuthority.producer.ref='refs/heads/dev';$productionAuthority.producer.event='workflow_dispatch';$productionAuthority.producer.environment='release-candidate';$productionAuthority.producer.runner_identity='self-hosted-windows';$productionAuthority.producer.trust_class='protected-release';$productionAuthority.producer.policy_sha256=Get-MIR4A08FileSha256 (Join-Path $repo 'validation/trust.json')
  $oldCandidate=$env:A08_CANDIDATE;$env:A08_CANDIDATE=[string]$candidate.commit
  $goodRunJson='{"id":123,"name":"MIR Control Plane v5","event":"workflow_dispatch","head_sha":"%A08_CANDIDATE%","head_branch":"dev","run_attempt":1,"status":"completed","conclusion":"success","actor":{"login":"synthetic"}}'
  $goodJobsJson='{"jobs":[{"name":"MIR / verification-gate","status":"completed","conclusion":"success","runner_name":"synthetic-runner","labels":["self-hosted","Windows"]}]}'
  $fakeGh=Join-Path $testRoot 'fake-gh.cmd';[IO.File]::WriteAllText($fakeGh,"@echo off`r`nsetlocal EnableExtensions`r`nif `"%~2`"==`"-X`" (`r`n  echo $goodJobsJson`r`n  exit /b 0`r`n)`r`necho $goodRunJson`r`n",[Text.UTF8Encoding]::new($false))
  ...
  $badRunnerJobsJson='{"jobs":[{"name":"MIR / verification-gate","status":"completed","conclusion":"success","runner_name":"synthetic-runner","labels":["Windows"]}]}'
  $badRunnerGh=Join-Path $testRoot 'bad-runner-gh.cmd';[IO.File]::WriteAllText($badRunnerGh,"@echo off`r`nsetlocal EnableExtensions`r`nif `"%~2`"==`"-X`" (`r`n  echo $badRunnerJobsJson`r`n  exit /b 0`r`n)`r`necho $goodRunJson`r`n",[Text.UTF8Encoding]::new($false))
  $badRunnerProvider=New-MIR4A08GitHubRestQualificationAuthorityProvider -GhExecutable $badRunnerGh;Expect-A08Failure {Assert-MIR4A08TrustedIssuer -Authority $productionAuthority -Candidate $candidate -CanonicalRepository 'Julesc013/more-infinite-research' -QualificationAuthorityProvider $badRunnerProvider} '[mir4-a08-qualification-authority-provider]'
  $badRefRunJson=$goodRunJson.Replace('"head_branch":"dev"','"head_branch":"side"');$badRefGh=Join-Path $testRoot 'bad-ref-gh.cmd';[IO.File]::WriteAllText($badRefGh,"@echo off`r`nsetlocal EnableExtensions`r`nif `"%~2`"==`"-X`" (`r`n  echo $goodJobsJson`r`n  exit /b 0`r`n)`r`necho $badRefRunJson`r`n",[Text.UTF8Encoding]::new($false));$badRefProvider=New-MIR4A08GitHubRestQualificationAuthorityProvider -GhExecutable $badRefGh;Expect-A08Failure {Assert-MIR4A08TrustedIssuer -Authority $productionAuthority -Candidate $candidate -CanonicalRepository 'Julesc013/more-infinite-research' -QualificationAuthorityProvider $badRefProvider} '[mir4-a08-qualification-authority-provider]'
  if($null-eq$oldCandidate){Remove-Item Env:A08_CANDIDATE}else{$env:A08_CANDIDATE=$oldCandidate}
