...
Assert-MIR4ComposableSourceModel (@($model.bindings).Count -eq 447 -and @($model.targets).Count -eq 4) 'mir4-composable-source-model-cardinality'
...
Assert-MIR4ComposableSourceModel ([string]$cli.status -ceq 'passed-canonical-composable-source-model' -and [int]$cli.bindings -eq 447 -and [int]$cli.targets -eq 4) 'mir4-composable-source-model-cli'
