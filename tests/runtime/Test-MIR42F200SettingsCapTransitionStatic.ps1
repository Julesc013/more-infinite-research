...
function Assert-MIR42F200BaseOnlyHarnessContract([string]$Text){
  ...
  Assert-MIR42F200Static ($engineClosure-match'Loading mod' -and $engineClosure-match'\$expectedEngineLoadedModNames' -and $engineClosure-match'\$officialExpansionMods' -and $engineClosure-match'loaded_mods') 'engine-loaded closure assertion is incomplete.'
