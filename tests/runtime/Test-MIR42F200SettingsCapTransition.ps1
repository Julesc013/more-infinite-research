...
$candidateClosure=@('src/mod','targets','tools/mir/application/package','tools/lib/validation/PackageIdentity.ps1','spec/schemas/mir4-canonical-package-authority-v1.schema.json','spec/schemas/mir4-package-composition-result-v1.schema.json')
...
function New-Stage([string]$Name,[int]$Cap){
  ...
  $modList=[ordered]@{mods=@('base','more-infinite-research',$fixtureName,'mir-validation-settings-overrides'|ForEach-Object{[ordered]@{name=$_;enabled=$true}})}
...
Assert-State $seedState seed 0 0 $true $false;Assert-State $cappedState capped 3 1 $false $false;Assert-State $relaxedState relaxed 0 2 $true $false
