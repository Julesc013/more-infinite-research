...
if ([string]$info.factorio_version -notin @("0.13", "0.14", "0.15", "0.16", "0.17", "0.18", "1.0", "1.1")) {
  throw "Reduced compiler contracts apply only to the maintained Factorio 0.13-1.1 lines."
}
...
$baseExtensionMarkers = if ([string]$info.factorio_version -in @("0.13", "0.14")) {
  @("research_cost_model.new", "research_cost_model.evaluate", "target-finite-substitute")
} else {
  @("research_cost_model.new", "count_formula = cost_model.count_formula")
}
$contracts = @(
  ...
  @{Path="prototypes\mir\emit\base_extensions.lua"; Markers=$baseExtensionMarkers},
...
if ([string]$info.factorio_version -in @("0.13", "0.14")) {
  $contracts += @(
    @{Path="prototypes\mir\platform\factorio\target_line.lua"; Markers=@("supports_native_infinite_technology", "return false", "finite_research_count")},
    @{Path="prototypes\mir\emit\technology_builder.lua"; Markers=@("unit.count = target_line.finite_research_count", "supports_native_infinite_technology")}
  )
}
