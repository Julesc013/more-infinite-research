...
function Invoke-MIRFactorioLoadCheck {
  ...
  if (-not (Test-Path -LiteralPath $factorioReadData)) {
    throw "Unable to find Factorio read-data directory for compatibility audit config: $factorioReadData"
  }
  $configPath = Join-Path $UserDataDir "mir-compat-config.ini"
  $configText = @"
