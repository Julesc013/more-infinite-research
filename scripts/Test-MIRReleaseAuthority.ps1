...
$c11Authority = [ordered]@{
  candidate_id = "C11"
  archive = "dist/more-infinite-research_3.2.0.zip"
  archive_bytes = 1000145
  archive_entries = 286
  package_source_commit = "dc4b1e12695dc915666b04caddbabce570942a74"
  package_source_tree = "68e37991bf360ba437101f042835ca7dfc14bc4f"
  package_source_sha256 = "A25F2978289D3BB761F513FB7CD4EBDAB25E2C7320BA91A2FCD74969E9E005BA"
  archive_sha256 = "3AA9155A74CF904ADFE5B5F79F1F130CFFC39047CD4F787C4B57D65338C02FED"
  package_content_sha256 = "A25F2978289D3BB761F513FB7CD4EBDAB25E2C7320BA91A2FCD74969E9E005BA"
}
foreach ($field in $c11Authority.Keys) {
  if ([string]$modern.$field -ne [string]$c11Authority[$field]) {
    throw "Canonical C11 authority field '$field' changed. C12 is required if candidate bytes change."
  }
}
if ([int]$modern.package_source_material.schema -ne 1 -or
  ...
  throw "Canonical C11 package-source material descriptor must bind its clean source tree and 286 package files."
}
& git -C $repo merge-base --is-ancestor ([string]$modern.package_source_commit) HEAD
if ($LASTEXITCODE -ne 0) { throw "C11 package-source commit is not an ancestor of release-engineering HEAD." }
...
if ($LASTEXITCODE -ne 0 -or $packageSourceTree.Count -ne 1 -or [string]$packageSourceTree[0] -ne [string]$modern.package_source_tree) {
  throw "C11 package-source tree does not match the canonical authority row."
}
...
if ($LASTEXITCODE -ne 0 -or $changedPackagePaths.Count -gt 0) {
  throw "Package-visible paths changed after immutable C11 package source: $($changedPackagePaths -join ', ')"
}
if ((Get-MIRPackageSourceFingerprint -RepoRoot $repo) -ne [string]$modern.package_source_sha256) {
  throw "Current package roots do not reproduce the canonical C11 package-source identity."
}
...
if (-not (Test-Path -LiteralPath $candidatePath -PathType Leaf) -or
  ...
  throw "Canonical C11 archive no longer matches its immutable authority row."
}
$superseded = $modern.supersedes_candidate
if ([string]$superseded.candidate_id -ne "C10" -or [long]$superseded.archive_bytes -ne 1000085 -or
    [int]$superseded.archive_entries -ne 286 -or
    [string]$superseded.archive_sha256 -ne "C1B41367811CA9C9E41A5612AAC8B9789B9EBEE1F3C71F7CA5A4440E83871695" -or
    [string]$superseded.package_content_sha256 -ne "3C80C5C72AA1E08325464315B902FA95090221177ADABCE2B0A18083404DEF66" -or
    [string]$superseded.package_source_commit -ne "6f9297bae1aee2e018ae7298e0661e99aa591028" -or
    [string]$superseded.package_source_tree -ne "c589d2956dc844557cc0483bb1b0a190995e3940" -or
    [string]$superseded.package_source_sha256 -ne "3C80C5C72AA1E08325464315B902FA95090221177ADABCE2B0A18083404DEF66") {
  throw "C11 must retain the complete immutable C10 authority as its superseded candidate."
}
$delta = Read-MIRText ".mir/evidence/3.2.0-c10-to-c11-delta.json" | ConvertFrom-Json
if ([int]$delta.schema -ne 1 -or [string]$delta.record_type -ne "MIRCandidateArchiveDelta" -or [string]$delta.status -ne "PASS" -or
  ...
  throw "Tracked C10-to-C11 archive delta does not match the two immutable candidate authorities."
}
$deltaPaths = @($delta.changes | ForEach-Object { [string]$_.path } | Sort-Object)
$expectedDeltaPaths = @("changelog.txt", "prototypes/mir/providers/pipeline/discovery.lua")
if (($deltaPaths -join "`n") -ne ($expectedDeltaPaths -join "`n") -or @($delta.changes | Where-Object { -not [bool]$_.allowed }).Count -ne 0) {
  throw "C10-to-C11 delta must contain only the exact candidate-seed correction and shipped changelog entry."
}
...
foreach ($required in @(
  ...
  @{Path=".mir/branches.yml"; Text=$branches; Pattern='(?ms)^\s*mir_3_2_0:\s*$.*?^\s*candidate_id:\s*C11\s*$'},
  @{Path=".mir/branches.yml"; Text=$branches; Pattern='(?m)^\s*archive_sha256:\s*3AA9155A74CF904ADFE5B5F79F1F130CFFC39047CD4F787C4B57D65338C02FED\s*$'},
  @{Path=".mir/branches.yml"; Text=$branches; Pattern='(?m)^\s*package_source_commit:\s*dc4b1e12695dc915666b04caddbabce570942a74\s*$'},
  @{Path=".mir/branches.yml"; Text=$branches; Pattern='tmp/2\.0'},
  @{Path=".mir/release-wave.yml"; Text=$releaseWave; Pattern='(?m)^\s*mir_3_2_0:\s*$'},
  @{Path=".mir/release-wave.yml"; Text=$releaseWave; Pattern='(?ms)^\s*mir_3_2_0:\s*$.*?^\s*candidate_id:\s*C11\s*$'},
  @{Path=".mir/release-wave.yml"; Text=$releaseWave; Pattern='(?m)^\s*archive_sha256:\s*3AA9155A74CF904ADFE5B5F79F1F130CFFC39047CD4F787C4B57D65338C02FED\s*$'},
  @{Path=".mir/release-wave.yml"; Text=$releaseWave; Pattern='(?m)^\s*package_source_commit:\s*dc4b1e12695dc915666b04caddbabce570942a74\s*$'},
