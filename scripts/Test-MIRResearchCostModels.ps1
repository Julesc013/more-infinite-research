...
$requiredSources = @(
  ...
  "prototypes/mir/domain/research_cost/compatibility_slice.lua",
  "prototypes/mir/emit/research_cost_compatibility_adapter.lua"
)
...
foreach ($token in @(
  "mir-research-cost-v1", "mir-research-cost-transition-v1", "mir-research-cost-qualification-v1",
  "mir-research-cost-algebraic-proof-v1", "fixed", "linear", "exponential", "hybrid",
  "[MIR-RESEARCH-COST-SUPPORT]", "validation-log"
)) {
...
$target = $targets.profiles."2.0"
if ($null -eq $target -or [bool]$target.prototype_shapes.mod_data -or
    [bool]$target.features.productivity_family_adoption -or
    @($target.emitter_families | Where-Object { [string]$_ -eq "adoption" }).Count -ne 0) {
  throw "Factorio 2.0 must use validation-log support with mod-data output and productivity-family adoption compiled out."
}

Write-Host "[ok] unified Factorio 2.0 research-cost model, sixteen transitions, and compiled-out target boundaries passed."
