...
<!-- MIR legacy RC plan start: tmp/0.6 -->
### `tmp/0.6` Factorio `0.6` RC Planning
...
- [x] Add `docs/notes/rc-plan-factorio-0.6.md` for the target-line API/code/docs audit.
- [ ] Read docs/notes/rc-plan-factorio-0.6.md before any implementation change on tmp/0.6.
- [ ] Keep tmp/0.6 documentation-only until a target implementation pass is explicitly started.
- [ ] Use factorio-data tag 0.6.4 and the report source links as first compatibility contract.
- [ ] Close unsupported-surface blockers before creating an RC package.
- [ ] Validate with a real Factorio 0.6.x binary or document the missing binary before public archive.
- [ ] After 1.9.9, fold durable findings into permanent legacy and delete tmp/0.6 only when remote refs are no longer needed.
- [ ] Acquire/configure a 0.6.4 binary before implementation.
- [ ] Build a one-tech package and confirm mod loader accepts metadata.
<!-- MIR legacy RC plan end: tmp/0.6 -->
