...
<!-- MIR legacy RC plan start: tmp/0.10 -->
### `tmp/0.10` Factorio `0.10` RC Planning
...
- [x] Add `docs/notes/rc-plan-factorio-0.10.md` for the target-line API/code/docs audit.
- [ ] Read docs/notes/rc-plan-factorio-0.10.md before any implementation change on tmp/0.10.
- [ ] Keep tmp/0.10 documentation-only until a target implementation pass is explicitly started.
- [ ] Use factorio-data tag 0.10.12 and the report source links as first compatibility contract.
- [ ] Close unsupported-surface blockers before creating an RC package.
- [ ] Validate with a real Factorio 0.10.x binary or document the missing binary before public archive.
- [ ] After 1.9.9, fold durable findings into permanent legacy and delete tmp/0.10 only when remote refs are no longer needed.
- [ ] Locate 0.10 mod structure/package requirements.
- [ ] Create a single finite technology proof package for 0.10.12.
<!-- MIR legacy RC plan end: tmp/0.10 -->
