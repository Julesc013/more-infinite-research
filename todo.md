...
<!-- MIR legacy RC plan start: tmp/0.7 -->
### `tmp/0.7` Factorio `0.7` RC Planning
...
- [x] Add `docs/notes/rc-plan-factorio-0.7.md` for the target-line API/code/docs audit.
- [ ] Read docs/notes/rc-plan-factorio-0.7.md before any implementation change on tmp/0.7.
- [ ] Keep tmp/0.7 documentation-only until a target implementation pass is explicitly started.
- [ ] Use factorio-data tag 0.7.5 and the report source links as first compatibility contract.
- [ ] Close unsupported-surface blockers before creating an RC package.
- [ ] Validate with a real Factorio 0.7.x binary or document the missing binary before public archive.
- [ ] After 1.9.9, fold durable findings into permanent legacy and delete tmp/0.7 only when remote refs are no longer needed.
- [ ] Find or reconstruct the 0.7 technology prototype schema.
- [ ] Create a one-technology 0.7.5 smoke package.
<!-- MIR legacy RC plan end: tmp/0.7 -->
